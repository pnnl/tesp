...
- ``generate_case.py`` (For generating a year's worth of runs, one month at a time)
   ...
   - NOTE: the naming convention for the runs generated by ``generate_case.py`` are
     ``[node]_[year]_[month]_[flag(s)]`` such that for the 8 node case with solar,
     battery, electric vehicles, and flexible loads available, the January case
     folder would be ``8_2016_01_pv_bt_fl_ev``.

- ``prepare_case_dsot.py`` (For generating a run less than a month long,
  with duration set in ``8_hi_system_case_config.json``)
...
as a .csv. That profile is read as ``tou_params`` to be used by the ``dso_rate_making.py``.
