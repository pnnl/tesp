...
Though these agents were created under a TESP use case, the DSO+T study, the architecture being developed here based on their behavior is expected to be a generic capability that all users of TESP would be able to leverage. The abstract classes are expected to be a part of the core TESP codebase while the analysis-specific implementations (_e.g._ DSO+T) are expected to build on these abstract classes and live in their distinct codebase. There could be reference implementations in the core TESP codebase where concrete versions of the abstract classes are implemented to demonstrate the capability but these demonstrations would likely exist in a testbed and would not implement the full interactions in the DSO+T analysis. This is all to say, some portion of what is described here will end as core TESP functionality and some will only be realized in the DSO+T-specific portion of the codebase.
...
To help further define the behavior of agents under the new architecture, using the classes defined in the ["hvad_dsot_new_agent_cd.plantuml"](./hvac_dsot_new_agent_cd.plantuml), UML sequence diagrams were defined, again using PlantUML. A sequence diagram is a standard UML diagram that shows the order of operations of the various class methods, helping define how the software system is expected to operate and showing the interactions between various classes. These diagrams are an essential part for architecting and designing a system as they take the interactions stated or implied in a class diagram and given them an element of time.

### Role of "substation.py"
The original software architecture of DSO+T was implemented with the all agents being owned by the "substation.py" code. That is, "substation.py" instantiated all the agents and was the software entity that called all their methods. This choice was made due to the manner in which Python loaded external libraries; if instantiated individually, the memory footprint for the agents ballooned dramatically. This central management of the agents has been preserved in re-architecting the agent code but may be revised as revisions throughout the code base continue.

(As a side note, "substation.py" is a misnomer. Electrical substations are physical locations with equipment to convert voltage levels and implement circruit protection and other management equipment. They are typically well instrumented and serve as keep constraint and management points in an electrical distribution system. They are not, generally, the locations of the management for DERs. The name was likely chosen as it connotates this central point of management for the distibution system though the equipment being managed is not located in the substation.)

### DSO+T Interaction Between "substation.py" and Agents
To document the sequence of operations performed by the agents, then, it is necessary to evaluate the code in "substation.py". Doing this, several sequence diagrams were constructed: ["substation_all_agents_sd.plantuml"](./substation_all_agents_sd.plantuml) is the most lengthly and comprehensive documentation of the object instanation and method calls made by "substation.py" related to the agents. A simplified version that aggregates and renames functionality can be found in ["substation_all_agents_simplier_sd.plantuml"](./substation_all_agents_simpler_sd.plantuml). Finally, a version that just includes the HVAC agent and ignores the other device agents can be found in ["substation_hvac_sd.plantuml"](./substation_hvac_sd.plantuml). We'll be using the later in reviewing the original and re-architected operations sequence of the HVAC agent. As noted in the diagram itself, some simplifications have been made in describing the operation of the HVAC agent.

After some perfunctory intialization, substation loops over all the HVACs having them define initial thermostate setpoints and solve the corresponding ETP models. Following this, a the HELICS co-simulation mappings are made and the the data collection metrics are configured. substation continues by continuing to configure the data exchanges and connections between itself and the HVAC agents. The initialization of the HVAC agents continue with substation calling the HVAC agent to create a real-time bid. This results in the HVAC agent calculating the heating caused by solar radiation and the formulating the bid, returing the bid to the substation. The substation then aggregates all the bids from all the HVAC agents and forms an (estimated?) supply curve from the wholesale market to use in clearing its retail market. 

The core operation of the Substation loop as it relates to the HVAC agent begins with the formation of the numerous forecasts needed to form the day-bid vector (hourly values for up to 48 hours). These include solar heating forecasts, internal heat gain forecasts, and ZIP load forecasts. The model parameters for the day-ahead optimization model are updated. substation then forms an bid for the uncontrollable loads in the feeder (which does not include the HVACs) and then calls the HVAC agents in parallel to solve their optimization problems and form a day-ahead bid. Substation aggregates the resulting day-ahead bids formed by each HVAC agent and then calls the DSO agent to perform its day-ahead and real-time retail bidding at the appropriate simulation times. Similarly, when the appropriate simulations times are reached, Substation sends the bid information up to AMES and the wholesale market and receives back down clearing information that is used to clear the retail market which is in turn communicated to the HVAC agents. Appropriate metrics information are written out.

### New Architecture Interaction Between "substation.py" and Agents
Currently, as the full end-to-end validation of the the new agent has not taken place, no new comprehensive sequence diagram showing the interactions under the new architecture has been created. Two draft sequence diagrams have been started: ["substation_all_agents_new_sd.plantuml"](./substation_all_agents_new_sd.plantuml) and ["substation_hvac_new_sd.plantuml"](./substation_hvac_new_sd.plantuml). "substation_hvac_new_sd.plantuml" clearly shows the initialization process for the classes, clearly indicating which classes are instantiating each other while "substation_all_agents_new_sd.plantuml" provides a broader, more summarized view of the entire process. Again, these are draft diagrams and largely do not show the impacts of the new software architecture yet.
...
An script has been written that extracts the class attributes and methods and outputs them in PlantUML format. This scripting may need to be modified to do the same for abstract classes and may be extended to also capture the relationship between classes (_e.g._ sub-classes, components).

### Complete the Sequence Diagrams
The existing sequence diagrams are far from complete and do not well-represent the functionality of the existing DSO+T code or the re-architected agent code. To bring meaningful understanding of how the DSO+T agents perform and to show comperable activities under the new architecture, these diagrams need to be comprehensively updated. 

Scripting to automatically generate sequence diagrams could be possible but will be more complex than that for creating the class diagrams. Especially when considering the standard library, the call sequence the stacks used in these scripts can be VERY deep, resulting in comprehensive but unweildy diagrams that obscure important details rather than illuminate them. The alternative to hand-crafting diagrams with manual summarization as has been done thus far is error prone and may summarize details that shouldn't be if the author doesn't fully understand the operations being performed by the agent. If a script to generate these sequence diagrams is written, a means of limiting its scope (_i.e._ limiting the classes it considers) and a means of summarizing sequences of calls needs to be implemented. This may be non-trivial.
