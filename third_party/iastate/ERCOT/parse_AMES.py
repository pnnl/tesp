    ...
    for lse_rec in LSEDataPriceSensitiveDemandRaw:
       ...
       if not lse_rec_ary[0] in lse_id_list:
           ...
           temp_LSEDataPriceSensitiveDemand[str(lse_rec_ary[0])] = {"ID":lse_rec_ary[1].strip(),
                    "atBus":lse_rec_ary[2].strip(), "hour-"+lse_rec_ary[3].strip(): {"c": float(lse_rec_ary[4]),
...
def convertDataToMATPOWER(output_file, NodeData, output_bus_data, output_gen_data,output_branch_data,output_gencost_data):
    # header
...
def convertDataToPYPowerJSON(output_file, NodeData, output_bus_data, output_gen_data,output_branch_data,output_gencost_data):
    pp_case = {}
    pp_case["version"] = "2"
