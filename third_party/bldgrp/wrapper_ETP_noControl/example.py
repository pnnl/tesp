...
if __name__ == "__main__":
	...
	LO_model1 = LargeOffice('./core/co_all.csv','./core/init2.csv',fmu_location,startDay, duration, timeStep, inputSettings) 
	
	# ------temporary read in from CSVs, should be from FMU/TESP---------------------
	Q  = np.genfromtxt('./core/Q.csv', delimiter=',',max_rows=(startDay+duration)*1440)[1:,1:] #the csv used is at 1min interval, timeStep = 60
	TO = np.genfromtxt('./core/TO.csv', delimiter=',',max_rows=(startDay+duration)*1440)[1:,1] #the csv used is at 1min interval, timeStep = 60
	...
	while(model_time < LO_model1.stopTime):# stopTime = (startDay + duration) * 86400
		...
		if model_time%86400 == 0: #temporarily added for initial values, Q and TO CSV read in. 
			...
			T_cur, curOutputs = LO_model1.step(model_time, curInputs, T_prev, TOValues[curMinofDay], QValues[curMinofDay], 18)
			T_prev = T_cur
			plotting["T_zones"].append(T_prev)
