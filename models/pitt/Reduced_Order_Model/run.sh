...
(export FNCS_CONFIG_FILE=TE_substation.yaml && export FNCS_FATAL=YES && exec python -c "import tesp_support.original.substation_f as tesp;tesp.substation_loop_f('TE_agent_dict.json','TE')" &> substation.log &)
(export FNCS_CONFIG_FILE=pypower.yaml && export FNCS_FATAL=YES && export FNCS_LOG_STDOUT=yes && exec python -c "import tesp_support.original.tso_PYPOWER_f as tesp;tesp.pypower_loop_f('TE_pp.json','TE')" &> pypower.log &)
