...
def process_agents(name_root, diction_name=''):
    ...
    *controller_[name_root]_metrics.json* for the data;
    ...
    if len(diction_name) > 0:
        ...
        lp = open(name_root + '_agent_dict.json').read()
    diction = json.loads(lp)
    a_keys = list(diction['markets'].keys())
    a_keys.sort()
    c_keys = list(diction['controllers'].keys())
    c_keys.sort()
    print("\nMarket Dictionary:")
    print("ID Period Unit Init StDev")
    for key in a_keys:
        row = diction['markets'][key]
    ...
    lp_a = open("auction_" + name_root + "_metrics.json").read()
