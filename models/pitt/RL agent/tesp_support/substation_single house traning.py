    ...
    *controller_metrics_root_metrics.json* upon completion.
    ...
    while (time_granted < time_stop):
        ...
        if time_granted >= tnext_action:
            ...
            state = np.array(
                [[hvacObjs[key].air_temp, hvacObjs[key].setpoint, T_bound, T_out[step], hvacObjs[key].cleared_price]])
            # choose action based on the state

            state_obs.append(np.array(
                [[hvacObjs[key].air_temp, hvacObjs[key].setpoint, T_bound, T_out[step], hvacObjs[key].cleared_price]]))
            ...
            if step >= 1:
                ...
                if state_obs[step][0][0] <= 60 or state_obs[step][0][0] >= 100 or state_obs[step][0][2] <= 66 or \
                        state_obs[step][0][2] >= 88:
                    reward = -10
                    end = 1
                else:
                    if T_d >= 0:
