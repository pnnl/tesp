...
def inner_substation_loop(configfile, metrics_root, hour_stop=1667, flag='WithMarket'):
    ...
    *controller_metrics_root_metrics.json* upon completion.
    ...
    topicMap = {'LMP': [aucObj, 0], 'refload': [aucObj, 1]}
    ...
    agent = Agent(alpha=0.00025, beta=0.0025, input_dims=[5], tau=0.001,
                  batch_size=72, layer1_size=90, layer2_size=60, n_actions=1)
    ...
    np.random.seed(0)
    T_out = np.load('T_out.npy')  # temperature

    while time_granted < time_stop:
        nextFNCSTime = int(
            min([tnext_hvac_energy, tnext_bid, tnext_agg, tnext_clear, tnext_adjust, time_stop, tnext_action]))
        ...
        if time_granted >= tnext_action:
            ...
            for key in hvac_keys:
                ...
                state = np.array([[hvacObjs[key].air_temp, hvacObjs[key].setpoint, T_bound, T_out[step]]])
                ...
                hvac_energy_data[key].append(hvac_energy_[key])
            ...
            if step % 500 == 0 and step > 0:
                ...
                with open('load.json', 'w') as outfile:
                    json.dump(hvac_energy_data, outfile)
            #                with open('loadt.json', 'w') as outfile:
            #                    json.dump(hvac_energy_, outfile)
...
if __name__ == '__main__':
    substation_loop('C:/Users/wang690/Desktop/projects/TESP/tesp_1st/ercot/case8/Bus1_agent_dict.json', 'Bus1',
                    24)
