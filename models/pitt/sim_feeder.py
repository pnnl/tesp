...
def getV(model_name, simlistfile, V_datafile, new_Vdatafile):
    ...
    simlistkeys = ['phase_name', 'i_node', 'f_node', 'junction', 'i_branch', 'f_branch', 'out_branch_1', 'out_branch_2',
                   'out_branch_3']
    global simlist
    ...
    for n in range(len(simlist)):
        ...
        Via.append(complex(float(Va[base_name + '_node_' + simlist[n]['i_node']]),
                           float(Vaimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vib.append(complex(float(Vb[base_name + '_node_' + simlist[n]['i_node']]),
                           float(Vbimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vic.append(complex(float(Vc[base_name + '_node_' + simlist[n]['i_node']]),
                           float(Vcimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vfa.append(complex(float(Va[base_name + '_node_' + simlist[n]['f_node']]),
                           float(Vaimg[base_name + '_node_' + simlist[n]['f_node']])))
        Vfb.append(complex(float(Vb[base_name + '_node_' + simlist[n]['f_node']]),
                           float(Vbimg[base_name + '_node_' + simlist[n]['f_node']])))
        Vfc.append(complex(float(Vc[base_name + '_node_' + simlist[n]['f_node']]),
                           float(Vcimg[base_name + '_node_' + simlist[n]['f_node']])))
...
def getV_sim(model_name, simlistfile, V_datafile, new_Vdatafile):
    ...
    for n in range(len(simlist)):
        #    temp_node='R5-12-47-1_'+nodelist[n]['i_node']
        Vias.append(complex(float(Va[base_name + '_node_' + simlist[n]['i_node']]),
                            float(Vaimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vibs.append(complex(float(Vb[base_name + '_node_' + simlist[n]['i_node']]),
                            float(Vbimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vics.append(complex(float(Vc[base_name + '_node_' + simlist[n]['i_node']]),
                            float(Vcimg[base_name + '_node_' + simlist[n]['i_node']])))
        Vfas.append(complex(float(Va[base_name + '_node_' + simlist[n]['f_node']]),
                            float(Vaimg[base_name + '_node_' + simlist[n]['f_node']])))
        Vfbs.append(complex(float(Vb[base_name + '_node_' + simlist[n]['f_node']]),
                            float(Vbimg[base_name + '_node_' + simlist[n]['f_node']])))
        Vfcs.append(complex(float(Vc[base_name + '_node_' + simlist[n]['f_node']]),
                            float(Vcimg[base_name + '_node_' + simlist[n]['f_node']])))
...
def getI_agg(model_name, simlistfile, I_datafile, new_I_datafile):
    ...
    for n in range(len(simlist)):
        if simlist[n]['out_branch_1'] == '' and simlist[n]['out_branch_2'] == '' and simlist[n]['out_branch_3'] == '':
            ...
            IicJ.append(complex(float(Ic[simlist[n]['f_branch']]), float(Icimg[simlist[n]['f_branch']])))
            IoaJ.append(
                complex(float(Ia[simlist[n]['out_branch_1']]), float(Iaimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ia[simlist[n]['out_branch_2']]), float(Iaimg[simlist[n]['out_branch_2']])))
            IobJ.append(
                complex(float(Ib[simlist[n]['out_branch_1']]), float(Ibimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ib[simlist[n]['out_branch_2']]), float(Ibimg[simlist[n]['out_branch_2']])))
            IocJ.append(
                complex(float(Ic[simlist[n]['out_branch_1']]), float(Icimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ic[simlist[n]['out_branch_2']]), float(Icimg[simlist[n]['out_branch_2']])))
        elif simlist[n]['out_branch_1'] != '' and simlist[n]['out_branch_2'] != '' and simlist[n]['out_branch_3'] != '':
            IiaJ.append(complex(float(Ia[simlist[n]['f_branch']]), float(Iaimg[simlist[n]['f_branch']])))
            IibJ.append(complex(float(Ib[simlist[n]['f_branch']]), float(Ibimg[simlist[n]['f_branch']])))
            IicJ.append(complex(float(Ic[simlist[n]['f_branch']]), float(Icimg[simlist[n]['f_branch']])))
            IoaJ.append(
                complex(float(Ia[simlist[n]['out_branch_1']]), float(Iaimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ia[simlist[n]['out_branch_2']]), float(Iaimg[simlist[n]['out_branch_2']])) + complex(
                    float(Ia[simlist[n]['out_branch_3']]), float(Iaimg[simlist[n]['out_branch_3']])))
            IobJ.append(
                complex(float(Ib[simlist[n]['out_branch_1']]), float(Ibimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ib[simlist[n]['out_branch_2']]), float(Ibimg[simlist[n]['out_branch_2']])) + complex(
                    float(Ib[simlist[n]['out_branch_3']]), float(Ibimg[simlist[n]['out_branch_3']])))
            IocJ.append(
                complex(float(Ic[simlist[n]['out_branch_1']]), float(Icimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ic[simlist[n]['out_branch_2']]), float(Icimg[simlist[n]['out_branch_2']])) + complex(
                    float(Ic[simlist[n]['out_branch_3']]), float(Icimg[simlist[n]['out_branch_3']])))
        if simlist[n]['out_branch_4'] != '':
            ...
            IocJ.append(
                complex(float(Ic[simlist[n]['out_branch_1']]), float(Icimg[simlist[n]['out_branch_1']])) + complex(
                    float(Ic[simlist[n]['out_branch_2']]), float(Icimg[simlist[n]['out_branch_2']])) + complex(
                    float(Ic[simlist[n]['out_branch_3']]), float(Icimg[simlist[n]['out_branch_3']])) + complex(
                    float(Ic[simlist[n]['out_branch_4']]), float(Icimg[simlist[n]['out_branch_4']])))
...
def calculate_Z_S():
    ...
    for n in range(len(Via)):
        ...
        if simlist[n]['phase_name'] == 'ABC':  # 3phase segment
            ...
            Ii[n] = np.array([[Iia[n], Iib[n], Iic[n], 0, 0, 0], [0, Iia[n], 0, Iib[n], Iic[n], 0],
                              [0, 0, Iia[n], 0, Iib[n], Iic[n]]])
            Zv[n] = np.dot(np.dot(Ii[n].transpose().conjugate(), inv((np.dot(Ii[n], Ii[n].transpose().conjugate())))),
                           dV[n])
            Z[n] = np.array(
                [[Zv[n].item(0), Zv[n].item(1), Zv[n].item(2)], [Zv[n].item(1), Zv[n].item(3), Zv[n].item(4)],
                 [Zv[n].item(2), Zv[n].item(4), Zv[n].item(5)]])
            ...
            Zv2[n] = np.dot(
                np.dot(Ii2[n].transpose().conjugate(), inv((np.dot(Ii2[n], Ii2[n].transpose().conjugate())))), dV2[n])
...
# create Meters to attach the loads
...
def _one_test(k):
    ...
    shutil.copyfile(fname, base)
    CreateVoltdump(base, 'Voltage_Dump_' + mname + '.csv')
    CreateCurrdump(base, 'Current_Dump_' + mname + '.csv')

    cmdline = 'gridlabd -D WANT_VI_DUMP=1 ' + base
    ...
    segment_node = []
    for n in range(len(simlist)):
        segment_node = nx.shortest_path(G, source=base_name + '_node_' + simlist[n]['i_node'],
                                        target=base_name + '_node_' + simlist[n]['f_node'])
