...
def buildNS3model(topo):
    ...
    for iNode in range(len(adjMatrix)):
        for jNode in range(len(adjMatrix[iNode])):
            if adjMatrix[iNode][jNode] == 1:
                ...
                pointToPoint.SetDeviceAttribute("DataRate", ns.core.StringValue(
                    str(linkRateMatrix[iNode][jNode]) + topo.__dict__["graph"]["capacity_unit"]))
                pointToPoint.SetChannelAttribute("Delay", ns.core.StringValue(
                    str(linkDelayMatrix[iNode][jNode]) + topo.__dict__["graph"]["delay_unit"]))
    ...
    for nodeInd in range(numNodes):
        sink = ns.applications.PacketSinkHelper("ns3::UdpSocketFactory",
                                                ns.network.InetSocketAddress(ns.network.Ipv4Address.GetAny(), portNum))
        ...
        for jNode in range(numNodes):
            if (iNode != jNode) & (adjMatrix[iNode][jNode] == 1):
                ...
                OnOff = ns.applications.OnOffHelper("ns3::UdpSocketFactory",
                                                    ns.network.InetSocketAddress(ipAddr, portNum))
