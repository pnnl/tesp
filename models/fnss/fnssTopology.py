...
def buildFNSStopology():
    ...
    specificDelay = fnss.PROPAGATION_DELAY_FIBER
    print(
        Fore.YELLOW + Back.BLUE + "===== Setting delays based on geographical distance between nodes =====" + Style.RESET_ALL)
    ...
    print(
        Fore.YELLOW + Back.RED + "===== Setting the weights to links proportionally to their delay =====" + Style.RESET_ALL)
    fnss.set_weights_delays(topo)

    print(
        Fore.YELLOW + Back.BLUE + '===== Setting the buffer sizes proportionally to the product of link bandwidth and average network RTT =====' + Style.RESET_ALL)
    bufferUnit = 'bytes'
    ...
    for node in receivers:
        fnss.add_application(topo, node, 'ns3::UdpEchoClient',
                             {'StartTime': '1s', 'StopTime': '10s', 'RemoteAddress': '3-4-0a:00:00:06',
                              'RemotePort': '2000'})
