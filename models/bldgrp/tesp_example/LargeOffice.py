...
class LargeOffice(object):
	...
	def ETP_init(self):
		self.initTemp = np.genfromtxt(self.etp_init_file, delimiter=',',
									  max_rows=self.startDay - 1 + self.duration)  # mid-night reset
	...
	def step(self, current_t, weather, control_inputs, voltage):
		...
		Q_InternalLoad, P_InternalLoad, Output_InternalLoad = self.internalLoad_step(current_t,
	...
	def internalLoad_step(self, current_t, control_inputs, windSpeed):
		...
		Q_Inf = np.multiply(self.Inf,
							Sch_Inf * 1.3716 * windSpeed)  # EP count in the hights of the level. 1.3716 is for mid-level
...
class Predictor_file(object):
	def __init__(self, fileName, start_day, duration):
		...
		self.model = np.genfromtxt(fileName, delimiter=',', max_rows=(start_day - 1 + duration) * 1440 + 1)[
					 (start_day - 1) * 1440 + 1:, :]
