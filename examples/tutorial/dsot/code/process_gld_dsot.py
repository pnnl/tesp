...
def process_gld(name_root, diction_name=''):
    ...
    *house_[name_root]_metrics.json* for the data;
    ...
    discarded_hours = 24 * discarded_days # discarded hours
    ...
    fig1, ax1 = plt.subplots(2, 2, figsize=(9, 6), sharex='col')
    ...
    ax1[1, 0].plot(hrs, out_temp, label='outside air temp')
    ax1[1, 0].set_title("Cooling setpoints for all HVAC units")
    ax1[1, 0].set_ylabel("Fahrenheit")
    ...
    fig, ax = plt.subplots(2, 2, figsize=(9, 6), sharex='col')
    house_key = hse_keys.index(sample_house)
    meter_key = mtr_keys.index(sample_meter)
    if lst_i[time_key]:
        inv_key = inv_keys.index(sample_inverter)
    ...
    # if lst_i[time_key]:
    #     fig, ax_in = plt.subplots(2, 1, figsize=(10, 8), sharex='col')
...
if __name__ == "__main__":
    curr_dir = os.getcwd()
    case_dir = os.path.dirname(curr_dir)
    case_name = 'skinny_aug_1'
    # case_name = 'skinny_aug_1_pv'
    case_name = 'skinny_aug_1_ev'
    # case_name = 'skinny_aug_1_pv_ev'
    
    name_root = 'Substation_2'
    case_path = os.path.join(case_dir, case_name, name_root)
    
    figure_dir = 'Figures'
    figure_path = os.path.join(case_dir, case_name, figure_dir) 
    try: 
        os.makedirs(figure_path, exist_ok = True) 
        print("Directory '%s' created successfully" % figure_dir) 
    except OSError as error: 
        print("Directory '%s' can not be created" % figure_dir) 
  
    
    global sample_house, sample_meter
    sample_house = 'R4_25_00_1_tn_106_Upper_hse_1'
    sample_meter = 'R4_25_00_1_tn_106_Upper_mtr_1'
    sample_inverter = 'R4_25_00_1_tn_118_Low_isol_2'
    
    global discarded_days 
    discarded_days =  0
    
    
    os.chdir(case_path)
    process_gld(name_root, diction_name='')
    os.chdir(curr_dir)