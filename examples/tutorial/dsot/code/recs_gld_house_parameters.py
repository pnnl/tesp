...
def get_residential_metadata(recs_data_file,
                             dsot_metadata_file,
                             output_file,
                             sample=None,
                             bin_size_thres=100,
    ...
    wh_size_range = {'30_or_less': {'min': 25, 'max': 30}, '31-49': {'min': 31, 'max': 49},
        ...
        for hd in sample['housing_density']:
            ...
            for il in sample['income_level']:
                ...
                if og_bin_size < bin_size_thres:
                    ...
                    rgn_bin_size = len(recs.loc[((recs['DIVISION'] == rgn) &
                                                 (recs['UATYP10'] == hd) &
                                                 (recs['Income_cat'] == il))])
                    if climate_zone is None:
                        cz_bin_size = 0
                    else:
                        cz_bin_size = len(recs.loc[((recs['IECC_climate_code'] == climate_zone) &
                                                    (recs['UATYP10'] == hd) &
                                                    (recs['Income_cat'] == il))])
                    if il == 'Low':
                        il_bin_size = len(recs.loc[((recs['state_postal'] == st) &
                                                    (recs['UATYP10'] == hd) &
                    ...
                    if rgn_bin_size > bin_size_thres or rgn_bin_size == max_bin:
                        sample_df = recs.loc[((recs['DIVISION'] == rgn) &
                                              (recs['UATYP10'] == hd) &
                                              (recs['Income_cat'] == il))]
                        warnings.warn(
                            f'Bin size={og_bin_size}. Bin size less than bin size threshold of {bin_size_thres}. '
                        ...
                        sample_df = recs.loc[((recs['IECC_climate_code'] == climate_zone) &
                                              (recs['UATYP10'] == hd) &
                                              (recs['Income_cat'] == il))]
                        warnings.warn(
                            f'Bin size={og_bin_size}. Bin size less than bin size threshold of {bin_size_thres}. '
                        ...
                        if il == 'Low':
                            ...
                            warnings.warn(
                                f'Bin size={og_bin_size}. Bin size less than bin size threshold of {bin_size_thres}. '
                                f'Widening income level to generate distributions with bin size {il_bin_size}.')
                        elif il == 'Middle':
                            sample_df = recs.loc[((recs['state_postal'] == st) &
                                                  (recs['UATYP10'] == hd) &
                                                  (recs['Income_cat'].isin(['Low', 'Middle', 'Upper'])))]
                ...
                if len(sample_df) < bin_size_thres:
                    warnings.warn(
                        f'Bin size={len(sample_df)}. Bin size less than bin size threshold of {bin_size_thres}. '
                        f'No expansion methods meet bin size threshold.')
                # Get total population for final sample after bin size check/adjustments
                total = sample_df['NWEIGHT'].sum()

                # Define probability distribution for housing_type
                total_dict = {}
                # Get house type distribution for triple
                for k, h in housing_type_dict.items():
                    metadata['housing_type'][st][hd_str][il][h] = round(
                        sample_df.loc[(sample_df[house_type_str] == k), 'NWEIGHT'].sum() / total, 4)
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        total_dict[h][y] = sample_df.loc[((sample_df[house_type_str] == k) &
                                                          (sample_df[vintage_str] == p)), 'NWEIGHT'].sum()
                        if total_dict[h][y] == 0:
                            total_dict[h][y] = 1
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        ...
                        for n, s in num_stories_dict.items():
                            metadata['num_stories'][st][hd_str][il][h][y][s] = (
                                round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                     (sample_df[vintage_str] == p) &
                                                     (sample_df[n_stories_str] == n)),
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        metadata['air_conditioning'][st][hd_str][il][h][y] = (
                            round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                 (sample_df[vintage_str] == p) &
                                                 (sample_df[ac_str] == 1)),
                                  'NWEIGHT'].sum() / total_dict[h][y], 4))
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        ...
                        metadata['space_heating_type'][st][hd_str][il][h][y]['gas_heating'] = (
                            round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                 (sample_df[vintage_str] == p) &
                                                 (sample_df[sh_fuel_str].isin([1, 2, 3, 7, 99, -2]))),
                                  'NWEIGHT'].sum() / total_dict[h][y], 4))
                        metadata['space_heating_type'][st][hd_str][il][h][y]['heat_pump'] = (
                            round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                 (sample_df[vintage_str] == p) &
                                                 (sample_df[sh_equip_str].isin([4, 13]))),
                                  'NWEIGHT'].sum() / total_dict[h][y], 4))
                        metadata['space_heating_type'][st][hd_str][il][h][y]['resistance'] = (
                            round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                 (sample_df[vintage_str] == p) &
                                                 (sample_df[sh_fuel_str] == 5) &
                                                 (~sample_df[sh_equip_str].isin([4, 13]))),
                                  'NWEIGHT'].sum() / total_dict[h][y], 4))
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        ...
                        both_gas = sample_df.loc[((sample_df[house_type_str] == k) &
                                                  (sample_df[vintage_str] == p) &
                                                  (sample_df[sh_fuel_str].isin([1, 2, 3, 7, 99, -2])) &
                                                  (sample_df[wh_fuel_str].isin([1, 2, 3, 7, 8, 99, -2]))),
                                                 'NWEIGHT'].sum()
                        both_electric = sample_df.loc[((sample_df[house_type_str] == k) &
                                                       (sample_df[vintage_str] == p) &
                                                       (sample_df[sh_fuel_str] == 5) &
                                                       (sample_df[wh_fuel_str] == 5)),
                                                      'NWEIGHT'].sum()
                        metadata['water_heating_type'][st][hd_str][il][h][y] = round((both_gas + both_electric) /
                                                                                     total_dict[h][y], 4)
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        metadata['high_ceilings'][st][hd_str][il][h][y] = (
                            round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                 (sample_df[vintage_str] == p) &
                                                 (sample_df[hiceiling_str] == 1)),
                                  'NWEIGHT'].sum() / total_dict[h][y], 4))
                ...
                for k, h in housing_type_dict.items():
                    ...
                    for p, y in housing_vintage_dict.items():
                        ...
                        for o in range(1, max_num_occupants + 1):
                            metadata['num_occupants'][st][hd_str][il][h][y][o] = (
                                round(sample_df.loc[((sample_df[house_type_str] == k) &
                                                     (sample_df[vintage_str] == p) &
                                                     (sample_df[n_occ_str] == o)),
                                      'NWEIGHT'].sum() / total_dict[h][y], 4))
                ...
                #             metadata['tank_size_dist'][h][y][w]=
                #                 round(sample_df.loc[((sample_df['TYPEHUQ'] == k) &
                #                                     (sample_df['YEARMADERANGE'] ==p ) &
                #                                     (sample_df['WHEATSIZ'] ==t )),
                #                                     'NWEIGHT'].sum() / total, 4)
...
if __name__ == "__main__":
    get_residential_metadata(
        '../../../../data/feeders/RECSwIncomeLvl.csv',
        '../data/DSOT_residential_metadata.json',
        '../data/RECS_residential_metadata.json',
        {'state': ['TX'], 'housing_density': ['R', 'U', 'C'], 'income_level': ['Middle', 'Upper', 'Low']},
        bin_size_thres=40, climate_zone='4C')
