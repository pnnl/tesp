...
os.environ["TESPDIR"] = "C:\\Users\\mukh915\\PNNL_Git_Repos\\tesp"
...
import write_experiment_scripts as write
...
def prepare_case(node, mastercase, pv=None, bt=None, fl=None, ev=None):
    ...
    for dso_key, dso_val in dso_config.items():
        ...
        if sys_config['solarPPlayerFile'] == '':
            bldPrep['SolarPPlayerFile'] = ''
        else:
            bldPrep['SolarPPlayerFile'] = prefix + dso_key+'/'+dso_key+'_'+sys_config['solarPPlayerFile']
        
        if sys_config['solarQPlayerFile'] == '':
            bldPrep['SolarQPlayerFile'] = ''
        else:
        ...
        shutil.copy(os.path.join(os.path.abspath(sys_config['WeatherDataSourcePath']), dso_val['weather_file']),
                    os.path.join(os.path.abspath(caseName), weather_agent_name, 'weather.csv'))
    ...
    write.write_dsot_management_script_h(master_file="",
                                          case_path=caseName,
                                          system_config=sys_config,
                                          substation_config=dso_config,
                                          weather_config=weather_config, 
                                          HELICS_flag =with_HELICS )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 6:
        ...
        prepare_case(1, "1_system_case_config", pv=1, bt=0, fl=0, ev=1)
