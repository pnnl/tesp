        ...
        if bldg == 0:  # Single-family homes
            ...
            if np.random.uniform(0, 1) <= solar_percentage:  # some single-family houses have PV
                ...
                if case_type['pv']:
                    ...
                    print('    generator_mode SUPPLY_DRIVEN;', file=op) ## MuMonish
                    # print('    generator_mode', solar_inv_mode+';', file=op)
                    print('    four_quadrant_control_mode', solar_inv_mode + ';', file=op)
                    #### MuMonish: Inverter output based on solar panel output ####
                    print('    P_Out', '{}'.format(inv_power), ';', file=op)
                    #### MuMonish: Inverter output based on profile from player ####
                    # print('    P_Out', 'P_out_inj.value * {}'.format(pv_scaling_factor), ';', file=op)
                    if '' not in solar_Q_player:
                    ...
                    print('      rated_power', '{:.2f}'.format(inv_power) + ';', file=op)
                    print('    };', file=op)
...
def ProcessTaxonomyFeeder(outname, rootname, vll, vln, avghouse, avgcommercial):
    ...
    if os.path.isfile(fname):
        ...
        if cosim:    
            ...
            print('object csv_reader {', file=op)
            print('  name', str(weather_name) + '_csv_reader;', file=op)
            print('  filename "../weather_' + substation_name + '/weather.csv' + '";', file=op)
            print('}', file=op)
        
            print('object climate {', file=op)
            print('  name', str(weather_name) + ';', file=op)
            print('  tmyfile "../weather_' + substation_name + '/weather.csv' + '";', file=op)
            print('  interpolate QUADRATIC;', file=op)
            print('  latitude', str(latitude) + ';', file=op)
            print('  longitude', str(longitude) + ';', file=op)
            print('  // altitude', str(altitude) + ';', file=op)
            print('  tz_meridian {0:.2f};'.format(15 * time_zone_offset), file=op)
            print('  reader', str(weather_name) + '_csv_reader;', file=op)
            if metrics_interval > 0 and "meter" in metrics:
                print('  object recorder {', file=op)
                print('    property temperature;', file=op)
                print('    file ', str('"recorder_outside_temperature.csv') + '";', file=op)
                print('    interval ', str(metrics_interval) + ';', file=op)
                print('  };', file=op)
            print('}', file=op)
        ...
        ### EV metrics Collector is not working
        if case_type['ev']:
            print('  object group_recorder {', file=op)
            print('    name EV_charger_recorder_' + rootname.replace('.', '_') + ';', file=op)
            print('    group "class=evcharger_det";', file=op)
            print('    property realized_charge_rate;', file=op)
            print('    file EV_charger_output' + rootname.replace('.', '_') + '.csv;', file=op)
            print('    interval 300;', file=op)
            print('  };', file=op)
