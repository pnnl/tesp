    ...
    if bldg['type'] not in ['large_office', 'ZIPLOAD']:
        ...
        if np.random.uniform(0, 1) <= solar_percentage:
            ...
            if case_type['pv']:
                ...
                #### MuMonish: Inverter output based on output from solar panels ####
                ...
                #### MuMonish: solar panels based on area ####
                # print('      area', '{:.2f}'.format(panel_area) + ';', file=op)
                #### MuMonish: solar panels based on rated power ####
        ...
        if np.random.uniform(0, 1) <= ev_percentage:
            ...
            if case_type['ev']:  # all pvCases(HR) have ev populated
