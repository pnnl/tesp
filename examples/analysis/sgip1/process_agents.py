...
# @Date:   2021-06-27T15:10:33-07:00
# @Email:  allison.m.campbell@pnnl.gov
# @Last modified by:   camp426
# @Last modified time: 2021-10-05T17:13:34-07:00
...
def read_agent_metrics(path, nameroot, dictname = '', print_dictionary=False):
  ...
  if len (dictname) > 0:
    ...
    dict_path = os.path.join(path, f'{nameroot}_agent_dict.json')
    lp = open (dict_path).read()
  dict = json.loads(lp)
  ...
  dict_path = os.path.join(path, f'auction_{nameroot}_metrics.json')
  lp_a = open (dict_path).read()
  ...
  dict_path = os.path.join(path, f'controller_{nameroot}_metrics.json')
  lp_c = open (dict_path).read()
