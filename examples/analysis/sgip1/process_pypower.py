...
# @Last modified time: 2021-10-22T15:29:49-07:00
...
def read_pypower_metrics (path, nameroot):
  ...
  dso_dict = json.loads(lp)
  baseMVA = dso_dict['baseMVA']
  gen_keys = list(dso_dict['generators'].keys())
  gen_keys.sort()
  bus_keys = list(dso_dict['dsoBuses'].keys())
  bus_keys.sort()
  print ('\n\nFile', m_dict_path, 'has baseMVA', baseMVA)
  print('\nGenerator Dictionary:')
  print('Unit Bus Type Pnom Pmax Costs[Start Stop C2 C1 C0]')
  for key in gen_keys:
    row = dso_dict['generators'][key]
  ...
  dict = {}
  dict['hrs'] = hrs
  dict['dso_b'] = dso_dict['dsoBuses']
  dict['generators'] = dso_dict['generators']
  dict['data_b'] = data_b
