...
(exec eplus_agent_helics 2d 5m SchoolDualController eplus_SGIP1b_metrics.json 0.10 25 4 4 eplus_agentH.json &> eplus_agent1b.log &)
(exec gridlabd -D SCHED_PATH=$SCHED_PATH -D USE_HELICS -D METRICS_FILE=SGIP1b_metrics.json SGIP1b.glm &> gridlabd1b.log &)
(exec python3 -c "import tesp_support.api as tesp;tesp.substation_loop('SGIP1b_agent_dict.json', 'SGIP1b', helicsConfig='SGIP1b_HELICS_substation.json')" &> substation1b.log &)
(exec python3 -c "import tesp_support.api as tesp;tesp.tso_pypower_loop('sgip1_pp.json','SGIP1b','pypowerConfig.json')" &> pypower1b.log &)
(export WEATHER_CONFIG=SGIP1b_HELICS_Weather_Config.json && exec python3 -c "import tesp_support.api as tesp;tesp.startWeatherAgent('weather.dat')" &> weather1b.log &)
