...
class DistributionMetricsProcessor:
    ...
    def loadAllMetricsFromJSONFiles(self, case_name="", case_path=""):
        ...
        print("\n\nFile", self.casename, "has substation <", sub_keys[0], ">at Matpower bus<", matBus, ">with", xfMVA,
              "MVA transformer")
        print("\nFeeder Dictionary:")
        for key in sub_keys:
        ...
        # In order to calculate the battery revenue,
        # the auction json files are also read in and processed here
        ...
        self.hrs = np.array(times, dtype=np.float64)
        denom = 3600.0
        self.hrs /= denom
        ...
        # Inverter object is not existing in some cases
        ...
        data_a = np.empty(shape=(len(a_keys), len(times), len(lst_a[str(times[0])][a_keys[0]])), dtype=np.float64)
        ...
        for _ in a_keys:
            ...
            for t in times[0:-2]:
                ...
                data_a[j, i, :] = ary
                i = i + 1
            # Since the auction agents do not have data for the last time step of the gld times,
            # need to insert one here
            data_a[j, -1, :] = data_a[j, -2, :]
            j = j + 1
        ...
        self.house_metrics = xr.Dataset({'Temperature': (['houseID', 'time'], data_h[:, :, HSE_AIR_AVG_IDX]),
                                         ...
                                         ['houseID', 'time'], data_h[:, :, HSE_AIR_DEV_COOLING_IDX]),
                                         'Total_HVACloads': (
                                         ['houseID', 'time'], data_h[:, :, HSE_TOTAL_HVAC_LOAD_AVG_IDX]),
        ...
        time_key = str(times[0])
        if lst_i[time_key] is not None:
            self.inverter_metrics = xr.Dataset({'Real_power_avg': (['inverterID', 'time'],
                                                                   data_i[:, :, INV_P_AVG_IDX]),
                                                'Reactive_power_avg': (['inverterID', 'time'],
                                                                       data_i[:, :, INV_Q_AVG_IDX]),
                                                },
                                               coords={'inverterID': list(map(str, inv_keys)),
                                                       'time': self.hrs},  # or hrs
                                               attrs={'solar_inverter_ids': solar_id_array,
                                                      'battery_inverter_ids': battery_id_array})
    ...
    def get_subs_metrics_for_period(self, start_time, end_time):
        return self.subs_metrics.where(
            np.logical_and(self.subs_metrics.time >= start_time, self.subs_metrics.time < end_time), drop=True)
    ...
    def get_house_metrics_for_period(self, start_time, end_time):
        return self.house_metrics.where(
            np.logical_and(self.house_metrics.time >= start_time, self.house_metrics.time < end_time), drop=True)
    ...
    def get_metering_metrics_for_period(self, start_time, end_time):
        return self.meter_metrics.where(
            np.logical_and(self.meter_metrics.time >= start_time, self.meter_metrics.time < end_time), drop=True)

    # # sum of counts of voltage violation above Range A over the whole simulation period
    # print("\n\n Sum of voltage violation counts: ",mp_meter_metrics.VoltageViolationCounts_aboveRangeA.sum(), "\n")
    ...
    def get_PV_energy_output(self):
        PVOutput = self.inverter_metrics.sel_points(inverterID=self.inverter_metrics.attrs[
            'solar_inverter_ids']).Real_power_avg * self.time_interval_hours / 1000  # inverter unit is W
        print('\n\nPV energy :', PVOutput)
        return PVOutput
    ...
    def get_battery_energy_output(self):
        batteryOutput = self.inverter_metrics.sel_points(inverterID=self.inverter_metrics.attrs[
            'battery_inverter_ids']).Real_power_avg * self.time_interval_hours / 1000  # inverter unit is W
    ...
    def get_PV_revenue_dataSet(self, feederID, start_time, end_time):
        PVOutput = (self.inverter_metrics.where(
            (self.inverter_metrics.time >= start_time) &
            (self.inverter_metrics.time <= end_time), drop=True)
                    .sel_points(inverterID=self.inverter_metrics.attrs['solar_inverter_ids'])
                    .Real_power_avg.values * self.time_interval_hours / 1000)  # inverter unit is W
        auctionPrice = self.auction_metrics.Clearing_price.where(
            (self.auction_metrics.time >= start_time) &
            (self.auction_metrics.time <= end_time), drop=True).values
    ...
    def get_battery_revenue_dataSet(self, feederID, start_time, end_time):
        batteryOutput = (self.inverter_metrics.where(
            (self.inverter_metrics.time >= start_time) &
            (self.inverter_metrics.time <= end_time), drop=True)
                         .sel_points(inverterID=self.inverter_metrics.attrs['battery_inverter_ids'])
                         .Real_power_avg.values * self.time_interval_hours / 1000)
        auctionPrice = self.auction_metrics.where(
            (self.auction_metrics.time >= start_time) &
            (self.auction_metrics.time <= end_time), drop=True).Clearing_price.values
...
# 
# # filter different source types by where (inverterID in solar_dict)
...
# 
# # battery revenue ( income from generated energy minus payment paid to charged energy
# # Since currently battery is not used as controlled agent, for battery revenue is calculated based on battery kW output and market clearing price
