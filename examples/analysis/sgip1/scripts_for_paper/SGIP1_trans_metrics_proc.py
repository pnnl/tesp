...
for i in range(0, len(subdir)):
    ...
    # if(i<2):
    #     tmp.get_bus_metrics_at_bus(7).LMP_P.plot(ax=ax1, color=colors[i],label=case_name)
    tmp.get_bus_metrics_at_bus(7).PD.plot(color=colors[i], label=case_name)
    # (tmp.get_bus_metrics_at_bus(7).LMP_P*1000).plot(ax=ax1[i], color=colors[i], label=case_name)
    # tmp.get_bus_metrics_at_bus(7).VMAG.plot(ax=axes[i])
    all_bus_metrics[case_name] = tmp.get_bus_metrics()
    all_gen_metrics[case_name] = tmp.get_gen_metrics()

    all_bus_metrics_nonEventDay[case_name] = tmp.get_bus_metrics_for_period(0, 24)
    all_gen_metrics_nonEventDay[case_name] = tmp.get_gen_metrics_for_period(0, 24)

    all_bus_metrics_eventDay[case_name] = tmp.get_bus_metrics_for_period(24, 48)
    all_gen_metrics_eventDay[case_name] = tmp.get_gen_metrics_for_period(24, 48)
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_bus_metrics_nonEventDay[case].sel(busNum=7).time.values,
             all_bus_metrics_nonEventDay[case].sel(busNum=7).PD.values, color=colors[i], label=case)
    i = i + 1

ax1.legend(loc='best')
ax1.grid(True)
# ax1.set(title ='Bus LMP comparison', xlabel='Time (hours)',ylabel='LMP ($/kWh)')
...
for case in folder_names:
    plt.plot(all_bus_metrics_nonEventDay[case].sel(busNum=7).time.values,
             all_bus_metrics_nonEventDay[case].sel(busNum=7).LMP_P.values, color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_gen_metrics_nonEventDay[case].sel(busNum=4).time.values,
             all_gen_metrics_nonEventDay[case].sel(busNum=4).PGEN.values, color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    _avg = all_bus_metrics_nonEventDay[case].sel(busNum=7).LMP_P.mean().values
    _max = all_bus_metrics_nonEventDay[case].sel(busNum=7).LMP_P.max().values
    _min = all_bus_metrics_nonEventDay[case].sel(busNum=7).LMP_P.min().values

    LMP_avg_nonEvent.append(_avg)
    LMP_max_nonEvent.append(_max)
    LMP_min_nonEvent.append(_min)
    print(case, ",", _avg, ",", _max, ",", _min)
...
for case in folder_names:
    ...
    fee = (all_bus_metrics_nonEventDay[case].sel(busNum=7).LMP_P * 1000.0 * time_interval_hours *
           all_bus_metrics_nonEventDay[case].sel(busNum=7).PD).sum().values
    print(case, ",", )
    i = i + 1
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_bus_metrics_eventDay[case].sel(busNum=7).time.values,
             all_bus_metrics_eventDay[case].sel(busNum=7).PD.values, color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    plt.plot(all_bus_metrics_eventDay[case].sel(busNum=7).time.values,
             all_bus_metrics_eventDay[case].sel(busNum=7).LMP_P.values, color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_gen_metrics_eventDay[case].sel(busNum=4).time.values,
             all_gen_metrics_eventDay[case].sel(busNum=4).PGEN.values, color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_bus_metrics[case].sel(busNum=7).time.values, all_bus_metrics[case].sel(busNum=7).LMP_P.values,
...
for case in folder_names:
    # all_gen_metrics[case].sel(busNum = 4).PGEN.plot( ax = axes,color = colors[i],label=case_name)
    plt.plot(all_bus_metrics[case].sel(busNum=7).time.values, all_bus_metrics[case].sel(busNum=7).PD.values,
             color=colors[i], label=case)
    i = i + 1
...
for case in folder_names:
    _avg = all_bus_metrics_eventDay[case].sel(busNum=7).LMP_P.mean().values
    _max = all_bus_metrics_eventDay[case].sel(busNum=7).LMP_P.max().values
    _min = all_bus_metrics_eventDay[case].sel(busNum=7).LMP_P.min().values
    print(case, ",", _avg, ",", _max, ",", _min)
...
for case in folder_names:
    # plt.plot( kind = 'bar', (all_bus_metrics[case].sel(busNum = 7).LMP_P*all_bus_metrics[case].sel(busNum = 7).PD).sum().values , color = colors[i],label=case)
    print(case, ",", (all_bus_metrics_eventDay[case].sel(busNum=7).LMP_P * 1000.0 * time_interval_hours *
                      all_bus_metrics_eventDay[case].sel(busNum=7).PD).sum().values)
    i = i + 1
...
for case in folder_names:
    _avg = all_bus_metrics[case].sel(busNum=7).LMP_P.mean().values
    _max = all_bus_metrics[case].sel(busNum=7).LMP_P.max().values
    _min = all_bus_metrics[case].sel(busNum=7).LMP_P.min().values
