...
class TransmissionMetricsProcessor:
    ...
    def loadAllMetricsFromJSONFiles(self, case_name="", case_path=""):
        ...
        bus_keys.sort()
        print("\n\nFile", self.casename, "has baseMVA", baseMVA, "with GLD load scaling = none", )
        ...
        gen_cost = np.empty(shape=(len(gen_keys), len(times)), dtype=np.float64)
        gen_revenue = np.empty(shape=(len(gen_keys), len(times)), dtype=np.float64)
        gen_emission_co2 = np.empty(shape=(len(gen_keys), len(times)), dtype=np.float64)
        gen_emission_sox = np.empty(shape=(len(gen_keys), len(times)), dtype=np.float64)
        gen_emission_nox = np.empty(shape=(len(gen_keys), len(times)), dtype=np.float64)

        co2_emission_rate = np.empty(shape=(len(gen_keys)), dtype=np.float64)
        sox_emission_rate = np.empty(shape=(len(gen_keys)), dtype=np.float64)
        nox_emission_rate = np.empty(shape=(len(gen_keys)), dtype=np.float64)
    ...
    def get_bus_metrics_for_period(self, start_time, end_time):
        return self.bus_metrics.where(
            np.logical_and(self.bus_metrics.time >= start_time, self.bus_metrics.time < end_time), drop=True)
    ...
    def get_gen_metrics_for_period(self, start_time, end_time):
        return self.gen_metrics.where(
            np.logical_and(self.gen_metrics.time >= start_time, self.gen_metrics.time < end_time), drop=True)
