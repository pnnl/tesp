...
(export FNCS_CONFIG_FILE=eplus.yaml && exec energyplus -w $EPWFILE -d output -r $EPLUS_PATH/SchoolDualController_f.idf &> eplus1ex.log &)
...
(export FNCS_FATAL=YES && export FNCS_LOG_STDOUT=yes && export WEATHER_CONFIG=SGIP1b_weather_f.json && exec python3 -c "import tesp_support.weatherAgent as tesp;tesp.startWeatherAgent('weather.dat')" &> weather1ex.log &)
