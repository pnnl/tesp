...
(exec eplus_agent_helics 2d 5m SchoolDualController eplus_SGIP1e_metrics.json 0.10 25 4 4 eplus_agentH.json &> eplus_agent1e.log &)
(exec gridlabd -D SCHED_PATH=$SCHED_PATH -D USE_HELICS -D METRICS_FILE=SGIP1e_metrics.json SGIP1e.glm &> gridlabd1e.log &)
(exec python3 -c "import tesp_support.api as tesp;tesp.substation_loop('SGIP1e_agent_dict.json', 'SGIP1e', helicsConfig='SGIP1b_HELICS_substation.json')" &> substation1e.log &)
(exec python3 -c "import tesp_support.api as tesp;tesp.tso_pypower_loop('sgip1_pp.json','SGIP1e','pypowerConfig.json')" &> pypower1e.log &)
(export WEATHER_CONFIG=SGIP1b_HELICS_Weather_Config.json && exec python3 -c "import tesp_support.api as tesp;tesp.startWeatherAgent('weather.dat')" &> weather1e.log &)
