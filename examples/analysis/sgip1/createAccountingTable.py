...
# @Last modified time: 2021-11-30T16:24:14-08:00
...
'''
edit Fig 33:
https://tesp.readthedocs.io/en/sgip1_doc/examples/SGIP1_Example.html#fig-sa-createaccountingtable
files needed:
from process_gld:
    {nameroot}_glm_dict.json
    substation_{nameroot}_metrics.json
    house_{nameroot}_metrics.json
    billing_meter_{nameroot}_metrics.json
    inverter_{nameroot}_metrics.json
    capacitor_{nameroot}_metrics.json
    regulator_{nameroot}_metrics.json
from process_pypower:
    {nameroot}_m_dict.json
    bus_{nameroot}_metrics.json
    gen_{nameroot}_metrics.json
from process_agents:
    {nameroot}_agent_dict.json
    auction_{nameroot}_metrics.json
    controller_{nameroot}_metrics.json
'''
...
def calculate_metrics(dir,name):
    ...
    # Wholesale electricity purchase cost for test feeder ($/day)
    LMP_idx = pypower_dict['idx_b']['LMP_P_IDX']
    time_diff = (pypower_dict['hrs'][1]-pypower_dict['hrs'][0])
    n_daily_obs = 24/time_diff
    n_days = len(pypower_dict['hrs'][1:])/n_daily_obs
    wholesale_purchase_price = pypower_dict['data_b'][:,1:,LMP_idx]
    ...
    # https://ieeexplore.ieee.org/document/8360969
    ...
    PD_idx = pypower_dict['idx_g']['PGEN_IDX']
...
# write paragraph interpreting the results
# plot results? bar graph? instead of table
# three tables
pd.concat([accounting_table,accounting_table_delta],axis=1)
