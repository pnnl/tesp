...
(exec python3 -c "import tesp_support.substation as tesp;tesp.substation_loop('SGIP1e_agent_dict.json', 'SGIP1ex',flag='NoMarket', helicsConfig='SGIP1b_substation.json')" &> substation1ex.log &)
(exec python3 -c "import tesp_support.tso_PYPOWER as tesp;tesp.tso_pypower_loop('sgip1_pp.json','SGIP1ex','pypowerConfig.json')" &> pypower1ex.log &)
(export WEATHER_CONFIG=SGIP1b_weather.json && exec python3 -c "import tesp_support.weather_agent as tesp;tesp.startWeatherAgent('weather.dat')" &> weather1ex.log &)
