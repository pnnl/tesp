...
(export FNCS_CONFIG_FILE=eplus.yaml && exec energyplus -w $EPWFILE -d output -r $EPLUS_PATH/SchoolDualController_f.idf &> eplus1e.log &)
...
(export FNCS_CONFIG_FILE=pypower.yaml && export FNCS_FATAL=YES && export FNCS_LOG_STDOUT=yes && exec python3 -c "import tesp_support.tso_PYPOWER_f as tesp;tesp.tso_pypower_loop_f('sgip1_pp.json','SGIP1e')" &> pypower1e.log &)
(export FNCS_FATAL=YES && export FNCS_LOG_STDOUT=yes && export WEATHER_CONFIG=SGIP1b_weather_f.json && exec python3 -c "import tesp_support.weatherAgent as tesp;tesp.startWeatherAgent('weather.dat')" &> weather1e.log &)
