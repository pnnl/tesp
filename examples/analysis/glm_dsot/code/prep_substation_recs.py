...
# file: prep_substation_recs_f.py
""" Sets up the FNCS or HELICS and agent configurations for DSOT ercot case 8 example
...
def process_glm(gldfileroot, substationfileroot, weatherfileroot, feedercnt):
    ...
    substation_name = case_config['substation']
    Q_dso_key = case_config['DSO']
    Q_forecast = case_config['Q_bid_forecast_correction']
    state = case_config['state']
    dso_type = case_config['DSO_type']
    ...
    if case_config['thermostat_schedule_version'] == 2:
        thermostat_schedule_config = case_config['ThermostatScheduleV2']
    else:
        thermostat_schedule_config = case_config['thermostat_schedule']
    ...
    trans_cust_per = case_config['transactive_house_percentage']
    ...
    site_agent = {}
    slider_ranges = case_config['slider_ranges']
    ...
    for key, val in gd['houses'].items():
        try:
            if val['cooling'] == 'ELECTRIC' or val['fuel_type'] == 'electric':
                ...
                if case_config['agent_debug_mode']['ON'] and not case_config['agent_debug_mode']['flex_hvac']:
                    cooling_participating = False
                    heating_participating = False
    ...
    for key, val in gd['houses'].items():
        ...
        try:
            ...
            if case_config['agent_debug_mode']['ON'] and not case_config['agent_debug_mode']['flex_wh']:
                participating = False
    ...
    for key, val in gd['ev'].items():
        ...
        # will this device participate in market
        participating = site_agent[meter_name]['participating']
        # ev won't participate at all if case_type is not ev
        if not case_config['case_type']['ev']:
            participating = False
    ...
    for key, val in gd['inverters'].items():
        if val['resource'] == 'solar':
            ...
            pv_scaling_fac = val['rated_W'] / case_config['rooftop_pv_rating_MW']
    ...
    for market in market_config:
        ...
        if market == 'DSO':
            ...
            markets[market_name] = {
                ...
                'metrics_full_detail': case_config['metrics_full_detail'],
                'quadratic': case_config['quadratic']
            }
    ...
    if case_config["messenger"] == 'FNCS':
        ...
        if feedercnt == 1:
            ...
            print('broker: tcp://localhost:' + str(case_config['port']), file=yp)
