...
case_config = tesp_support.dsot.DSOT_plots.load_json(config_path, 'system_case_config.json')
...
DSOmetadata = tesp_support.dsot.DSOT_plots.load_json(metadata_path, metadata_file)

for dso_num in dso_range:
    # load DSO metadata
    file_name = 'Substation_' + str(dso_num) + '_glm_dict.json'
    metadata = tesp_support.dsot.DSOT_plots.load_json(case_path + agent_prefix + str(dso_num), file_name)
    ...
    commdata = tesp_support.dsot.DSOT_plots.load_json(metadata_path, 'DSOT_commercial_metadata.json')
    commbldglist = []
    ...
    agent_metadata = tesp_support.dsot.DSOT_plots.load_json(case_path + agent_prefix + str(dso_num), agent_file_name)

    metadata = tesp_support.dsot.DSOT_plots.customer_meta_data(metadata, agent_metadata, True)

    # Need to preprocess agent retail data prior to calculating customer energy meter data.  This function saves an h5.
    if read_meters:
        tesp_support.dsot.DSOT_plots.tic()
        for day_num in day_range:
            retail_data_df, retail_index_df = tesp_support.dsot.DSOT_plots.load_retail_data(case_path, agent_prefix,
                                                                                            str(dso_num), str(day_num), 'retail_site')
        print('Retail agent data processing complete: DSO ' + str(dso_num) + ', Month ' + month_name)
        tesp_support.dsot.DSOT_plots.toc()
...
# --------------- CALCULATE AMENITY SCORES  ------------------------------
    ...
    if calc_amenity:
        tesp_support.dsot.DSOT_plots.tic()
        amenity_df = tesp_support.dsot.DSOT_plots.amenity_loss(metadata, case_path, '\Substation_', str(dso_num),
                                                               day_range)
...
if pop_stats:
        bill = False
        rci_df = tesp_support.dsot.DSOT_plots.RCI_analysis(dso_range, case_path, case_path, metadata_path, dso_meta_file, bill)


# ------------  PLOT TSO GENERATION AND AMES DATA
if gen_plots:

    tesp_support.dsot.DSOT_plots.generation_load_profiles(case_path, config_path, metadata_path, case_path, day_range, True)

    tesp_support.dsot.DSOT_plots.generation_load_profiles(case_path, config_path, metadata_path, case_path, day_range, False)

    GenAMES_df = tesp_support.dsot.DSOT_plots.generation_statistics(case_path, config_path, metadata_path, day_range, False)

    GenPYPower_df = tesp_support.dsot.DSOT_plots.generation_statistics(case_path, config_path, metadata_path, day_range, True)

if load_stack_plots:
    tesp_support.dsot.DSOT_plots.tic()
    tesp_support.dsot.DSOT_plots.der_load_stack(dso_range, day_range, case_path, agent_prefix, GLD_prefix,
