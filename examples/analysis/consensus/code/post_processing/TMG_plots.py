...
def customer_meta_data(glm_meta, agent_meta, tariff_path, transactive=False):
    ...
    # TODO: this seems to be counting sqft and zones twice
    for house in agent_meta['hvacs']:
        meter = glm_meta['houses'][house]['billingmeter_id']
        glm_meta['billingmeters'][meter]['num_zones'] += 1
        glm_meta['billingmeters'][meter]['sqft'] += glm_meta['houses'][house]['sqft']
    ...
    if transactive:
        ...
        for meter in agent_meta['site_agent']:
            # meter = glm_meta['houses'][house]['billingmeter_id']
            # house = glm_meta['billingmeters'][meter]['children'][0]
            glm_meta['billingmeters'][meter]['cust_participating'] = agent_meta['site_agent'][meter]['participating']
            glm_meta['billingmeters'][meter]['slider_setting'] = agent_meta['site_agent'][meter]['slider_setting']
...
def load_gen_data(dir_path, gen_name, day_range):
    ...
    # TODO: Need a more generic way to find first calendar day of simulation:
    dso_num = '1'
    ...
    if gen_name in ['da_q', 'da_lmp', 'da_line']:
        ...
        index = pd.MultiIndex.from_product([[start_time], dso_list], names=['Time', 'Variable'])
        ...
        for day in day_range:
            for i in range(0, 24):
                for dso in dso_list:
                    ...
                    hour = column_key[gen_name] + str(i+1)
                    gen_data_df.loc[(time, dso), column_key[gen_name][:-1]] = data_df.loc[(clear_time, dso), hour]
    else:
...
def load_ames_data(dir_path, day_range):
    ...
    # Load AMES data
    os.chdir(dir_path)
    data_df = pd.read_csv('opf.csv', index_col='seconds')
    # TODO: Need to make this generic in the future.
    dso_num = '3'
    case_config = load_json(dir_path, 'case_config_' + dso_num + '.json')
    sim_start = datetime.strptime(case_config['SimulationConfig']['StartTime'], '%Y-%m-%d %H:%M:%S')
...
def load_ercot_data(ercot_path, dir_path, day_range):
    ...
    Returns:
        ...
        """
    os.chdir(ercot_path)
    data_df = pd.read_csv('2016_ERCOT_5min_Load_Data_Revised.csv', index_col='Seconds ')
...
def load_ercot_fuel_mix(ercot_path, dir_path, day_range):
    ...
    Returns:
        data_df : dataframe of AMES data
        """
    # TODO: need to fix this hardwired number.
    dso_num = '1'
    case_config = load_json(dir_path, 'case_config_' + dso_num + '.json')
    sim_start = datetime.strptime(case_config['SimulationConfig']['StartTime'], '%Y-%m-%d %H:%M:%S')

    ercot_fuel_df = pd.read_excel(ercot_path + '\\ERCOTGenByFuel2016.xlsx', sheet_name='AllMonths')
...
def load_da_retail_price(dir_path, folder_prefix, dso_num, day_num):
    ...
    for i in range(len(retail_da_data_df)):
        time.append(bid_time + timedelta(hours=int(retail_da_data_df.iloc[i,5]+1)))
...
def load_agent_data(dir_path, folder_prefix, dso_num, day_num, agent_name):
    ...
    if agent_name == 'energy':
        ...
        if agent_name in ['dso_tso', 'water_heater_agent']:
            agent_bid_df = None
...
def RCI_analysis(dso_range, case, data_path, metadata_path, dso_metadata_file, energybill=False):
    ...
    for dso in dso_range:
        ...
        file_name = 'Microgrid_' + str(dso) + '_glm_dict.json'
...
def DSO_loadprofiles(dso_range, day_range, case, dso_metadata_file, ercot_path, plot_weather=True):
    """  For a specified dso range and case this function will analyze the ratios of Res, Comm, and Industrial.
    Args:
        dso_range (range): the DSO range that the data should be analyzed
        case (str): folder extension of case of interest
    ...
    ercot_df = load_ercot_data(ercot_path, case, day_range)
...
def bldg_load_stack(dso_range, day_range, case, agent_prefix, gld_prefix, dso_metadata_file, ercot_path):
    ...
    # Load ERCOT load profile data
    ercot_df = load_ercot_data(ercot_path, case, day_range)
    # TODO: Need a more generic way to find day of first run
    dso_num = '1'
    case_config = load_json(case, 'case_config_' + dso_num + '.json')
    sim_start = datetime.strptime(case_config['SimulationConfig']['StartTime'], '%Y-%m-%d %H:%M:%S')
    ...
    # TODO: Replace this with read in of industrial tape.
    temp['Industrial'] = np.ones(len(temp))*10300
    ...
    plt.plot(temp.index, temp['Substation']+temp['Industrial'], label='Total Substation Load', color='black', linestyle=':')
    bus_cols = [col for col in ercot_df.columns if 'Bus' in col]
    plt.plot(ercot_df.index, ercot_df[bus_cols].sum(axis=1), label='ERCOT Load', color='red', linestyle='dashdot')

    plt.legend(loc='upper left', prop={'size': 14})
    plt.xlabel('Time', size=20)
    plt.ylabel('Load (MW)', size=20)
    ax = plt.gca()
...
def der_load_stack(dso_range, day_range, case, agent_prefix, gld_prefix, dso_metadata_file, ercot_path):
    """  For a specified dso and day range this function will load in the required data, plot the stacked DER loads
    ...
    case_config = load_json(case, 'case_config_' + dso_num + '.json')
    sim_start = datetime.strptime(case_config['SimulationConfig']['StartTime'], '%Y-%m-%d %H:%M:%S')
    if case_config['SimulationConfig']['caseType']['batteryCase'] == 1:
        battery_case = True
    else:
        battery_case = False
    if case_config['SimulationConfig']['caseType']['flexLoadCase'] == 1:
        flexload_case = True
    else:
        flexload_case = False
...
# TODO: Add transmission losses (e.g. total generation line).
    variables = ['Substation', 'Industrial Loads', 'Plug Loads',
     'HVAC Loads', 'WH Loads', 'Battery']
    ...
    indust_df = load_indust_data(ercot_path, day_range)
    ...
    for dso in dso_range:
        ...
        for day in day_range:
            ...
            # der_loads_df.loc[(slice(substation_df.index[0], substation_df.index[-1]), 'dso' + str(dso)), 'Substation Losses'] = \
            #     substation_df['real_power_losses_avg'].values * scale_target / 1e6

            # Plot Building Stacked Chart with ERCOT and Substation loads for reference
            # plt.figure(figsize=(20, 10))
            # plt.stackplot(temp.index, y, labels=temp.columns.tolist())
            # plt.plot(temp.index, substation_df['real_power_avg'], label='Substation Load', color='black',
            #          linestyle=':')
            ...
            # plt.xlabel('Time', size=12)
            # plt.ylabel('Load (kW)', size=12)
            # plt.title('DSO load profile by building type (DSO ' + str(dso) + '; Day ' + temp.index[0].strftime(
            #     '%m-%d') + ')')
            # plot_filename = datetime.now().strftime(
            #     '%Y%m%d') + 'BuildingLoadStackGraph_DSO_' + str(dso) + '_' + temp.index[0].strftime(
            #     '%m-%d') + '.png'
            # file_path_fig = os.path.join(case, 'plots', plot_filename)
            # plt.savefig(file_path_fig, bbox_inches='tight')
            #
...
def der_stack_plot(day_range, ercot_path, case, comp = None):
    """  For a specified dso and day range this function will load in the required data, plot the stacked DER loads
    and save the plot to file.
    Args:
        dso (str): the DSO that the data should be plotted for (e.g. '1')
    ...
    start_time = sim_start + timedelta(days=day_range[0] - 1)
    stop_time = start_time + timedelta(days=day_range[-1] - 1) - timedelta(minutes=5)
    ...
    y = []
    for col in temp.columns:
        if col not in ['Substation', 'Battery', 'Substation Losses']:
            y.append(temp[col].values.tolist())
    ...
    pal = ['grey'] + ['darkseagreen'] + ['wheat'] + ['cornflowerblue']
    plt.stackplot(temp.index, y, labels=label_list, colors=pal)
...
def daily_load_plots(dso, system, subsystem, variable, day, case, comp, agent_prefix, gld_prefix):
    ...
    if variable == 'air_temperature_avg':
        coolsetpoint_df = get_day_df(dso, system, subsystem, 'air_temperature_deviation_cooling', day, case, agent_prefix, gld_prefix)
        heatsetpoint_df = get_day_df(dso, system, subsystem, 'air_temperature_deviation_heating', day, case,
        ...
        plt.plot(coolsetpoint_df, label='air_temperature_deviation_cooling')
        plt.plot(heatsetpoint_df, label='air_temperature_deviation_heating')
        if plot_weather:
            color = 'tab:red'
    ...
    plt.legend()
    plt.title(system + ' ' + subsystem + ': ' + variable + ' vs. Time (DSO ' + dso_num + '; Day '
              + date.strftime("%m-%d") + ')')
    plt.xlabel('Time')
    plt.ylabel(variable)
    plot_filename = datetime.now().strftime('%Y%m%d') + '_daily_load_profile_DSO ' + dso_num + '_day_' \
...
def dso_market_plot(dso_range, day, case, dso_metadata_file, ercot_dir, config_file):
    ...
    ercot_df = load_ercot_data(ercot_dir, case, [int(day)])
    ames_rt_df = load_ames_data(case, range(int(day),int(day)+1))
    indust_df = load_indust_data(ercot_dir, range(int(day),int(day)+1))

    with open(config_file) as json_file:
        FNCS = json.load(json_file)['FNCS']
    ...
    for dso in dso_range:
        # TODO: deprecate once scaling factor is in every metadata file.
        scale_target = DSOmetadata['DSO_' + str(dso)]['number_of_customers'] \
                       * DSOmetadata['DSO_' + str(dso)]['RCI customer count mix']['residential'] \
                       / DSOmetadata['DSO_' + str(dso)]['number_of_gld_homes']
    ...
    plt.plot(ames_da_gen_df.groupby(level=0)['Pgen'].sum(), label='DA Generation', marker='.')
    plt.legend()
    ...
    # TODO: Add DSO agent DA cleared price (prior to retail agent)
    fig, axes = plt.subplots(nrows=2, ncols=4, figsize=(20, 10))
...
def dso_forecast_stats(dso_range, day_range, case, dso_metadata_file, ercot_dir, config_file):
    ...
    # ames_rt_df = load_ames_data(case, range(int(day),int(day)+1))
    # indust_df = load_indust_data(ercot_dir, day_range)
    ...
    for day in day_range:
        ...
        ercot_df = load_ercot_data(ercot_dir, case, range(int(day), int(day) + 1))
        indust_df = load_indust_data(ercot_dir, range(int(day), int(day) + 1))
        indust_df = indust_df.set_index(ercot_df.index)
        for dso in dso_range:
            # TODO: deprecate once scaling factor is in every metadata file.
        ...
        if day == day_range[0]:
            load = substation_df
            rt_forecast = rt_q_df
            da_forecast = da_retail_q_df
        else:
            load = pd.concat([load, substation_df])
            rt_forecast = pd.concat([rt_forecast, rt_q_df])
            da_forecast = pd.concat([da_forecast, da_retail_q_df])
        # substation_df = substation_df.set_index(ercot_df.index)
    ...
    # Subplots of quantity for each DSO in dso_range
    fig, axes = plt.subplots(nrows=2, ncols=4, figsize=(20, 10))
    fig.suptitle('Comparisons of DSO Realtime Relative Forecast Errors: '+ date_start.strftime("%m-%d") +' through ' + date_end.strftime("%m-%d"))
    for ax, dso in zip(axes.flatten(), dso_range):
        ax.hist(rt_error_df['DSO ' + str(dso)], label='DSO Market RT Q', bins=20)
        if stats:
            mean = np.format_float_positional(rt_error_df['DSO '+str(dso)].mean(), precision=3, unique=False, fractional=False, trim='k')
            std = np.format_float_positional(rt_error_df['DSO '+str(dso)].std(), precision=3, unique=False, fractional=False, trim='k')
            rms = np.format_float_positional((rt_error_df['DSO '+str(dso)]** 2).mean() ** .5, precision=3, unique=False, fractional=False, trim='k')
            ax.text(0.2, 0.95, "Mean = " + mean , size=10, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
            ax.text(0.2, 0.9, "Stdev = " + std, size=10, horizontalalignment='center', verticalalignment='center',
                    transform=ax.transAxes)
            ax.text(0.2, 0.85, "RMS = " + rms, size=10, horizontalalignment='center', verticalalignment='center',
                    transform=ax.transAxes)
        ax.set_title(label='DSO ' + str(dso), pad=-9, )
        ax.set_xlabel('Relative Error')
    plot_filename = datetime.now().strftime('%Y%m%d') + 'DSO_realtime_forecast_error'  \
                    + date_start.strftime("%m-%d") + '.png'
    file_path_fig = os.path.join(case, 'plots', plot_filename)
    fig.savefig(file_path_fig, bbox_inches='tight')

    fig, axes = plt.subplots(nrows=2, ncols=4, figsize=(20, 10))
    fig.suptitle('Comparisons of DSO Day-ahead Relative Forecast Errors: '+ date_start.strftime("%m-%d") +' through ' + date_end.strftime("%m-%d"))
    for ax, dso in zip(axes.flatten(), dso_range):
        ax.hist(da_error_df['DSO ' + str(dso)], label='DSO Market RT Q', bins=10)
        if stats:
            mean = np.format_float_positional(da_error_df['DSO '+str(dso)].mean(), precision=3, unique=False, fractional=False, trim='k')
            std = np.format_float_positional(da_error_df['DSO '+str(dso)].std(), precision=3, unique=False, fractional=False, trim='k')
            rms = np.format_float_positional((da_error_df['DSO '+str(dso)]** 2).mean() ** .5, precision=3, unique=False, fractional=False, trim='k')
            ax.text(0.2, 0.95, "Mean = " + mean , size=10, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
            ax.text(0.2, 0.9, "Stdev = " + std, size=10, horizontalalignment='center', verticalalignment='center',
                    transform=ax.transAxes)
            ax.text(0.2, 0.85, "RMS = " + rms, size=10, horizontalalignment='center', verticalalignment='center',
        ...
        ax.set_title(label='DSO ' + str(dso), pad=-9, )
        ax.set_xlabel('Relative Error')
    plot_filename = datetime.now().strftime('%Y%m%d') + 'DSO_day_ahead_forecast_error'  \
                    + date_start.strftime("%m-%d") + '.png'
    file_path_fig = os.path.join(case, 'plots', plot_filename)
    fig.savefig(file_path_fig, bbox_inches='tight')
...
def generation_load_profiles(dir_path, config_dir, ercot_path, data_path, day_range, use_ercot_fuel_mix_data = False):
    ...
    ercot_df = load_ercot_data(ercot_path, dir_path, day_range)
    ...
    if use_ercot_fuel_mix_data:
        fuel_df = load_ercot_fuel_mix(ercot_path, dir_path, day_range)
        ...
        # TODO: Need to update system case config name.
        file_name = 'system_case_config.json'
        config_data = load_json(config_dir, file_name)
    ...
    plt.figure(figsize=(20, 10))
    plt.stackplot(ercot_sum_df.index, y, colors=colors, labels=fuel_list)
    plt.plot(ercot_sum_df.index, ercot_sum_df['Net Load'], label='ERCOT Load', color='black', linestyle=':')
    if not use_ercot_fuel_mix_data:
        plt.plot(ercot_sum_df.index, ercot_sum_df['AMES Load'], label='AMES Load', color='red', linestyle=':')
...
def generation_statistics(dir_path, config_dir, data_path, day_range, use_gen_data = True):
    """  For a specified day range this function plots the stacked profiles of all generators (by fuel type) along with
    load plots.
    Args:
        dir_path (str): path locating the AMES data file
    ...
    variables = ['Fuel', 'Gen Type', 'Capacity (MW)','Min Capacity (MW)', 'Capacity Factor (-)', 'Generation (MW-hrs)', 'Online Hours',
                 'Starts', 'Online hours per start', 'Fuel cost', 'Startup costs', 'Total revenue',
                 'Total production cost', 'Effective LMP', 'Ramp Limit (MW/min)', 'Max Ramp up (MW/min)',
    ...
    for fuel in fuel_list:
        ...
        for gen in gen_list:
            ...
            if generator_df.loc['Starts', gen] != 0:
                ...
                generator_df.loc['Online hours per start', gen] = generator_df.loc['Online Hours', gen]
            generator_df.loc['Fuel cost', gen] = (len(data_df[gen]) * gen_key[gen][4] + data_df[gen] * gen_key[gen][5]
                                                  + data_df[gen] * data_df[gen] * gen_key[gen][6]).sum()/12
            generator_df.loc['Startup costs', gen] = generator_df.loc['Starts', gen] * gen_key[gen][3]
            #  TODO: Revise revenue once fully defined.
            generator_df.loc['Total revenue', gen] = (data_df[gen] * ames_df[' LMP'+str(gen_key[gen][9])]).sum() / 12
            generator_df.loc['Total production cost', gen] = generator_df.loc['Fuel cost', gen] + generator_df.loc['Startup costs', gen]
            if generator_df.loc['Generation (MW-hrs)', gen] != 0:
                generator_df.loc['Effective LMP', gen] = generator_df.loc['Total production cost', gen] /generator_df.loc['Generation (MW-hrs)', gen]
            else:
...
#     # ercot_df = load_ercot_data(ercot_path, base_case, day_range)
#     # ercot_sum = ercot_df.loc[:, ercot_df.columns[ercot_df.columns.str.contains('Bus')]].sum(
#     #     axis=1)
...
def metadata_dist_plots(system, sys_class, variable, dso_range, case, agent_prefix):
...
def amenity_loss(gld_metadata, dir_path, folder_prefix, dso_num, day_range):
    ...
    for day in day_range:
        ...
        house_meta_df, house_df = load_system_data(dir_path, '\\Substation_', dso_num, str(day), 'house')
        house_df = house_df.set_index(['time', 'name'])
        dead_band = 1
        # Start calculating
        # house_df['cool_excursion'] = house_df['air_temperature_avg'] \
        #                              - (house_df['air_temperature_deviation_cooling'] + dead_band)
        # house_df['heat_excursion'] = (house_df['air_temperature_deviation_heating'] - dead_band) \
        ...
        for each in gld_metadata['houses']:
            ...
            temp_df = house_df.xs(each, level=1)[['air_temperature_avg', 'waterheater_temp_avg',
                                                  'air_temperature_deviation_cooling', 'air_temperature_deviation_heating',
            ...
            if schedules['cool_weekday'] is not None:
                if date.weekday() < 5:
                    temp_df['air_temperature_deviation_cooling'] = schedules['cool_weekday']
                    temp_df['air_temperature_deviation_heating'] = schedules['heat_weekday']
                else:
                    temp_df['air_temperature_deviation_cooling'] = schedules['cool_weekend']
                    temp_df['air_temperature_deviation_heating'] = schedules['heat_weekend']
            if schedules['wh_Tdesired'] is not None:
                temp_df['waterheater_setpoint_avg'] = [schedules['wh_Tdesired'] for i in range(len(temp_df))]

            temp_df['cool_excursion'] = temp_df['air_temperature_avg'] \
                                         - (temp_df['air_temperature_deviation_cooling'] + dead_band)
            temp_df['heat_excursion'] = (temp_df['air_temperature_deviation_heating'] - dead_band) \
                                         - temp_df['air_temperature_avg']
...
def customer_comparative_analysis(case_data, comp_data, case_path, comp_path, dso_num, month='sum', slice=None):
    ...
    Returns:
        saves plot to file.
        """
    base_metadata = load_json(case_path + '/Microgrid_' + dso_num, 'Microgrid_'+dso_num+'_glm_dict.json')
    metadata = load_json(comp_path + '/Microgrid_' + dso_num, 'Microgrid_'+dso_num+'_glm_dict.json')
    ...
    plt.figure()
    plt.scatter(participating[('metadata', 'slider_setting')], participating[(month, 'HVAC_deg-hrs')])
...
def get_day_df(dso, system, subsystem, variable, day, case, agent_prefix, gld_prefix):
    ...
    elif system in ['house', 'billing_meter', 'inverter']:
...
if __name__ == '__main__':
    ...
    day_num = '5'  # Needs to be non-zero integer
    ...
    data_path = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_v1/TMG_helics_3_agent/'
    metadata_path = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_data/'
    case_config_file = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_v1/system_case_config.json_TM'
    dso_meta_file = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_data/8-node-metadata-revised-lean.json'
    ercot_path = 'home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_data/'
    base_case = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_v1/TMG_helics_3_agent_base/'
    trans_case = '/home/mukh915/HELICS_plus/Transactive_MG/helics-uc-4.1-transactive-microgrids/examples/microgrid_v1/TMG_helics_3_agent/'
    ...
    customer_analysis = True   # plot change in customer metrics between two cases.
    ...
    'DSO_2': ['R5_12_47_2_load_1_bldg_297_zone_1', 'R5_12_47_2_load_2_bldg_314_zone_all', 'R5_12_47_2_tn_11_hse_1'],
    ...
    'Microgrid_1':['Houses_A_hse_1','Houses_B_hse_2','Houses_C_hse_3']}

    file_name = 'Microgrid_' + dso_num + '_glm_dict.json'
    metadata = load_json(base_case + agent_prefix + dso_num, file_name)

    DSO_meters = []
    for house in DSO_Houses['Microgrid_' + dso_num]:
    ...
    if LoadExData:
        ...
        fuel_mix = load_ercot_fuel_mix(ercot_path, data_path, day_range)
        gen_data_df = load_gen_data(base_case, 'gen', day_range)
    ...
    if DictUpdate:
        for dso in dso_range:
            ...
            GLM_metadata = customer_meta_data(GLD_metadata, agent_metadata, ercot_path, Transactive)
    ...
    if DailyProfilePlots:
        ...
        #     # ['house', 'mean', 'air_temperature_deviation_cooling', base_case, None],
        ...
        # params = [
        #     ['house', DSO_Houses['DSO_' + dso_num][0], 'air_temperature_avg', trans_case, None],
        #     ['house', DSO_Houses['DSO_' + dso_num][1], 'air_temperature_avg', trans_case, None],
        #     ['house', DSO_Houses['DSO_' + dso_num][2], 'air_temperature_avg', trans_case, None],
        #     ['house', DSO_Houses['DSO_' + dso_num][3], 'air_temperature_avg', trans_case, None]
        # ]
    ...
    if gen_plots:
        ...
        generation_load_profiles(base_case, config_path, ercot_path, data_path, day_range, True)

        generation_load_profiles(base_case, config_path, ercot_path, data_path, day_range, False)

        GenAMES_df = generation_statistics(base_case, config_path, data_path, day_range, False)

        GenPYPower_df = generation_statistics(base_case, config_path, data_path, day_range, True)
...
# ------------
    if transmission_plots:
        ...
        LMP_col = [col for col in ames_df.columns if 'LMP' in col]
        for LMP in LMP_col:
            axes[2].plot(ames_df[LMP], label=LMP.replace(' LMP',''),
                            # label=[col for col in test_data_df.columns if 'nodenorm' in col], marker='o',
                            linestyle='-', alpha=0.8)
        ...
        plot_filename = datetime.now().strftime(
            '%Y%m%d') + '_Transmission_master_plot.png'
        file_path_fig = os.path.join(data_path, 'plots', plot_filename)
        fig.savefig(file_path_fig, bbox_inches='tight')
    ...
    if dso_plots:
        ...
        #dso_forecast_stats(dso_range, day_range, base_case, dso_meta_file, ercot_path, case_config_file)

        der_load_stack(dso_range, day_range, base_case, agent_prefix, GLD_prefix, dso_meta_file, ercot_path)

        der_stack_plot(day_range, ercot_path, base_case)

        for day in day_range:
            dso_market_plot(dso_range, str(day), base_case, dso_meta_file, ercot_path, case_config_file)

        # bldg_load_stack(dso_range, day_range, base_case, agent_prefix, GLD_prefix, dso_meta_file, ercot_path)

        # DSO_loadprofiles(dso_range, day_range, base_case, dso_meta_file, ercot_path)
