...
if __name__ == "__main__":
    ...
    iday = 13
    Tday = 2
    da_convergence_start = 24*5 # DA interaction to start looking for convergence
...
#### Homes
    if Homes:
        ...
        V_file = 'Substation_metrics_house'
...
####### HVAC
        if HVAC:
            ...
            mask = (AVG_power.index >= '{}-{}-2016'.format(imonth,iday)) & (AVG_power.index <= '{}-{}-2016'.format(imonth,iday+Tday))
            ...
            for i in range(len(data_individual)):
                ...
                if data_key[i] not in hvac_agent.keys():
                    ...
                    participating_hvac.update({data_key[i]:data_individual[i][meta_S[V_analis]['index']].resample('60min').mean().values})
            ...
            plt.show()
            # import pickle
            # with open('GLD_power_basecase.pkl', 'wb') as f:
            #     # ppc, ppopt_regular = pickle.load(f)
            #     pickle.dump([GLD_power], f)
            # import sys
            # sys.exit()
            # V_analis = 'hvac_load_avg'
            # AVG_power = data_s[0][meta_S[V_analis]['index']]#plots min
            ...
            if HVAC_comparisons:
                ...
                meta_S, start_time, data_s, data_individual,data_keys = get_metrics_full_multiple_KEY(V_file, pre_file, pos_file)
                ...
                # meta_S, start_time, data_s, data_individual,data_keys = get_metrics_full_multiple_KEY(V_file, pre_file, pos_file)
                # V_analis = 'hvac_load_avg'  # variable being analyzed
                # #inv_ind_power = data_individual[10][meta_S[V_analis]['index']]
                # hvac_power = data_s[3][meta_S[V_analis]['index']]
                ...
                # orig_power = sub_power + hvac_power
                # to_kW = 1000
                #
                # participating_hvac = []
                # GLD_power = {}
                ...
                #     AVG_power = data_individual[i][meta_S[V_analis]['index']]
                #     GLD_power.update({data_key[i]: AVG_power})
                #     if hvac_agent[data_key[i]][property]:
                #         participating_hvac.append(data_individual[i][meta_S[V_analis]['index']])
                ...
                meta_da, start_time, data_da, da_bid_hvac, data_keys_da = get_metrics_full_multiple_KEY_Mdays_H(V_file, pre_file,
                                                                                                  '_metrics' + pos_file)
                ...
                if False:
                    ...
                    actual = participating_hvac_df[data_keys_da[i]]#rt_bid_hvac[i][meta_rt['inverter_p_setpoint']['index']]
    ...
    if Retail_site:
        ...
        for i in range(50):#len(Order[meta_I_ver['meters']['index']][0])):
            ...
            if True:#(Order[meta_I_ver['meters']['index']][0][i]).replace("mtr","hse") == "R5_12_47_2_tn_2_hse_2":
                ...
                power = (GLD_power[(Order[meta_I_ver['meters']['index']][0][i]).replace("mtr","hse")]).resample('60min').mean()
...
#### new HVAC
    if HVAC_agent and 1:
        ...
        V_file = 'hvac_agent_Substation_300'
        #meta_S, start_time, data_s, data_individual,data_key = get_metrics_full_multiple_KEY(V_file,pre_file,pos_file)
        ...
        for i in range(10):#len(data_individual)):
            ...
            for ivar in range(len(V_analis)-1):
                ...
                ax1.plot((AVG_power.resample('5min').mean()/to_kW).index,(AVG_power.resample('5min').mean()/to_kW).values,marker='x',label=V_analis[ivar]);ax1.set_ylabel('(kW)');plt.xlabel('time (hours)');plt.grid(True);plt.title('home '+str(data_key[i])+' - day {:.0f}'.format(ihour));
            ...
            ax2.plot((AVG_power/to_kW).index,(AVG_power/to_kW).values,'-', color='k',label=V_analis[-1]);ax2.set_ylabel('price ($/kW)');plt.xlabel('time (hours)');plt.grid(True)
            # ax2.legend()
            plt.xlim([datetime.date(2016, imonth, iday), datetime.date(2016, imonth, iday + Tday)])
            figno += 1
            plt.savefig(pre_file_out + "figure{}.png".format(figno))
    ...
    if HVAC_agent and 1:
        ...
        for i in range(10):#len(data_individual)):
            ...
            for ivar in range(len(V_analis)-1):
                AVG_power = data_individual[i][meta_S[V_analis[ivar]]['index']]
                ax1.plot((AVG_power.resample('60min').mean()/to_kW).index,(AVG_power.resample('60min').mean()/to_kW).values,marker='x',label=V_analis[ivar]);ax1.set_ylabel('(kW)');plt.xlabel('time (hours)');plt.grid(True);plt.title('home '+str(data_key[i])+' - day {:.0f}'.format(ihour));
            ax1.plot((GLD_power[data_key[i]].resample('60min').mean()/to_kW).index,(GLD_power[data_key[i]].resample('60min').mean()/to_kW).values,label = 'GLD power')
            ax1.legend()
            AVG_power = data_individual[i][meta_S[V_analis[-1]]['index']]
            ax2.plot((AVG_power/to_kW).index,(AVG_power/to_kW).values,'-', color='k',label=V_analis[-1]);ax2.set_ylabel('price ($/kW)');plt.xlabel('time (hours)');plt.grid(True)
    ...
    if HVAC_agent and 0:
        ...
        V_analis = ['GLD_air_temperature','outdoor_temperature','thermostat_setpoint']#,'cooling_basepoint','cleared_price']
        to_kW = 1
        fig, ax1 = plt.subplots()
        ax2 = ax1.twinx()
    ...
    if HVAC_agent and 0:
        pre_file = pre_file_out + 'DSO_2/'
        V_file = 'hvac_agent_Substation_2_300'
        meta_S, start_time, data_s, data_individual, data_key = get_metrics_full_multiple_KEY_Mdays(V_file,pre_file,'_metrics'+pos_file)
        V_analis = ['GLD_air_temperature','outdoor_temperature','cooling_basepoint','cooling_setpoint','agent_room_temperature','cleared_price']#,'thermostat_setpoint']
        ...
        for i in range(len(data_individual)):
            ...
            mask = (price.index >= '{}-{}-2016'.format(imonth,iday)) & (price.index <= '{}-{}-2016'.format(imonth,iday+Tday))
            price = price.loc[mask]
            power = GLD_power[data_key[i]]
            mask = (power.index >= '{}-{}-2016'.format(imonth,iday)) & (power.index <= '{}-{}-2016'.format(imonth,iday+Tday))
            power = power.loc[mask]
            ...
            for ivar in range(len(V_analis)-1):
                ...
                ln = ax1.plot(AVG_power.index,AVG_power.values,marker='x',label=V_analis[ivar]);plt.xlabel('time (hours)');plt.grid(True);plt.title('home '+str(data_key[i]));#ax1.set_ylabel('(F)');
                lns+=ln#.append(ln)
            #ax1.legend()

            AVG_power = data_individual[i][meta_S[V_analis[-1]]['index']]
            ln = ax2.plot(AVG_power.index,AVG_power.values, '-', color='k', label=V_analis[-1]);ax2.set_ylabel('price ($/kW)');plt.xlabel('time (hours)');plt.grid(True)
            lns+=ln#.append(ln)
            #ax2.legend()
...
#### new waterheater agent 
    if Water_agent and 1:
         ...
         V_file = 'water_heater_agent_Substation_2_3600_'
         meta_S, start_time, data_s, data_individual = get_metrics_full_multiple_KEY_Mdays_H(V_file,pre_file,'_metrics'+pos_file)
...
#         pre_file = pre_file_out + 'dso_1/'
#         V_file = 'water_heater_agent_Substation_2_300_'
#         meta_S, start_time, data_s, data_individual = get_metrics_full_multiple_KEY_Mdays(V_file,pre_file,'_metrics'+pos_file)
#         V_analis = 'upper_tank_setpoint'
#         AVG_power = data_s[2][meta_S[V_analis]['index']]
#         to_kW = 1
#         plt.plot((AVG_power.resample('60min').mean()/to_kW).values,marker='x');plt.ylabel('Upper tank setpoint (F)');plt.xlabel('time (hours)');plt.grid(True);plt.show()
#         V_analis = 'lower_tank_setpoint'
#         AVG_power = data_s[1][meta_S[V_analis]['index']]
#         plt.plot((AVG_power.resample('60min').mean()/to_kW).values,marker='x');plt.ylabel('Lower tank setpoint (F)');plt.xlabel('time (hours)');plt.grid(True);plt.show()
#         V_analis = 'upper_tank_setpoint'
#         for i in range(len(data_individual)):
#             AVG_power = data_individual[i][meta_S[V_analis]['index']]
#             plt.plot((AVG_power.resample('60min').mean()/to_kW).values,marker='x');plt.ylabel('UPPER_SETPOINT(kW)');plt.xlabel('time (hours)');plt.grid(True);plt.title('home: '+str(i));plt.show()
# 
#             
# =============================================================================
