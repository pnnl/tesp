...
if __name__ == "__main__":
    ...
    if DSO:
        ...
        if DSO_RT_curve:
            ...
            plt.plot(supply_q[day_of_sim][hour_of_day],
                     supply_p[day_of_sim][hour_of_day],
                     label='wholesale node supply curve')
            plt.plot((ret_q[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1]),
                     (ret_p[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1]),
                     label='RT Aggregated demand curve at retail level')
            plt.plot(dso_q[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1],
                     dso_p[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1],
            ...
            plt.plot((ret_seller_q[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1]),
                     (ret_seller_p[(day_of_sim + 1) * (hour_of_day + 1) * 12 - 1]),
                     label='RT Substation supply curve')
            plt.ylabel('Retail price ($/kWh)')
            plt.xlabel('Quantity (kW)')
            plt.grid(True)
            plt.legend()
    ...
    if Retail:
        ...
        figno += 1
        plt.savefig(pre_file_out + f"figure{figno}.png")
        plt.show()
    ...
    if Homes:
        ...
        meta_S, start_time, data_s, data_individual, data_key = get_metrics_full_multiple_KEY(V_file, pre_file,
                                                                                              pos_file)
        # print (data_key)
        #### Water
        if Water:
        ...
        if HVAC:
            ...
            mask = (AVG_power.index >= f'{imonth}-{iday}-2016') & (
                        AVG_power.index <= f'{imonth}-{iday + Tday}-2016')
            ...
            file = open(pre_file_out + f'DSO_{dso_num}/' + agent_dict, 'r')
            text = file.read()
            file.close()
            ...
            for i in range(len(data_individual)):
                ...
                #     continue
                if data_key[i] not in hvac_agent:
                    continue
            ...
            if HVAC_comparisons:
                ...
                V_file = f'hvac_agent_Substation_{dso_num}_3600'
                ...
                meta_rt, start_time, data_rt, rt_bid_hvac, data_keys = get_metrics_full_multiple_KEY_Mdays(V_file,
                                                                                                           pre_file,
                                                                                                           '_metrics' + pos_file)
                to_kW = 1
                ...
                if False:
                    ...
                    for i in range(len(da_bid_hvac)):
                        if data_keys_da[i] == "R5_12_47_2_tn_133_hse_1":
                            ...
                            ax1[0].grid(True)
                            ax1[0].set_title(
                                f"Optimal bid and actual kW by hvac agent with slider setting {0.71}")  # ax1.set_ylabel('(F)');
                            ax1[0].legend()
                        ...
                        if data_keys_da[i] == "R5_12_47_2_tn_4_hse_3":  # "R5_12_47_2_tn_3_hse_5":
                            ...
                            ax1[1].grid(True)
                            ax1[1].set_title(
                                f"Optimal bid and actual kW by battery agent with slider setting {0.1225}")  # 0.0295
                            ax1[1].legend()
                    plt.xlabel('time (hours)')
                    plt.show()
        ...
        for i in range(50):  # len(Order[meta_I_ver['meters']['index']][0])):
            ...
            if True:  # (Order[meta_I_ver['meters']['index']][0][i]).replace("mtr","hse") == "R5_12_47_2_tn_2_hse_2":
                ...
                power = (GLD_power[(Order[meta_I_ver['meters']['index']][0][i]).replace("mtr", "hse")]).resample(
                    '60min').mean()
                mask = (power.index >= f'{imonth}-{iday}-2016') & (
                        power.index <= f'{imonth}-{iday + Tday}-2016')
    ...
    if HVAC_agent and 1:
        ...
        meta_S, start_time, data_s, data_individual, data_key = get_metrics_full_multiple_KEY_Mdays(V_file, pre_file,
                                                                                                    '_metrics' + pos_file)
        # meta_I_ver, start_time, Order = get_data_multiple_days(V_file, days, pre_file, pos_file)
        V_analis = ['agent_room_temperature', 'outdoor_temperature', 'cooling_setpoint', 'cooling_basepoint',
                    'cleared_price']
        ...
        for i in range(10):  # len(data_individual)):
            ...
            for ivar in range(len(V_analis) - 1):
                ...
                plt.xlabel('time (hours)')
                plt.grid(True)
                plt.title('home ' + str(data_key[i]) + f' - day {ihour:.0f}')
    ...
    if HVAC_agent and False:
        ...
        print(GLD_power)
        for i in range(len(data_individual)):
            if data_key[i] not in houses:  # not (data_key[i] == 'R5_12_47_2_tn_3_hse_5' or data_key[i] == 'R5_12_47_2_tn_133_hse_1' or data_key[i] == 'R5_12_47_2_load_10_bldg_82_zone_all'):
                continue
            price = data_individual[i][meta_S[V_analis[-1]]['index']]
            mask = (price.index >= f'{imonth}-{iday}-2016') & (
                        price.index <= f'{imonth}-{iday + Tday}-2016')
    ...
    if Water_agent and 1:
        ...
        meta_S, start_time, data_s, data_individual = get_metrics_full_multiple_KEY_Mdays_H(V_file, pre_file,
                                                                                            '_metrics' + pos_file)

        # this code portion plots quantity with sliding window of an hour for an individual home
        home_num = 45

        index = [382]  # index that needs to be collected
        ...
        for i in range(len(index)):
            data_plot.append(
                data_individual[home_num][index].sort_index().values[i + (24 * 3)][i])  # plots for last two days
        plt.plot(data_plot, marker='x')
        plt.ylabel('Quantity bid')
        ...
        plt.plot(data_plot, marker='x')
        plt.ylabel('Quantity bid for all houses')
        plt.xlabel('time index')
        plt.grid(True)
        plt.show()
