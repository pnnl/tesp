...
if __name__ == '__main__':
    ...
    base_case = os.getcwd() + '/TMG_IEEE-123_base_flex/'
    ...
    day_num = '3'  # Needs to be non-zero integer
    ...
    Data_DSO_base = dso_DSO_price_qunatity_rt(case_eval, "DSO", day_num)
    
    if gridlabd_plot:
        fig, ax = plt.subplots(1,1,figsize=(16,7), dpi= 160)
        ax.plot(gld_date, substation_load, label='Substation')
        ...
        for MG in range(3):
          MG_num = str(MG+1) 
          ax.plot(gld_date, net_meter_load[str(agent_prefix)+MG_num], label=str(agent_prefix)+MG_num )
        ...
        # fig, ax = plt.subplots(1,1,figsize=(16,7), dpi= 160)
        ax.plot(gld_date, meter_loads['n57'], label='n57')
        ax.plot(gld_date, meter_loads['n67'], label='n67')
        ax.plot(gld_date, meter_loads['n97'], label='n97')
        ax.plot(gld_date, meter_loads['n135'], label='n135')
    ...
    ax.plot(Data_DSO_base.index, Data_DSO_base['cleared_quantity_rt_DG_1'], label='DG1')
    ax.plot(Data_DSO_base.index, Data_DSO_base['cleared_quantity_rt_DG_2'], label='DG2')
    ax.plot(Data_DSO_base.index, Data_DSO_base['cleared_quantity_rt_DG_3'], label='DG3')
    ax.plot(Data_DSO_base.index, Data_DSO_base['cleared_quantity_rt_DG_4'], label='DG4')
    ax.legend(loc='upper left')
    ax.set_xlabel('Time', size=12)
    ax.set_ylabel('Entity\'s Net Supply (kW)', size=12)
    plt.grid(True)
    #plt.show()
    fig.tight_layout()
    ...
    for MG in range(No_MGs):
        ...
        if Transactive_market_plot and gridlabd_plot:
            ...
            if plot_sample_bid:
                ...
                plt.title(f'Bids at {date_start} - DA at 0th hour and RT ')
                ax.legend(loc='upper left')
                plt.grid(True)
            ...
            fig.tight_layout()
            fig.savefig(case_name + 'Microgrid_'+ MG_num + '.png', dpi=fig.dpi)
