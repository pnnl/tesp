...
import consensus_microgrid as consensus
...
#    r = input("Ready")
    ...
    while time_granted < simulation_duration:
        ...
        if with_market:
            ...
            next_fncs_time = int(min([tnext_retail_bid_rt,  tnext_retail_bid_da, tnext_dso_bid_da, tnext_dso_bid_rt,  tnext_dso_clear_rt,
        ...
        for i in range(0, subkeys_count):
            ...
            if topic in topic_map:
                ...
                if (topic != 'gld_load') and (topic != 'Market_status'): ## As we have modified gld_load to be MG_load ## Market_status is for temporary fix
        ...
        if time_granted >= tnext_retail_bid_rt:
            ...
            if time_granted < (retail_period_rt + retail_period_da * 1): # first real-time clearing has not been observed
                retail_cleared_quantity_diff_applied = 0.0
            else:
            ...
            log.info("Hour 0 day-ahead diff applied                 --> " + str(retail_day_ahead_diff_observed/1000.0) + "MW")
            log.info("Real-time diff applied                        --> " + str(retail_cleared_quantity_diff_observed/1000.0) + "MW")
...
#            print(zip_loads)
        ...
        if time_granted >= tnext_dso_bid_rt:
            ...
            dso_market_obj, time_granted = consensus.Consenus_dist_RT(dso_market_obj, fed, time_granted, time_to_complete_market_RT)
        ...
        if time_granted >= tnext_dso_bid_da:
            ...
            dso_market_obj, time_granted = consensus.Consenus_dist_DA(dso_market_obj, 24, fed, time_granted, time_to_complete_market_DA)
        ...
        if time_granted >= tnext_retail_clear_rt:
            ...
            log.info('current real-time gld load scaled + industrial load -> ' + str((gld_load_scaled_mean)/1e3) + ' MW')

            retail_cleared_quantity_RT_unadjusted = load_base_unadjusted
            log.info('current real-time quantity unadjusted -> ' + str(retail_cleared_quantity_RT_unadjusted/1e3) + ' MW')

            retail_cleared_quantity_diff_observed = retail_cleared_quantity_RT - gld_load_scaled_mean
            log.info('current real-time cleared quantity difference from actual GLD load -> ' + str(retail_cleared_quantity_diff_observed/1e3) + ' MW')
        ...
        if time_granted >= tnext_retail_clear_da:
            ...
            forecast_obj.retail_price_forecast = deepcopy(retail_market_obj.cleared_price_DA)  ## Adding actual forecast prices
        ...
        if time_granted >= tnext_retail_adjust_rt and with_market:
            ...
            for key, obj in water_heater_agent_objs.items():
                if obj.participating and obj.bid_accepted(11, current_time):
                    ...
                    pub_lts = h.helicsFederateGetPublication(fed, str(fed_name + '/' + water_heater_name + '/lower_tank_setpoint'))
                    status = h.helicsPublicationPublishDouble(pub_lts, obj.Setpoint_bottom)
                    pub_uts = h.helicsFederateGetPublication(fed, str(fed_name + '/' + water_heater_name + '/upper_tank_setpoint'))
