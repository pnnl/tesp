...
def define_comm_bldg(bldg_metadata, dso_type, num_bldgs):
    ...
    """
    bldgs = {}
    bldg_types = normalize_dict_prob(bldg_metadata['general']['building_type'][dso_type])
    i = 0
    while i < num_bldgs:
        bldg_type = rand_bin_select(bldg_types, np.random.uniform(0, 1))
        area = normalize_dict_prob(bldg_metadata['building_model_specifics'][bldg_type]['total_area'])
...
def define_comm_loads(bldg_type, bldg_size, dso_type, climate, bldg_metadata):
    ...
    if bldg_type not in ['ZIPLOAD']:
        ...
        wall_area = bldg['ceiling_height'] * \
                    2*math.sqrt(bldg['floor_area']/bldg['no_of_stories']/bldg['aspect_ratio'])*(bldg['aspect_ratio']+1)
        ...
        hours_bin = rand_bin_select(data['occupancy']['weekly_hours_distribution'], np.random.uniform(0, 1))
        if bldg_type in ['lodging', 'healthcare_inpatient']:
            hours_bin = '168'
        if hours_bin == '168':
...
def create_comm_zones(bldg, comm_loads, key, op, batt_metadata, storage_percentage, caseType, metrics, metrics_interval, mode=None):
    ...
    if bldg['type'] not in ['ZIPLOAD']:
        ...
        if np.random.uniform (0, 1) <= storage_percentage:
            ...
            batt_capacity = num_batt * tesp_support.feederGenerator_dsot_v1.get_dist(batt_metadata['capacity(kWh)']['mean'],
                                     batt_metadata['capacity(kWh)']['deviation_range_per'])*1000
            max_charge_rt = tesp_support.feederGenerator_dsot_v1.get_dist(batt_metadata['rated_charging_power(kW)']['mean'],
            ...
            charging_loss = tesp_support.feederGenerator_dsot_v1.get_dist(batt_metadata['rated_charging_loss(per)']['mean'],
...
def write_one_commercial_zone(bldg, op, metrics, metrics_interval, mode=None):
    ...
    print ('  window_exterior_transmission_coefficient {:.2f};'.format(bldg['window_exterior_transmission_coefficient']), file=op)
