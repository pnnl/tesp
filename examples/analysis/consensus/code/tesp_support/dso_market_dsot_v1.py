...
class DSOMarketDSOT:
    ...
    def update_wholesale_node_curve(self):
        ...
        for day in range(7):
            ...
            for hour in range(24):
                ...
                self.curve_ws_node[day][hour].prices = np.array([self.curve_a[day][hour] * quantity * quantity + self.curve_b[day][hour] * quantity + self.curve_c[day][hour]
    ...
    def set_Pwclear_RT (self, hour_of_day, day_of_week):
        ...
        Args:
            ...
            day_of_week (int): current day of the week   
        """
        self.Pwclear_RT, self.trial_cleared_quantity_RT, self.trial_clear_type_RT = self.trial_wholesale_clearing(self.curve_ws_node[day_of_week][hour_of_day], self.curve_DSO_RT)
    ...
    def set_Pwclear_DA (self, hour_of_day, day_of_week):
        ...
        for idx in range(self.windowLength):
            ...
            self.Pwclear_DA[idx], self.trial_cleared_quantity_DA[idx], self.trial_clear_type_DA[idx] = self.trial_wholesale_clearing(self.curve_ws_node[day][hour], self.curve_DSO_DA[idx])
                
    def trial_wholesale_clearing(self, curve_ws_node, curve_DSO):
    ...
    def substation_supply_curve_RT(self, retail_obj):
        ...
        if self.transformer_degradation == True:
            ...
            id_original_Q_max = int(Q_max_retail / (self.trial_cleared_quantity_RT / self.num_samples))
            price_range_original_Q_max = np.array([Prclear_RT] * id_original_Q_max)
            range_congestion = self.num_samples - id_original_Q_max
            congestion_quantity = self.trial_cleared_quantity_RT - Q_max_retail
            ...
            supply_curve_RT.quantities = np.linspace(0, Q_max_retail, self.num_samples)
