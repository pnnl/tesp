...
# This method is now done by commbldgenerator.py
...
def write_houses(basenode, op, vnom):
    ...
    for i in range(nhouse):
        ...
        if forERCOT == False:
            ...
            print ('  voltage_1 ' + vstart + ';', file=op)
            print ('  voltage_2 ' + vstart + ';', file=op)
            if metrics_interval > 0 and "line" in metrics:
                print ('  object metrics_collector {', file=op)
                print ('    interval', str(metrics_interval) + ';', file=op)
                print ('  };', file=op)
            print ('}', file=op)
...
#                print('    tank_setpoint', '{:.1f}'.format(tank_set) + ';', file=op)
        ...
        if bConsiderStorage:
            if np.random.uniform (0, 1) <= storage_percentage:
                ...
                print ('    inverter_efficiency', '{:.2f}'.format(inv_efficiency) + ';', file=op)
                print ('    power_factor 1.0;', file=op)
...
def ProcessTaxonomyFeeder(outname, rootname, vll, vln, avghouse,
                          avgcommercial):
    ...
    if os.path.isfile(fname):
        ...
        if solar_percentage > 0.0:
            print(
                '// default IEEE 1547-2018 for Category B; modes are CONSTANT_PF, VOLT_VAR, VOLT_WATT',
                file=op)
        ...
        for bldg in comm_bldgs_pop:
            ...
            bldg_definition = tesp_support.commbldgenerator.define_comm_loads(comm_bldgs_pop[bldg][0], comm_bldgs_pop[bldg][1], dso_type, ashrae_zone, comm_bldg_metadata)
            tesp_support.commbldgenerator.create_comm_zones(bldg_definition, comm_loads, bldg, op, batt_metadata, storage_percentage, caseType, metrics, metrics_interval, None)
...
def populate_feeder(config=None):
    ...
    global starttime, endtime, timestep, metrics, metrics_interval, metrics_interim, metrics_type, electric_cooling_percentage
