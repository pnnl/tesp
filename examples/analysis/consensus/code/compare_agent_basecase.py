...
for ii in range(len(cases)):
    ...
    for i in range(len(data_individual)):  # [0, 200]:  #
        ...
        if False:
            ...
            # plt.plot((AVG_power2.resample('60min').mean()).index,
            # (AVG_power2.resample('60min').mean()).values, marker='x',label=labels[ii])
...
for ihouse in houses.keys():
    ...
    for ii in range(len(cases)):
        plt.plot(plot_data[cases[ii]][ihouse][0], plot_data[cases[ii]][ihouse][1], marker='x', label=labels[ii])
        print(labels[ii])
