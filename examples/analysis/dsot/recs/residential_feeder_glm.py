...
from tesp_support.api.time_helpers import (
    add_hhmm_secs,
    get_dist,
    get_duration,
    get_hhmm_from_secs,
...
def getDsoIncomeLevelTable():
    ...
    dsoIncomePct = {key: income_mat[key] for key in income_level}
...
def checkResidentialBuildingTable():
    ...
    for tbl in range(len(dsoThermalPct)):
        ...
        print(rgnName[tbl], 'dsoThermalPct sums to', f'{total:.4f}')
...
def write_houses(basenode, op, vnom):
    ...
    for i in range(nhouse):
        ...
        print('  mass_solar_gain_fraction', f'{mass_solar_gain_frac}' + ';', file=op)
        print('  mass_internal_gain_fraction', f'{mass_int_gain_frac}' + ';', file=op)
        print('  aspect_ratio', f'{aspect_ratio:.2f}' + ';', file=op)
        print('  exterior_wall_fraction', f'{ewf:.2f}' + ';', file=op)
        print('  exterior_floor_fraction', f'{eff:.2f}' + ';', file=op)
        print('  exterior_ceiling_fraction', f'{ecf:.2f}' + ';', file=op)
        print('  window_exterior_transmission_coefficient', f'{wetc:.2f}' + ';', file=op)
        print('  window_wall_ratio', f'{wwr:.2f}' + ';', file=op)
        ...
        if house_fuel_type == 'gas' and np.random.uniform(0, 1) <= properties['sh_gas']['electric']:
            wh_fuel_type = 'electric'
        if house_fuel_type == 'electric' and np.random.uniform(0, 1) <= properties['sh_electric']['electric']:
            wh_fuel_type = 'electric'
        ...
        # P(SF|income)
        p_sf_g_inc = res_bldg_metadata['housing_type'][state][res_dso_type][income]['single_family_detached'] + \
                     res_bldg_metadata['housing_type'][state][res_dso_type][income]['single_family_attached']
        # P(income)
    ...
    if os.path.isfile(fname):
        ...
        for o in model[t]:
            if o not in seg_loads:
                print("WARNING: %s not in the seg loads", o)
                continue
        ...
        print(solar_count, 'pv totaling', f'{solar_kw:.1f}', 'kw with', battery_count, 'batteries and',
...
def populate_feeder(configfile=None, config=None, taxconfig=None):
    ...
    global metrics, metrics_type, metrics_interval, metrics_interim
    ...
    # Should be a list of income levels for the DSO being tested
    income_level = config['SimulationConfig']['income_level']
