...
def main(path, list_dates, num_hours, outputfilename, depletionassumption, randomsoc):
    ...
    day_to_idx_map = {}
    for idx_day, day in enumerate(num_days):
        day_to_idx_map[day] = idx_day
                ...
                for idx_row, each_unique_loc in enumerate(location):
                    ...
                    idx_location = [idx for idx, val in enumerate(list(location_all)) if
                                    val == each_unique_loc and t >= reach_home_time[
                                        idx]]  # finding EVs in corresponding location
    ...
    # re adjust the days so the day 0 refers to the first date from gridlabd simulation ...that way post processing
    # scripts will synch up well when adding building loads and ev loads
    ...
    for day_key, day_value in loads_at_locations.items():
        ...
        output_df["day"] = day_to_idx_map[day_key]
...
if __name__ == '__main__':
    ...
    path = r"vehicle_master_Large_Year_2042.xlsx"
    ...
    all_load_days, per_day_profile, loads_at_locations, main_output_df = main(path, list_of_days_to_simulate, num_hours, outputfilename, depletionassumption, randomsoc)
