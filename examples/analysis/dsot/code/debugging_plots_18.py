...
if __name__ == "__main__":
    pre_file_out = 'lean_8_bt/'  #
    pre_file_out_base = 'lean_8_bt/'
    pos_file ='.json'
    
    days = 5
    da_convergence_start = 24*2 # DA interaction to start looking for convergence
    ...
    hour_of_day = 1  # anywhere from 0 to 23
...
#### Inverters
    if Inverters:
        ...
        for key, value in batt_agent.items():
            ...
            dev.append((abs(optimal_pt[0:216] - actual_p)).mean())
        plt.scatter(prop[:len(dev)], dev, label='deviation from optimal bid')
...
#### EVs
    if EV_agent:
        #base run results
        gld_pre_file_base =  pre_file_out_base + 'Substation_{}/'.format(dso_num)
        V_file_base = 'Substation_{}_metrics_substation'.format(dso_num)
        meta_S_base, start_time, data_base, data_individual_base = get_metrics_full_multiple_KEY(V_file_base, gld_pre_file_base, pos_file)
        # # aggregated house load without ev
        V_analis = 'real_power_avg'
        sub_power_base = data_base[3][meta_S_base[V_analis]['index']]  # plots aggregated
        ...
        # V2G
        # gld_pre_file = 'pv_ev_case_trans_7days_30per_V2G/' + 'Substation_{}/'.format(dso_num)
        # V_file = 'Substation_{}_metrics_substation'.format(dso_num)
        # meta_S, start_time, data_s, data_individual = get_metrics_full_multiple_KEY(V_file, gld_pre_file, pos_file)
        # # # aggregated house load without ev
        # V_analis = 'real_power_avg'
        # sub_power_v2g = data_s[3][meta_S[V_analis]['index']]  # plots aggregated

        V_file = 'Substation_{}_metrics_house'.format(dso_num)
        # V_file = 'your_metricshouse'
        ...
        total_power = data_house[3][meta_S['total_load_avg']['index']]  # sum


        V_file = 'Substation_{}_metrics_evchargerdet'.format(dso_num)
        gld_ev_meta_S, start_time, data_s, ev_data_individual = get_metrics_full_multiple_KEY(V_file, pre_file, pos_file)
        V_analis = 'charge_rate_avg'  # variable being analyzed
        ev_power = data_s[3][gld_ev_meta_S[V_analis]['index']]
        ev_power_avg = data_s[1][gld_ev_meta_S[V_analis]['index']]
        ev_power_min = data_s[0][gld_ev_meta_S[V_analis]['index']]
        ev_power_max = data_s[2][gld_ev_meta_S[V_analis]['index']]
        ev_soc_avg = data_s[1][gld_ev_meta_S['battery_SOC_avg']['index']]
        orig_power = sub_power - ev_power
        to_kW = 1000
        ...
        # # flatten the list to 1-D
        # da_cleared_q = [j for sub in data_s_sum for j in sub]
        # # remove the last 24 entries as they are forecast for non-existing day
        # da_cleared_q = da_cleared_q[:-24]
        # get actual inverter power
        ev_power_hr = ev_power.resample('60min').mean().values
        # fig2, ax2 = plt.subplots()
        # # ax2 = ax1.twinx()
        # ax2.plot((da_cleared_q), label='cleared DA bid');
        # ax2.plot(ev_power_hr[24 + 1:] / 1000, label='actual consumption');  # remove 1st day
        # ax2.set_ylabel('kW')
        # ax2.grid(True);
        # ax2.set_title(
        #     "Cleared DA bid and actual kW by battery agent ");  # ax1.set_ylabel('(F)');
        ...
        # Plotting agent parameters and optimal bids
        V_file = 'ev_agent_Substation_{}_3600'.format(dso_num)
        meta_da, start_time, data_da, da_bid_ev = get_metrics_full_multiple_KEY_Mdays_H(V_file, pre_file,
                                                                                          '_metrics' + pos_file)

        V_file = 'ev_agent_Substation_{}_300'.format(dso_num)
        meta_rt, start_time, data_rt, rt_bid_ev = get_metrics_full_multiple_KEY_Mdays(V_file, pre_file,
                                                                                        '_metrics' + pos_file)
        to_kW = 1
        ...
        text = file.read()
        file.close()
        ev_agent = json.loads(text)
        ev_agent = ev_agent['ev']
        property = 'slider_setting'
        prop = []
        opt = []
        dev = []
        ...
        #
        # ev_name = 'R5_12_47_2_tn_108_ev_1'
        # i = meta_da['names'].index(ev_name.replace('ev', 'hse'))
        # opt_bid = da_bid_ev[i][meta_da['bid_four_point_rt_3']['index']]
        # ax1[0].plot((opt_bid.resample('60min').mean() / to_kW).values, label='optimal bid');
        # actual = rt_bid_ev[i][meta_rt['ev_charge']['index']]
        ...
        soc = np.zeros(24 * days)
        soc_all = []
        for i in range(len(rt_bid_ev)):
            temp = rt_bid_ev[i][meta_rt['ev_charge']['index']].resample('60min').mean().values
            soc_temp = rt_bid_ev[i][meta_rt['battery_soc']['index']].resample('60min').mean().values
            # cap = ev_agent[meta_rt['names'][i]]['capacity'] / 1000
            if len(temp) != 24 * days:
                temp = np.append(temp, temp[-1])
            if len(soc_temp) != 24 * days:
                soc_temp = np.append(soc_temp, soc_temp[-1])
            actual = actual + temp / 1000
            soc = soc + soc_temp
            soc_all.append(soc_temp)
            actual_all.append(temp / 1000)
        actual = actual[0:]
        soc = soc[0:] / len(rt_bid_ev)

        # plot soc and setpoint and gridlabd output of individual battery
        figsoc, ax1soc = plt.subplots(2, sharex='col')
        ax1soc[0].plot(actual, label='real-time P set-point')
        ax1soc[0].plot(ev_power_hr[1:] / 1000, label='actual consumption (gridlabd)')
        plt.ylabel('kW')
        plt.title('Agrregated EV performance')
        ax1soc[0].grid()
        ax1soc[1].plot(soc, label='mean soc')
        ev_soc_hr = ev_soc_avg.resample('60min').mean().values
        # ax1soc[1].plot(ev_soc_hr[1:]/100, label='g_soc')
        ax1soc[1].grid()
        # for i in range(len(soc_all)):
        #     if i==114: # max(soc_all[i]) >= 1:
        #         print(i)
...
#### new waterheater agent 
    ...
    if substation:
        ...
        # plt.plot(hvac_power.resample('5min').mean().values, label="hvac load")
        # plt.plot(wh_power.resample('5min').mean().values, label="wh load")
        # plt.plot(total_power.resample('5min').mean().values, label="total load")
