...
class MarkerCollorsJSONpython:
    ...
    def __init__(self):
        ...
        """
        self.lines = ['-', '--', '-.', ':']
        self.Marker = ['.', 'o', 'v', '^', '<', '>', '1', '2', '3', '4', '8', 's', 'p', 'P', '*', 'h', 'H', '+', 'x',
                       'X', 'D', 'd', '|', '_']
        self.Color_convergency = ['IndianRed', 'DarkSalmon', 'Red', 'MediumVioletRed', 'Coral', 'DarkOrange', 'Gold',
                                  'Moccasin', 'GreenYellow', 'Lime', 'LimeGreen', 'MediumSeaGreen', 'Green',
                                  'YellowGreen', 'Olive', 'LightSeaGreen', 'BurlyWood', 'Tan', 'RosyBrown',
                                  'DarkGoldenrod', 'Peru', 'Chocolate', 'SlateGray', 'Black']
        self.Color_market = ['LightSkyBlue', 'DeepSkyBlue', 'DodgerBlue', 'MediumSlateBlue', 'MediumBlue', 'DarkBlue']
        self.Color_DA = ['BurlyWood', 'RosyBrown', 'SandyBrown', 'DarkGoldenrod', 'Peru', 'Chocolate', 'SaddleBrown',
                         'Sienna', 'Brown', 'Maroon', 'Silver', 'Gray', 'SlateGray', 'Black']
        self.Color_RT = ['GreenYellow', 'Lime', 'LimeGreen', 'MediumSpringGreen', 'MediumSeaGreen', 'SeaGreen', 'Green',
                         'DarkGreen', 'YellowGreen', 'Olive', 'LightSeaGreen', 'Teal', 'Aqua', 'Turquoise', 'CadetBlue',
                         'SkyBlue', 'DeepSkyBlue', 'DodgerBlue']
        self.Color_GLD = ['IndianRed', 'Salmon', 'DarkSalmon', 'LightSalmon', 'Crimson', 'Red', 'DarkRed', 'Pink',
                          'MediumVioletRed', 'Coral', 'OrangeRed', 'DarkOrange', 'Gold', 'Moccasin', 'PeachPuff',
                          'khaki', 'Violet', 'Indigo']
...
def DSOplots(obj_Market, obj_Color):
    ...
    for i in range(da_convergence_start, da_convergence_start + N_convergence_hours):
        convergency_max = make_convergency_test(data_s=obj_Market.DSO3600[2][obj_Market.DSO3600[0][V_analis]['index']],
                                                t=i)
...
def RETplots(obj_Market, obj_Color):
    ...
    M_in = 0
    for i in range(da_convergence_start, da_convergence_start + N_convergence_hours):
        convergency_max = make_convergency_test(data_s=obj_Market.RET3600[2][obj_Market.RET3600[0][V_analis]['index']],
                                                t=i)
        plt.plot(convergency_max, marker=markers[M_in], color=obj_Color.Color_convergency[M_in], label=str(i) + '-h',
                 linewidth=1, markersize=5)
...
def Inverter(obj_Market, obj_DER_PYT_battery, obj_DER_GLD_inverter, obj_Color):
    ...
    plt.plot(sumAgreDA.resample('5min').fillna('ffill').values, color=obj_Color.Color_DA[M_in], marker=markers[M_in],
             label='DA')
    M_in = M_in + 1
    ...
    fig = plt.figure('Plot inverter sum power and RET price Inv')
...
def Water_Heater(obj_Market, obj_DER_PYT_water, obj_DER_GLD_house, obj_Color):
    ...
    for i in range(da_convergence_start, da_convergence_start + N_convergence_hours):
        convergency_max = make_convergency_test(data_s=row_list, t=i)
        plt.plot(convergency_max, marker=markers[M_in], color=obj_Color.Color_convergency[M_in], label=str(i) + '-h')
        plt.ylabel('optimal quantity bid (kWh)')
        plt.xlabel('from time ahead to present (hours)')
        M_in = M_in + 1
    plt.grid(True)
    plt.legend(bbox_to_anchor=(1.1, 1.00))
...
def HVAC(obj_Market, obj_DER_PYT_hvac, obj_DER_GLD_house, obj_Color):
    ...
    M_in = 0
    V_analis = ['room_air_temperature', 'outdoor_temperature', 'cooling_setpoint', 'cooling_basepoint',
                'heating_setpoint']
    home = 0
    ...
    indexM = meta[V_analis]['index']
    ax2 = ax1.twinx()
    ax2.plot(df.loc[:, indexM].values, marker=markers[M_in], color=obj_Color.Color_market[0])
    ax2.set_ylabel('RT retail HVAC agent price ($/kWh)', color=obj_Color.Color_market[0])
