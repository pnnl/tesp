...
def get_weather(Load_weather,base_case):
    starting_weather_dataframe = 1
    if Load_weather:
        for day in day_range: 
            if starting_weather_dataframe == 1:
                weather_df = load_weather_data(base_case, '/DSO_', str(dso), str(day))
            else:
                temp_weather_df = load_weather_data(base_case, '/DSO_', str(dso), str(day))
                weather_df = pd.concat([weather_df, temp_weather_df], axis=0)
            starting_weather_dataframe += 1 
    return weather_df


def plots_residential_load_weather(hse_attr,day_range,houses,base_case,Load_House_data,Load_weather):
    if Load_House_data: 
    ...
    house_att_df['Datetime'] = pd.to_datetime(house_att_df['Datetime'])
    house_att_df.set_index('Datetime', inplace=True)
    house_att_df['Total Residential load'] = house_att_df.sum(axis=1)
    house_att_df.reset_index(inplace=True)
    weather_df = get_weather(Load_weather,base_case)
    if Load_weather:
        fig, ax = plt.subplots(figsize=(12,6))
        fig.suptitle('Residential Load (Substation_1_house.h5) and Temperature Plot')
        ax.set_xlabel('Time of day (hours)',  size=label_font)
        ax.set_ylabel('Load (kW)', size=label_font)
        ax.plot(house_att_df['Datetime'],house_att_df['Total Residential load'], label='Residential Load')
        plt.legend(loc='upper left', prop={'size': legend_font})
        ax2 = ax.twinx() 
        color = 'tab:red'
...
def plot_commercial_load_weather(path,Load_weather):
    df_final = sum_all_csv_Files(path) 
    df_final.set_index('# timestamp', inplace=True)
    df_final['Total Commercial load'] = df_final.sum(axis=1)/1000
    df_final.reset_index(inplace=True)
    df_v1 = df_final[['# timestamp','Total Commercial load']]
    ...
    if Load_weather:
        ...
        ax.set_xlabel('Time of day (hours)',  size=label_font)
        ax.set_ylabel('Load (unit?)', size=label_font)
        ax.plot(df_v1['# timestamp'],df_v1['Total Commercial load'], label='Commercial Load')
        plt.legend(loc='upper left', prop={'size': legend_font})
        ax2 = ax.twinx() 
        color = 'tab:red'
        ax2.set_ylabel('Temperature (F)', color=color, size=label_font)
...
def plot_comm_res(house_att_df,df_v1):
    df_v1.set_index('# timestamp', inplace=True) #good
    df_v1_resampled = df_v1.resample('5min').mean() #good 
    df_v1_resampled.reset_index(inplace=True)
    df_sum = pd.DataFrame()
    df_sum['sum'] = house_att_df['Total Residential load']+ df_v1_resampled.iloc[:-1,1]
    ...
    ax.set_xlabel('Time of day (hours)',  size=label_font)
    ax.set_ylabel('Loads (Unit?)', size=label_font)
...
def substation_plot(path):
    ...
    ax.plot(df_substation['timestamp'],df_substation['avg(distribution_load.mag)'],label=' Substation Load (Commercial & Residential)')
    plt.legend(loc='upper left', prop={'size': legend_font})
    ax2 = ax.twinx() 
    color = 'tab:red'
    ax2.set_ylabel('Temperature (F)', color=color, size=label_font)
...
if __name__ == '__main__':
    ...
    folder_name = 'AK_Anchorage_Large_parallel_and_comm_overload_test2_15_fl'  # 'AZ_Tucson_Large_substation_load_verify_fl'
    ...
    Load_weather = True
    # substation_plot(base_case+GLD_prefix+'1/'+'substation_data.csv')
    ...
    ###########################################################################
    ####################### residential plot loading ###############################
    ##########################################################################
    hse_attr = 'total_load_avg'
    Load_House_data = True
    #Load_weather = True
    house_att_df=plots_residential_load_weather(hse_attr,day_range,houses,base_case,Load_House_data,Load_weather)
    ####################### commercial plot loading ###############################
    df_v1=plot_commercial_load_weather(base_case+GLD_prefix+'1/',Load_weather)
    plot_comm_res(house_att_df,df_v1)
    ###########################################################################
    ########################### Load House Data ###############################
    ###########################################################################
