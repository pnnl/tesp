...
plt.ylabel('DSO price ($/kWh)');plt.xlabel('Quantity (kW)');plt.grid(True);plt.legend()
        ...
        if HVAC:
            ...
            for i in range(len(data_individual)):
                ...
                if data_key[i] not in hvac_agent.keys():
                    ...
                    participating_hvac.update({data_key[i]:data_individual[i][meta_S[V_analis]['index']].values})#resample('60min').mean().values})
...
#### new HVAC
    if HVAC_agent and 1:
        ...
        meta_S, start_time, data_s, data_individual, data_key = get_metrics_full_multiple_KEY_Mdays(V_file, pre_file,
                                                                                                    '_metrics' + pos_file)
        #meta_I_ver, start_time, Order = get_data_multiple_days(V_file, days, pre_file, pos_file)
        V_analis = ['agent_room_temperature','GLD_air_temperature','outdoor_temperature','cooling_setpoint','cooling_basepoint','heating_setpoint','heating_basepoint','cleared_price']#
...
#### new waterheater agent 
    if Water_agent and 1:
         ...
         meta_S, start_time, data_s, data_individual, data_key = get_metrics_full_multiple_KEY_Mdays_wh(V_file,pre_file,'_metrics'+pos_file)
