...
from tesp_support.dso_CFS import dso_CFS
from tesp_support.customer_CFS import customer_CFS

import tesp_support.DSOT_plots
import tesp_support.DSO_rate_making
import tesp_support.Wh_Energy_Purchases

import tesp_support.DSO_quadratic_curves
...
DSOmetadata = tesp_support.DSOT_plots.load_json(metadata_path, dso_metadata_file)
...
for month in month_def:
    generate_case_config = tesp_support.DSOT_plots.load_json(month[1], 'generate_case_config.json')
    ...
    # Start at day 'n' after first few days are discarded.  Assumes that simulation runs to end of month with 'm' extra days at the end.
...
for dso_num in dso_range:
    ...
    GLD_metadata = tesp_support.DSOT_plots.load_json(month_def[0][1] + agent_prefix + str(dso_num), file_name)
    if annual_energy:
        tesp_support.DSOT_plots.tic()
        year_meter_df, year_energysum_df, year_trans_sum_df = tesp_support.DSO_rate_making.annual_energy(month_def, GLD_prefix, str(dso_num), GLD_metadata)
        os.chdir(case_path)
...
# --------------- AGGREGATE ANNUAL AMENITY SCORES  ------------------------------
    if annual_amenity:
        annual_amenity_df = tesp_support.DSOT_plots.annual_amenity(GLD_metadata, month_def, GLD_prefix, str(dso_num))
        os.chdir(case_path)
        tesp_support.DSOT_plots.tic()
...
if load_stats:   # Finds Q_max amongst other things.
    tesp_support.DSOT_plots.dso_load_stats(dso_range, month_def, case_path, metadata_path, True)

# --------------- AGGREGATE ANNUAL LMPS LOADS for FORECASTER RETUNING  ------------------------------
if annual_lmps:
    dso_num = '3'
    tesp_support.DSOT_plots.dso_lmp_stats(month_def, case_path, renew_forecast_file)
    tesp_support.DSOT_plots.plot_lmp_stats(case_path, case_path, dso_num, 7)

if gen_stats:
    # Annual LMP needs to be run once to ensure that the annual opf file is created
    GenAMES_df = tesp_support.DSOT_plots.generation_statistics(case_path, config_path, system_case, total_day_range, False)

if train_lmps:
    obj=tesp_support.DSO_quadratic_curves.DSO_LMPs_vs_Q(case_path)
...
if wholesale:
    for dso_num in dso_range:
        Market_Purchases = tesp_support.Wh_Energy_Purchases.Wh_Energy_Purchases(case_path, str(dso_num), True)
...
if retail:
    ...
    for dso_num in dso_range:
        ...
        agent_metadata = tesp_support.DSOT_plots.load_json(month_def[0][1] + agent_prefix + str(dso_num), agent_file_name)

        GLD_metadata = tesp_support.DSOT_plots.customer_meta_data(GLD_metadata, agent_metadata, metadata_path)
        num_ind_cust = DSOmetadata['DSO_' + str(dso_num)]['number_of_customers'] * DSOmetadata['DSO_' + str(dso_num)]['RCI customer count mix']['industrial']
        ...
        DSO_Cash_Flows, DSO_Revenues_and_Energy_Sales, tariff, surplus = tesp_support.DSO_rate_making.DSO_rate_making(case_path, dso_num, GLD_metadata,
                                                                                         required_revenue,
...
if customer_cfs:
    # dso_range = [1]
    customer_df = tesp_support.dso_helper_functions.get_customer_df(dso_range, case_path, metadata_path)
    ...
    customer_mean_df = tesp_support.dso_helper_functions.get_mean_for_diff_groups(customer_df,
                                                                                  main_variables, variables_combs,
                                                                                  cfs_start_position=24)
...
if dso_cfs:
    DSO_df, CapitalCosts_dict_list, Expenses_dict_list, Revenues_dict_list, DSO_Cash_Flows_dict_list = \
        tesp_support.dso_helper_functions.get_DSO_df(dso_range, generate_case_config, DSOmetadata, case_path, base_case_path)
...
if stats:
    bill = True
    rci_df = tesp_support.DSOT_plots.RCI_analysis(dso_range, month_def[0][1], case_path, metadata_path, dso_metadata_file,
                                              bill)
    ...
    for para in params:
        tesp_support.DSOT_plots.metadata_dist_plots(system=para[0], sys_class=para[1], variable=para[2], dso_range=dso_range,
                            case=month_def[0][1], data_path=case_path, metadata_path=metadata_path, agent_prefix=agent_prefix)
