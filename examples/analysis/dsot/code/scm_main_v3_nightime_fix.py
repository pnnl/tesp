...
# from examples.capabilities.ercot.case8.prepare_case import hours
...
def ev_schedule(ev_charger_availability, ev_cap_charger, ev_e_rated, energy_depleted, 
                ...
                n_ports_used_dict, hours_here, night_start, night_end):
    ...
    weight_idx_t = cp.Variable((n_batt, n_intervals), integer=True, name='b_t')
    weight_idx = cp.Variable(n_batt, integer=True, name='b')
    M = 1000.0

    n_intervals = 12  # Avijit: to stop simulation at a specific interval.
    ...
    objective = sum(-100000 * (soc[idx][t] - soc[idx][t-1]) for idx in range(n_batt) for t in range(n_intervals) if (t >
                    0) and (ev_charger_availability[idx][t] == 1))# if
                    # ev_charger_availability[idx][t] == 1)
    # objective = sum(-100000 * (soc[idx][t]) for idx in range(n_batt) for t in range(n_intervals))
    # # Obj 2: Avoid multiple charging instances
    # objective += sum(-100000 * lambda_c[idx][t] for idx in range(n_batt) for t in range(n_intervals))
    # if smooth:
    ...
    for idx in range(n_batt):
        for t in range(n_intervals):
            # constraints.append(lambda_c[idx][t] >= 0)
            # constraints.append(lambda_c[idx][t] <= 1)

            # if t == 0:
                # # initialize weight vector
                # constraints.append(weight_idx_t[idx][t] == 0)
            ...
            if t > 0:
                ...
                if (ev_charger_availability[idx][t] == 0) and (all_allocated_ev_dayofweek_availability[idx][t] == 1):
                    ...
                    # once plugged in by SCM then it can come to zero but cannot become one again.
                    # constraints.append(lambda_c[idx][t] >= lambda_c[idx][t - 1])
                    # constraints.append(lambda_c[idx][t] <= lambda_c[idx][t-1])

                    constraints.append(lambda_c[idx][t+1] - lambda_c[idx][t] >= -M* lambda_c[idx][t])
                    constraints.append(lambda_c[idx][t] - lambda_c[idx][t+1] >= -M* (1-lambda_c[idx][t]))


                    # # penalize vehicles that are available but not connected to the port
                    # constraints.append(weight_idx_t[idx][t] == weight_idx_t[idx][t-1] + (1-lambda_c[idx][t])*5)
                ...
                # constraints.append(lambda_c[idx][t + 1] <= lambda_c[idx][t])
            ...
            # # do not change ports or vehicle charging decisions during the nighttime. This must only happen for
            # # GOVs and POVs. POVs do not have infrastructure limitations but this will make sure same as GOVs just in
            # # case.
            # if (hours_here[t] >= night_start) or (hours_here[t] <= night_end):
            #     constraints.append(lambda_c[idx][t] == lambda_c[idx][t - 1])
            #     # constraints.append(lambda_c[3][t] == 1)

            # if idx == 3 and t == 13:
            #     constraints.append(lambda_c[idx][t] == 1)
            #     constraints.append(p_batt_c[idx][t] == 0)

        # constraints.append(weight_idx[idx] == sum(weight_idx_t[idx][t] for t in range(n_intervals)))
    ...
    # try:
    mydict = {"MSK_DPAR_OPTIMIZER_MAX_TIME": 3000}
...
def scm_ev(df_ev, df_port, xfmr_loading_df, xfmr_rating_info, batch_size, n_intervals, f1, f2, sens_flag,
           threshold_cutoff, smooth, f3, f4, f5, f6, night_start, night_end):
    ...
    for batch in range(0, total_xfmrs, batch_size):
        ...
        for ktoo in range(batch, min(batch + batch_size, total_xfmrs)):
            ...
            if len(evs_allocation_df) > 0:
                ...
                soc_init = [x/200 for x in evs_allocation_df['max SOC'].tolist()]
                ...
                for ev_idx in range(n_batt):
                    ...
                    if start_drive < end_drive:    
                        ...
                        ev_availability = [0] * 24  # n_intervals
                        ev_availability[int(end_drive):int(start_drive+1)] = [1] * (int(start_drive+1) - int(end_drive))
                        # ev_availability[end_drive:start_drive] = [0] * (start_drive - end_drive)  # bug
...
def main(inventory_filename, grid_forecast_filename, size_of_batch, xfmr_rating_data_filename, f1, f2, sens_flag,
    ...
    controlled_demand_evs = scm_ev(df_ev_input, df_port_info, base_xfmr_loading_df, xfmr_rating_data, size_of_batch,
                                   n_hrs, f1, f2, sens_flag, threshold_cutoff, smooth, f3, f4, f5, f6, night_start, night_end)
...
if __name__ == '__main__':
    ...
    inventory_filename = "vehicle1_master_Large_Year_2042_jul31.xlsx"  #
    # 'vehicle_master_Large_Year_2040_randmaxsoc.xlsx'
    # inventory_filename = "vehicle_master_Large_Year_2040_randmaxsoc.xlsx"
    grid_forecast_filename = 'AZ1_Tucson_Large_grid_forecast_jul31.csv'
    # decide on size of batch
    size_of_batch = 50
    xfmr_rating_data_filename = "AZ1_Tucson_Large_grid_dummy_to_size_mapping_jul31.json"
    ...
    failed_xfrmrs = main(inventory_filename, grid_forecast_filename, size_of_batch, xfmr_rating_data_filename, f1,
                         f2, sens_flag, threshold_cutoff, smooth, f3, f4, f5, f6, night_start, night_end)
    end_time = time.time()
