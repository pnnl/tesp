...
def ev_schedule(ev_charger_availability, ev_cap_charger, ev_e_rated,
            ...
            if t > 0:
                if ev_charger_availability[idx][t] == 0:
                    ...
                    constraints.append(soc[idx][t] == soc[idx][t - 1] +
                                       p_batt_c[idx][t-1] * eff[idx] / ev_e_rated[idx])
                    if smooth:
    ...
    for t in range(n_intervals):
        ...
        for idx in range(n_batt):
            ...
            ev_demand_recorder[idx].append(p_batt_c[(idx, t)].value)
            s += p_batt_c[(idx, t)].value

            energy_sum += ev_e_rated[idx]*soc[(idx, t)].value
    ...
    for idx in range(n_batt):
        for t in range(n_intervals):
            soc_record[idx].append(soc[(idx, t)].value)
            energy_record[idx].append(ev_e_rated[idx]*soc[(idx, t)].value)
            pbat_record[idx].append(p_batt_c[(idx, t)].value)
            s += lambda_c[(idx, t)].value
    ...
    for batch in range(0, total_xfmrs, batch_size):
        ...
        for ktoo in range(batch, min(batch + batch_size, total_xfmrs)):
            ...
            if len(evs_allocation_df) > 0:
                ...
                soc_min = evs_allocation_df['min SOC'].tolist()
                ...
                p = multiprocessing.Process(target=ev_schedule,
                                            args=(ev_charger_availability, ev_cap_charger, ev_e_rated,
                                                  ...
                                                  xfmr_rating_info[str(k)], xfmr_dict2, sens_flag, threshold_cutoff, smooth, soc_per_hour, soc_dict, energy_dict, p_batt_c_dict))

                # ev_schedule(ev_charger_availability, ev_cap_charger, ev_e_rated,
                #                                   soc_min, soc_max, eff, soc_init, n_batt, n_intervals,
                #                                   xfmr_loading, num_ports, controlled_demand_xfmrs,
                #                                   controlled_demand_evs, k, xfmr_dict,
                #                                   xfmr_rating_info[str(k)], xfmr_dict2, sens_flag, threshold_cutoff, smooth, soc_per_hour, soc_dict, energy_dict, p_batt_c_dict)
...
if __name__ == '__main__':
    ...
    f1 = 'controlled_xfmr_demand_smooth3.csv'

    f2 = 'controlled_ev_demand_smooth3.csv'

    f3 = 'ev_energy_info.csv'
