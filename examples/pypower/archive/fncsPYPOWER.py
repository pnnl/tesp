...
def make_dictionary(ppc, rootname):
	...
	print (json.dumps(ppdict), file=dp)
	dp.close()
...
with warnings.catch_warnings():
	...
	print ('t[s],Converged,Pload,P7,V7,LMP_P7,LMP_Q7,Pgen1,Pgen2,Pgen3,Pgen4', file=op)
	fncs.initialize()
...
#		ts += dt
	...
	while ts <= tmax:
		...
		if ts >= tnext:
			...
			print (ts, res['success'], bus[:,2].sum(), bus[6,2], bus[6,7], bus[6,13], bus[6,14], gen[0,1], gen[1,1], gen[2,1], gen[3,1], sep=',', file=op)
			fncs.publish('LMP_B7', 0.001 * bus[6,13])
			fncs.publish('three_phase_voltage_B7', 1000.0 * bus[6,7] * bus[6,9])
			print('publishing LMP=', 0.001 * bus[6,13], 'vpos=', 1000.0 * bus[6,7] * bus[6,9])
			# update the metrics
