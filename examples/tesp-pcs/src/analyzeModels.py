...
def analyzeDOT(dotModel):
  ...
  pattern1 = '[a-zA-Z]+'
  pattern2 = '[0-9]+'
  ids = np.unique(np.array([re.findall(pattern1, node['id'])
                            for node in dotModel['nodes']]))
  print('The unique ids of the nodes are: {0} (total {1})'.format(
      ids, len(ids)))

  capNodes = [node for node in dotModel['nodes'] if ids[0] in node['id']]
  capNodes = sorted(capNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))
  loadNodes = [node for node in dotModel['nodes'] if ids[1] in node['id']]
  loadNodes = sorted(loadNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))
  meterNodes = [node for node in dotModel['nodes'] if ids[2] in node['id']]
  meterNodes = sorted(meterNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))
  ...
  nodeNodes = sorted(nodeNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))
  tmNodes = [node for node in dotModel['nodes'] if ids[4] in node['id']]
  tmNodes = sorted(tmNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))
  tnNodes = [node for node in dotModel['nodes'] if ids[5] in node['id']]
  tnNodes = sorted(tnNodes, key=lambda i: int(
      re.findall(pattern2, i['id'])[0]))

  houses = [node for node in dotModel['nodes']
            if 'tn' in node['id'] and 'house' in node['shape']]
  print('In the DOT graph there are:')
  print('\tnodes: {0}, out of which'.format(len(dotModel['nodes'])))
  print('\t\tcap: {0} ({1} .. {2})'.format(
      len(capNodes), capNodes[0]['id'], capNodes[-1]['id']))
  print('\t\tload: {0} ({1} .. {2})'.format(
      len(loadNodes), loadNodes[0]['id'], loadNodes[-1]['id']))
  print('\t\tmeter: {0} ({1} .. {2})'.format(
      len(meterNodes), meterNodes[0]['id'], meterNodes[-1]['id']))
  print('\t\tnode: {0} ({1} .. {2})'.format(
      len(nodeNodes), nodeNodes[0]['id'], nodeNodes[-1]['id']))
  print('\t\ttm: {0} ({1} .. {2})'.format(
      len(tmNodes), tmNodes[0]['id'], tmNodes[-1]['id']))
  print('\t\ttn: {0} ({1} .. {2})'.format(
      len(tnNodes), tnNodes[0]['id'], tnNodes[-1]['id']))
  print('\t\t------------------------')
...
def analyzeGLMdict(glmModel):
  ...
  """
  print('======================== GLM ==================================')
  print('The populated GLM dict has the following keys: {0}'.format(
      glmModel.keys()))
  ...
  glmMeterNodes = [node for node in glmModel['billingmeters'].keys(
  ) if bmIds[0].split('_')[0] in node]
  glmTnNodes = [node for node in glmModel['billingmeters'].keys()
                if bmIds[1].split('_')[0] in node]
  # GLM triplex nodes extracted, sorted and unique
  glmMeterNodesNum = np.unique(
      np.array(sorted([int(meter.split('_')[5]) for meter in glmMeterNodes])))
  glmTnNodesNum = np.unique(
      np.array(sorted([int(tn.split('_')[5]) for tn in glmTnNodes])))

  bmTnGkids = np.array([])
  for bm in glmTnNodes:
    bmTnGkids = np.append(bmTnGkids, np.array(
        glmModel['billingmeters'][bm]['children']))
  tnGkidId = np.unique(np.array(sorted(
      [child.split('_')[4] + '_' + child.split('_')[6] for child in bmTnGkids])))
  bmHse = [bmGkid for bmGkid in bmTnGkids if tnGkidId[0].split('_')[
      1] in bmGkid]
  bmBat = [bmGkid for bmGkid in bmTnGkids if tnGkidId[1].split('_')[
      1] in bmGkid]
  bmSol = [bmGkid for bmGkid in bmTnGkids if tnGkidId[2].split('_')[
      1] in bmGkid]

  bmMeterGkids = np.array([])
  for bm in glmMeterNodes:
    bmMeterGkids = np.append(bmMeterGkids, np.array(
        glmModel['billingmeters'][bm]['children']))
  meterGkidId = np.unique(np.array(sorted(
      [child.split('_')[4] + '_' + child.split('_')[6] for child in bmMeterGkids])))
  bmLoadZones = [bmGkid for bmGkid in bmMeterGkids if meterGkidId[0].split('_')[
      1] in bmGkid]

  hsIds = np.unique(sorted([hs.split('_')[4] + '_' + hs.split('_')[6]
                            for hs in glmModel['houses'].keys()]))
  hsTnHses = [hs for hs in glmModel['houses'].keys() if hsIds[1].split('_')[
      0] in hs]
  hsLoadHses = [hs for hs in glmModel['houses'].keys() if hsIds[0].split('_')[
      0] in hs]

  hsTnHsesParent = np.array([])
  for hs in hsTnHses:
    hsTnHsesParent = np.append(
        hsTnHsesParent, np.array(glmModel['houses'][hs]['parent']))
  hsTnHsesParentIds = np.unique(np.array(sorted([parent.split(
      '_')[4] + '_' + parent.split('_')[6] for parent in hsTnHsesParent])))

  hsLoadHsesParent = np.array([])
  for hs in hsLoadHses:
    hsLoadHsesParent = np.append(
        hsLoadHsesParent, np.array(glmModel['houses'][hs]['parent']))
  hsLoadHsesParentIds = np.unique(np.array(sorted([parent.split('_')[4] + '_' + parent.split(
      '_')[6] if len(parent.split('_')) > 6 else parent.split('_')[4] for parent in hsLoadHsesParent])))

  print('The unique IDs of the billingmeters are: {0}'.format(bmIds))
  print('\tGrand kids to the billing meters `{0}` IDs: {1}'.format(
      bmIds[1], tnGkidId))
  print('\tGrand kids to the billing meters `{0}` IDs: {1}'.format(
      bmIds[0], meterGkidId))
  ...
  print('\t\ttriplex nodes: {0} (_tn_{1}_mtr_# .. _tn_{2}_mtr_#)'.format(
      len(glmTnNodesNum), glmTnNodesNum[0], glmTnNodesNum[-1]))
  print('\t\tmeters on triplex nodes (_tn_#_mtr_#): {0}'.format(
      len(glmTnNodes)))
  ...
  print('\t\tmeters: {0} (_meter_{1} .. _meter_{2})'.format(
      len(glmMeterNodesNum), glmMeterNodesNum[0], glmMeterNodesNum[-1]))
  ...
  for mtrDot in dotTms:
    ...
    for currBillMtr in currBillMtrs:
      ...
      G.add_edge(edgeTnSource, currBillMtr,
                 ename='line_tn_{0}_mtr_{1}'.format(
                     mtrN, currBillMtr.split('_')[-1]),
                 edata={
                     'from': edgeTnSource,
                     'to': currBillMtr,
                     'length': 0
      ...
      for child in glmModel['billingmeters'][currBillMtr]['children']:
        ...
        if child.split('_')[6] == 'hse':
          ...
          if glmModel['houses'][child]['parent'].split('_')[6] == 'mhse':
            ...
            G.nodes()[glmModel['houses'][child]
                      ['parent']]['nclass'] = 'house meter'
            ...
            G.add_edge(currBillMtr, glmModel['houses'][child]['parent'],
                       ename='line_tn_{0}_mtr_{1}_mhse_{2}'.format(mtrN, currBillMtr.split(
                           '_')[-1], glmModel['houses'][child]['parent'].split('_')[-1]),
                       edata={
                'from': currBillMtr,
            ...
            G.add_edge(glmModel['houses'][child]['parent'], child,
                       ename='line_tn_{0}_mhse_{1}_hse_{2}'.format(
                mtrN, glmModel['houses'][child]['parent'].split('_')[-1], child.split('_')[-1]),
                edata={
          ...
          if child.split('_')[6] == 'ibat':
            parentName = '{0}_tn_{1}_mbat_{2}'.format(
                feederRename, mtrN, child.split('_')[-1])
            ...
            G.add_edge(currBillMtr, parentName,
                       ename='line_tn_{0}_mtr_{1}_mbat_{2}'.format(
                           mtrN, currBillMtr.split('_')[-1], parentName.split('_')[-1]),
                       edata={
                           'from': currBillMtr,
                           'to': parentName,
                           'length': 0
            ...
            G.add_edge(parentName, child,
                       ename='line_tn_{0}_mbat_{1}_ibat_{2}'.format(
                           mtrN, parentName.split('_')[-1], child.split('_')[-1]),
                       edata={
                           'from': parentName,
                           'to': child,
                           'length': 0
                       })
            gchildName = '{0}_tn_{1}_bat_{2}'.format(
                feederRename, mtrN, child.split('_')[-1])
            ...
            G.add_edge(child, gchildName,
                       ename='line_tn_{0}_ibat_{1}_bat_{2}'.format(
                           mtrN, child.split('_')[-1], gchildName.split('_')[-1]),
                       edata={
                           'from': child,
                           'to': gchildName,
                           'length': 0
            ...
            parentName = '{0}_tn_{1}_msol_{2}'.format(
                feederRename, mtrN, child.split('_')[-1])
            G.add_node(parentName)
            ...
            G.add_edge(currBillMtr, parentName,
                       ename='line_tn_{0}_mtr_{1}_msol_{2}'.format(
                           mtrN, currBillMtr.split('_')[-1], parentName.split('_')[-1]),
                       edata={
                           'from': currBillMtr,
                           'to': parentName,
            ...
            G.add_edge(parentName, child,
                       ename='line_tn_{0}_msol_{1}_isol_{2}'.format(
                           mtrN, parentName.split('_')[-1], child.split('_')[-1]),
                       edata={
                           'from': parentName,
                           'to': child,
            ...
            gchildName = '{0}_tn_{1}_sol_{2}'.format(
                feederRename, mtrN, child.split('_')[-1])
            G.add_node(gchildName)
            ...
            G.add_edge(child, gchildName,
                       ename='line_tn_{0}_isol_{1}_sol_{2}'.format(
                           mtrN, child.split('_')[-1], gchildName.split('_')[-1]),
                       edata={
                           'from': child,
                           'to': gchildName,
