...
(export FNCS_CONFIG_FILE=SGIP1b_auction.yaml && export FNCS_FATAL=NO && exec python -c "import tesp_support.api as tesp;tesp.auction_loop('SGIP1b_agent_dict.json','SGIP1b')" &> auction1b.log &)
(export FNCS_CONFIG_FILE=pypower.yaml && export FNCS_FATAL=NO && export FNCS_LOG_STDOUT=yes && exec python -c "import tesp_support.api as tesp;tesp.pypower_loop('sgip1_pp.json','SGIP1b')" &> pypower1b.log &)
