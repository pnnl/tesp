...
(export FNCS_CONFIG_FILE=SGIP1d_auction.yaml && export FNCS_FATAL=NO && exec python -c "import tesp_support.api as tesp;tesp.auction_loop('SGIP1d_agent_dict.json','SGIP1d')" &> auction1d.log &)
(export FNCS_CONFIG_FILE=pypower.yaml && export FNCS_FATAL=NO && export FNCS_LOG_STDOUT=yes && exec python -c "import tesp_support.api as tesp;tesp.pypower_loop('sgip1_pp.json','SGIP1d')" &> pypower1d.log &)
