#!/bin/bash
(exec helics_broker -f 3 --loglevel=7 --name=mainbroker &> broker.log & jobs -p > tesp.pid)
...
#(exec helics_recorder --input=recorderSources.json --stop 21600s &> recorder.log & jobs -p >> tesp.pid)
