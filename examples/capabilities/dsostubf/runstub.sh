...
(export FNCS_CONFIG_FILE=gld_player.yaml && exec python3 -c \"import tesp_support.load_player as tesp;tesp.load_player_loop('./case_config', 'gldLoad')\" &> ./gld_player.log &)
(export FNCS_CONFIG_FILE=ref_player.yaml && exec python3 -c \"import tesp_support.load_player as tesp;tesp.load_player_loop('./case_config', 'refLoadMn')\" &> ./ref_player.log &)
" > run.sh
