...
def add_comm_zones(fdr, bldg, key, mode=None):
    ...
    """
    glm = fdr.config.glm
    comm_loads = fdr.config.base.comm_loads
    batt_metadata = fdr.config.batt_metadata
    ev_metadata = fdr.config.ev_metadata
    ...
    if bldg['type'] not in ['large_office', 'ZIPLOAD']:
        ...
        if np.random.uniform(0, 1) <= fdr.config.storage_percentage:
        ...
        if np.random.uniform(0, 1) <= fdr.config.solar_percentage:
            ...
            pv_scaling_factor = inv_power / fdr.config.rooftop_pv_rating_MW
        ...
        if np.random.uniform(0, 1) <= fdr.config.ev_percentage:
            ...
            drive_sch = ev_metadata.match_driving_schedule(ev_range, ev_mileage, ev_max_charge)
            ...
            if not ev_metadata.is_drive_time_valid(drive_sch):
            ...
            if fdr.config.case_type['ev']:  # all pvCases(HR) have ev populated
                ...
                glm.add_metrics_collector(evname, "evchargerdet")
