...
(exec gridlabd -D SCHED_PATH="${TESP_DATA}/schedules" -D USE_HELICS -D METRICS_FILE=TE_ChallengeH_metrics.json TE_Challenge.glm &> helics_gridlabd.log &)
...
(export HELICS_CONFIG_FILE=helics_eplus.json && exec energyplus -w "${TESP_DATA}/energyplus/USA_AZ_Tucson.Intl.AP.722740_TMY3.epw" -d output -r MergedH.idf &> helics_eplus.log &)
