...
(export FNCS_CONFIG_FILE=eplus.yaml && exec energyplus -w "${TESP_DATA}/energyplus/USA_AZ_Tucson.Intl.AP.722740_TMY3.epw" -d output -r Merged.idf &> eplus.log &)
...
(export FNCS_FATAL=YES && exec gridlabd -D SCHED_PATH="${TESP_DATA}/schedules" -D USE_FNCS -D METRICS_FILE=TE_Challenge_metrics.json TE_Challenge.glm &> gridlabd.log &)
