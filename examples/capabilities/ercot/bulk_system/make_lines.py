...
#	labels = dict(zip(range(nbus), [str(x) for x in range(nbus)]))
	...
	nx.draw_networkx_nodes (graph, pos, nodelist=list(n345), node_color='r', node_size=80, alpha=0.3, ax=ax) 
	nx.draw_networkx_edges (graph, pos, edgelist=list(e345), edge_color='r', width=2, alpha=0.8, ax=ax)
	nx.draw_networkx_nodes (graph, pos, nodelist=list(n138), node_color='b', node_size=20, alpha=0.3, ax=ax) 
	nx.draw_networkx_edges (graph, pos, edgelist=list(e138), edge_color='b', width=1, alpha=0.8, ax=ax)
	nx.draw_networkx_labels (graph, pos, lbl345, font_size=10, font_weight='bold', ax=ax)
	nx.draw_networkx_labels (graph, pos, lbl138, font_size=8, ax=ax)
