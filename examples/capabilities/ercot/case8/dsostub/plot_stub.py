...
def unit_color_label(diction, key):
...
def process_pypower(name_root):
    ...
    for key in bus_keys:
        ...
        print('{:2d} {:8.2f} {:8.2f} {:8.2f}  {:s}'.format(int(key), float(row['Pnom']),
                                                           float(row['Qnom']), float(row['ampFactor']),
                                                           row['GLDsubstations'][0]))  # TODO curveScale, curveSkew
    ...
    for i in range(data_g.shape[0]):
        clr, lbl = unit_color_label(diction, gen_keys[i])
