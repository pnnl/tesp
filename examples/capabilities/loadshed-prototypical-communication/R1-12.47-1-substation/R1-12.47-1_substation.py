  ...
  while grantedTime <= simTime:
    ...
    for ind in range (0, endpoint_count):
      ...
      if epName.split('/')[0] == fedName and 'substation' in epName.split('/')[1]:
        logger.info('<<<<< time: {0}; federate: {1}; endpoint: {2}; message? {3}  >>>>>'.format(grantedTime, fedName, epName, h.helicsEndpointHasMessage(fedEP) and 'YES' or 'NO'))
        epMessages = dict({})
        while h.helicsEndpointHasMessage(fedEP):
          ...
          # If there are multiple messages from different previous time instances the following line would overwrite them
        ...
        for epKey in epMessages.keys():
          logger.info(f'\toriginal source: {epMessages[epKey]["original source"]}, source: {epMessages[epKey]["source"]}, data: {epMessages[epKey]["data"]}, time received: {epMessages[epKey]["time received"]}')
