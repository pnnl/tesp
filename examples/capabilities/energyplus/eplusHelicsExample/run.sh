...
#(exec helics_player prices.txt -n player --local --time_units=ns --stop 172800s --loglevel=7 &> player.log &)
...
#(exec helics_recorder --config-file helicsRecorder.json --timedelta 1s --period 300s --stop 172800s --loglevel=7 &> tracer.log &)
...
(exec helics_player prices.txt -n player --local --time_units=ns --stop 172800s &> player.log &)
(exec eplus_agent_helics 172800s 300s SchoolDualController eplus_eplus_metrics.json  0.10 25 4 4 helics_eplus_agent2.json &> eplus_agent.log &)
(exec helics_recorder --config-file helicsRecorder.json --timedelta 1s --period 300s --stop 172800s &> tracer.log &)
