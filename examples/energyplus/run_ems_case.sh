...
#(export FNCS_LOG_STDOUT=yes && export FNCS_CONFIG_FILE=eplus.yaml && exec energyplus -w $EPWFILE -d out$OUTPUT -r $MERGED.idf &> eplus$OUTPUT.log &)
...
(export HELICS_CONFIG_FILE=eplusH.json && exec energyplus -w $EPWFILE -d out$OUTPUT -r $MERGED.idf &> eplus$OUTPUT.log &)
