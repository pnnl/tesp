...
from ..dsot.helpers_dsot import Curve, MarketClearingType, get_intersect
...
class DSOMarket:
    ...
    def curve_aggregator_DSO_DA(self, demand_curve_DA, Q_max):
        ...
        """
        for idx in range(self.windowLength):
            if (max(demand_curve_DA[idx].quantities) > Q_max and
                max(demand_curve_DA[idx].quantities) > self.DSO_Q_max):
    ...
    def curve_preprocess(self, substation_demand_curve, Q_max):
        """ An internal shared function called by curve_aggregator_DSO_RT and
        curve_aggregator_DSO_DA functions to truncate the substation demand curve
        before aggregation as well as convert the retail prices into wholesale prices
