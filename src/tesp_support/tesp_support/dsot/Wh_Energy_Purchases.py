...
def Wh_Energy_Purchases(dir_path, dso_num, simdata=False, h1=5, h2=16, h3=20, place='Houston'):
    ...
    * Total annual average price, total cost and total energy purchases from the bilateral market are then computed.
    ...
    conditions_Q = [
        (bilateral_MW_data['hour'] >= h1) & (bilateral_MW_data['hour'] < h2) & (bilateral_MW_data['weekday'] < 5),
        ((bilateral_MW_data['hour'] >= h2) & (bilateral_MW_data['hour'] < h3) & (bilateral_MW_data['weekday'] < 5)) | (
    ...
    conditions_P = [
        (bilateral_price_data['hour'] >= h1) & (bilateral_price_data['hour'] < h2) & (
                    bilateral_price_data['weekday'] < 5),
        ((bilateral_price_data['hour'] >= h2) & (bilateral_price_data['hour'] < h3) & (
                    bilateral_price_data['weekday'] < 5)) | (
    ...
    choices_P = [bilateral_price_data[(bilateral_price_data['hour'] >= h1) & (bilateral_price_data['hour'] < h2) & (
                bilateral_price_data['weekday'] < 5)][price_name].mean(),
                 bilateral_price_data[((bilateral_price_data['hour'] >= h2) & (bilateral_price_data['hour'] < h3) & (
                         bilateral_price_data['weekday'] < 5)) | ((bilateral_price_data['hour'] >= h1) & (
                             bilateral_price_data['hour'] < h3) & (
                                                                          bilateral_price_data['weekday'] >= 5))][
                     price_name].mean(),
                 bilateral_price_data[(bilateral_price_data['hour'] < h1) | (bilateral_price_data['hour'] >= h3)][
                     price_name].mean()]
...
if __name__ == '__main__':
    # ercot_path = 'C:/Users/mayh819/PycharmProjects/tesp-private/tesp-private/ercot/case8/dsostub'
    ...
    hh2 = 16
    hh3 = 20
    location = 'Houston'  # place_names = ['CPS', 'North', 'South', 'West', 'Houston']

    Market_Purchases = Wh_Energy_Purchases(data_path, dso_number, sim_data, hh1, hh2, hh3, location)
    print(Market_Purchases)

    case_path = 'C:/Users/reev057/PycharmProjects/DSO+T/Data/May/Base_858c4e40'
    os.chdir(case_path)
    with open('DSO' + str(dso_number) + '_Wholesale_Purchases.json', 'w') as f:
