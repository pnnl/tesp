...
import commercial_feeder_glm_dod as comm_FG_dod
        ...
        for o in list(model[t].keys()):
            if 'load_class' in model[t][o]:
                if model[t][o]['load_class'] == 'C':
                    ...
                    #   comm_type = 'STRIPMALL'
                    #   total_stripmall += 1
                    select_bldg = None  # Kishan: This will always make all load_class == c to be a zipload object
...
def write_houses(basenode, op, vnom):
    ...
    for i in range(nhouse):
        ...
        print('  cooling_setpoint 72.0; // ', cooling_str + ';', file=op)
        print('  heating_setpoint 70.0; // ', heating_str + ';', file=op)
...
def write_players(feedercnt, current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder, rootname, current_weather_folder_name, each_grid_name, op):
    # current_player_filenames: not used but need it to be saved in json
    # rootname: not used but keeping it for now.
    # bldgs_per_feeder: not used, will be used when adding the players to commercial loads
    # players_added = []
    # total_count = 0
    if feedercnt == 1:
        for bldg_key, bldg_count_to_add in current_bldg_count_for_each_player.items():
            # add player object with unique name
            player_filename = f'../../{current_weather_folder_name}/{each_grid_name}/{bldg_key}.csv'
            player_name = bldg_key
            print("object player {", file=op)
            print(f"  name {player_name};", file=op)
            print(f"  file {player_filename};", file=op)
            print("};", file=op)
            # players_added.append(bldg_key)

        # add recorders as well
        print("object group_recorder {", file=op)
        print("  name loads_power_A_recorder;", file=op)
        print("  group "'class=load'" AND "'groupid=STREETLIGHTS'";", file=op)
        print("  property base_power_A;", file=op)
        print("  file loads_power_A.csv;", file=op)
        print("  interval 1;", file=op)
        print("  limit 10000000;", file=op)
        print("}", file=op)

        print("object group_recorder {", file=op)
        print("  name loads_power_B_recorder;", file=op)
        print("  group "'class=load'" AND "'groupid=STREETLIGHTS'";", file=op)
        print("  property base_power_B;", file=op)
        print("  file loads_power_B.csv;", file=op)
        print("  interval 1;", file=op)
        print("  limit 10000000;", file=op)
        print("}", file=op)

        print("object group_recorder {", file=op)
        print("  name loads_power_C_recorder;", file=op)
        print("  group "'class=load'" AND "'groupid=STREETLIGHTS'";", file=op)
        print("  property base_power_C;", file=op)
        print("  file loads_power_C.csv;", file=op)
        print("  interval 1;", file=op)
        print("  limit 10000000;", file=op)
        print("}", file=op)
    ...
    #     for bldg_key, bldg_count_to_add in current_bldg_count_for_each_player.items():
    #         players_added.append(bldg_key)
    # all buildings assigned, none left to assign to the commercial loads in glm. These extra com
    # loads from glm will be zeroes out.
    # if all([x == 0 for x in list(current_bldg_count_for_each_player.values())]):
    #
    ...
    #
    # if not n_f:
    #     pass  # if empty dictionary then no need to add anymore.
    # else:
    #     for bldg_key, bldg_count_to_add in n_f.items():
    #         # add player object with unique name
    #         player_filename = f'../../{current_weather_folder_name}/{each_grid_name}/{bldg_key}.csv'
    #         player_name = bldg_key
    #         print("object player {", file=op)
    ...
    #         total_count += 1
    #
    #         if total_count > bldgs_per_feeder:
    #             break
    #
    #
    # if feedercnt == 1:
    ...
    #     print("object group_recorder {", file=op)
    #     print("  name loads_power_A_recorder;", file=op)
    #     print("  group "'class=load'";", file=op)
    #     print("  property base_power_A;", file=op)
    #     print("  file loads_power_A.csv;", file=op)
    #     print("  interval 1;", file=op)
    #     print("  limit 10000000;", file=op)
...
def ProcessTaxonomyFeeder(climate_zone_count, feedercnt, current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder, current_weather_folder_name, each_grid_name, outname, rootname, vll, vln, avghouse, avgcommercial, list_bldgs_assigned):
    ...
    if os.path.isfile(fname):
        ...
        for line in itr:
            if re.search('object', line):
                ...
                elif 'default_maximum_voltage_error' in line:
                    print('  default_maximum_voltage_error 1e-2;', file=op)
                else:
                    print(line, file=op)
        ...
        # add players here
        # current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder,
        write_players(feedercnt, current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder, rootname, current_weather_folder_name, each_grid_name, op)
        ...
        if forERCOT:
            ...
            for key in comm_loads:
                ...
                bldg_definition = comm_FG_dod.define_comm_loads(comm_loads[key][1], comm_loads[key][2],
                                                    dso_type, ashrae_zone, comm_bldg_metadata)
                comm_FG_dod.create_comm_zones(bldg_definition, comm_loads, key, op, batt_metadata,
            ...
            # below script removes any bldgs that are fully mapped into the circuits and keep the pending bldgs only in the dictionary "n_f"
            n_f = {}
            for k, v in current_bldg_count_for_each_player.items():
                if v == 0:
                    pass
                    ...
                    n_f[k] = v
            # we consider all player file names in glm whose bldgs are not fully mapped on existing circuits
            players_added = []
            for k, v in n_f.items():
                players_added.append(k)

            # This will make sure the same comm loads from glm will have same csv building player names from FEDS.
            if list_bldgs_assigned is None:
                list_bldgs_assigned = []# climate zone = 1 run and feeder_1 run
                list_bldgs_assigned3 = []
                allzero = False
                for x in range(len(comm_loads)):
                    curr_plr_name = players_added[int(x % len(players_added))]
                    if current_bldg_count_for_each_player[curr_plr_name] > 0:
                        list_bldgs_assigned3.append(curr_plr_name)
                        current_bldg_count_for_each_player[curr_plr_name] -= 1
                    elif allzero:
                        list_bldgs_assigned3.append(None)
                    else:
                        notfound = True
                        while(notfound):
                            x = x + 1
                            ...
                            if all([x == 0 for x in list(current_bldg_count_for_each_player.values())]):
                                ...
                                list_bldgs_assigned3.append(None)
                                break
                list_bldgs_assigned.append(list_bldgs_assigned3)
            elif players_added == []:
                list_bldgs_assigned2 = []
                for x in range(len(comm_loads)):
                    list_bldgs_assigned2.append(None)

                list_bldgs_assigned.append(list_bldgs_assigned2)
            else:
                if feedercnt == 2:  # feeder_2 run for climate zone 1
                    list_bldgs_assigned2 = []
                    allzero = False
                    for x in range(len(comm_loads)):
                        curr_plr_name = players_added[int(x % len(players_added))]
                        if current_bldg_count_for_each_player[curr_plr_name] > 0:
                            list_bldgs_assigned2.append(curr_plr_name)
                            current_bldg_count_for_each_player[curr_plr_name] -= 1
                        elif allzero:
                            list_bldgs_assigned2.append(None)
                        else:
            ...
            # feds_player_add_count = 0
            map_playername_to_comm_loads = {}
            cload_idx = 0
            list_bldgs_assigned4 = list_bldgs_assigned[feedercnt-1]
            for key in comm_loads:
                player_name = list_bldgs_assigned4[cload_idx]
                # if feds_player_add_count < len(players_added):
                #     if current_bldg_count_for_each_player[players_added[feds_player_add_count]] != 0:
                #         pass  # stay at the same player name as the previous iteration
                #     else: # move to next player name
                #         feds_player_add_count += 1
                #     if feds_player_add_count >= len(players_added):
                #         map_playername_to_comm_loads[key] = "exception"
                #         continue
                #     player_name = players_added[feds_player_add_count]
                #     current_bldg_count_for_each_player[player_name] -= 1
                # write_commercial_loads (rgn, key, op)
                bldg_definition = comm_FG_dod.define_comm_loads(comm_loads[key][1], comm_loads[key][2],
                                                    dso_type, ashrae_zone, comm_bldg_metadata)
                comm_FG_dod.create_comm_zones(bldg_definition, comm_loads, key, op, batt_metadata,
                                  storage_percentage, ev_metadata, ev_percentage,
                                  solar_percentage, pv_rating_MW, solar_Q_player,
                                  case_type, metrics, metrics_interval, player_name, None)
                map_playername_to_comm_loads[key] = player_name
                cload_idx += 1
                # else:
                #     player_name = None
                #     # write_commercial_loads (rgn, key, op)
        ...
        # Adjust the transformer ratings
        # if rating_xfrmr_map is None:
        #     rating_xfrmr_map = {}
        rating_to_update_list = []
        config_names_updated = []
        for key in comm_loads:
            parent_meter = comm_loads[key][0]
            # Find the transformers connected to the street light zip loads
            for trfrmr_key, trfrmr_properties in model["transformer"].items():
                f_name = trfrmr_properties["from"]
                t_name = trfrmr_properties["to"]
                # below property matches with the hash variable too since prefix is empty.
                config_name = trfrmr_properties["configuration"]

                if (parent_meter == f_name) or (parent_meter == t_name):
                    csv_name = map_playername_to_comm_loads[key]
                    if csv_name == None:
                        continue
                    # if config_name == "XF3_POLETOP_ABCN_30":
                    #     k = 1
                    # read csv file and find the peak demand
                    df = pd.read_csv(f'{current_weather_folder_name}/{each_grid_name}/{csv_name}.csv', header=None)
                    # Find the transformer configuration of the identified transformers
                    # modify them based on the peak demand
                    # if csv_name == "bld_11":
                    #     k = 1
                    peak_demand = df.iloc[:,1].max()/1000  # in VAs read from csv file, converted to kVA for tf config.
                    nphs = int(comm_loads[key][4])
                    if nphs > 1:
                        rating_to_update = Find3PhaseXfmrKva2(peak_demand)
                    else:
                        rating_to_update = Find1PhaseXfmrKva2(peak_demand)
                    rating_to_update_list.append(rating_to_update)
                    config_names_updated.append(config_name)
                    if rating_to_update > xfused[config_name][1]:  # this ensures, highest ratng value from csv is assigned to a particular transformer configuration. Since one configuration is shared by multiple transformers.
                        xfused[config_name][1] = rating_to_update
        #                 rating_xfrmr_map[config_name] = rating_to_update
        # else:
        #     pass
        ...
        if len(Eplus_Bus) > 0 and Eplus_Volts > 0.0 and Eplus_kVA > 0.0:
            ...
            if metrics_interval > 0 and "transformer" in metrics:  # TODO: make sure "metrics" variable has transformer in there.
                print('  object metrics_collector {', file=op)
                print('    interval', str(metrics_interval) + ';', file=op)
                print('  };', file=op)
            print('}', file=op)
    ...
    return current_bldg_count_for_each_player, list_bldgs_assigned, model, xfused, comm_loads


def populate_feeder(climate_zone_count, feedercnt, current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder, current_weather_folder_name, each_grid_name, list_bldgs_assigned, configfile=None, config=None, taxconfig=None):
    ...
    if taxconfig is not None:
        ...
        for c in taxchoice:
            if c[0] in rootname:
                case_name = config['SimulationConfig']['CaseName']
                current_bldg_count_for_each_player, list_bldgs_assigned, model, xfused, comm_loads = ProcessTaxonomyFeeder(climate_zone_count, feedercnt, current_player_filenames, current_bldg_count_for_each_player, bldgs_per_feeder, current_weather_folder_name, each_grid_name, case_name, rootname, c[1], c[2], c[3], c[4], list_bldgs_assigned)
#                quit()
    return current_bldg_count_for_each_player, list_bldgs_assigned, model, xfused, comm_loads
