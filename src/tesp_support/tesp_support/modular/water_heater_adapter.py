"""Adapter pattern for Water Heater agent — wraps legacy WaterHeaterDSOT as DeviceStrategy.
...
from ..modular.dsot_device_strategies import WaterHeaterDSOTStrategy
from ..modular.market_protocol import Bid, MarketWindow
from ..modular.price_signal import PriceSignal


class WaterHeaterDSOTStrategyAdapter(WaterHeaterDSOTStrategy):
    """Adapter that wraps an existing WaterHeaterDSOT legacy agent as a DeviceStrategy.
    
    This adapter:
    - Preserves all existing water heater logic (tank temperature, setpoint, demand)
    ...
    Water heater is a flexible load that can shift consumption across time periods.
    
    Args:
        device_id (str): Identifier for this water heater device
        wh_dict (dict): Configuration dictionary for water heater parameters
        house_properties (dict): GLM house properties
        legacy_agent (WaterHeaterDSOT, optional): Existing legacy agent to wrap.
    
    Attributes:
        legacy_agent (WaterHeaterDSOT): The wrapped legacy agent instance
    """
    
    def __init__(self, device_id: str, wh_dict: dict, house_properties: dict, legacy_agent=None):
        """Initialize water heater adapter with legacy agent wrapping."""
        self.legacy_agent = legacy_agent
        
        # Initialize as WaterHeaterDSOTStrategy
        super().__init__(device_id, wh_dict)
        ...
        if self.legacy_agent is None:
            ...
            solver = wh_dict.get('solver', 'ipopt')
            
            self.legacy_agent = WaterHeaterDSOT(wh_dict, house_properties, device_id, 11, current_time_str, solver)
    ...
    def observe_prices(self, price_signal: PriceSignal) -> None:
        ...
        Water heater uses prices to decide when to heat (high flexibility).
    ...
    def _convert_4pt_to_bid(self, bid_4pt: list, market_window: MarketWindow, bid_type: str) -> Bid:
        ...
        return Bid(
            ...
            metadata={
                ...
                'points': list(zip(quantities, prices)),
                'source': 'WaterHeaterDSOT_legacy_adapter',
            },
        )


if __name__ == '__main__':
    print("Water Heater Adapter module loaded")
