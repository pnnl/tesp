...
def load_player_loop_f(casename, keyName):
    ...
    if tmax > nRow:
        raise Exception(
            f"Error: Tmax {tmax} is more than nRow {nRow}, the time period specified in StartTime and EndTime")
    if tmax < nRow:
    ...
    print('Initialize FNCS', flush=True)
    fncs.initialize()
    print('Starting FNCS', flush=True)
    ...
    while ts <= tmax:
        ...
        if load and new_row:
            ...
            for row in fncs_bus:
                ...
                if output:
                    fncs.publish(prefix + '_load_' + idx, val)
                ...
                if tb % day == 0 or ts == 0:
                    ...
                    if output_hist:
                        fncs.publish(prefix + '_load_history_' + idx, json.dumps(time_series_history_pub))
        ...
        if not load and new_row:
            ...
            for i in range(len(genFuel)):
                ...
                if genFuel[i][0] in renew:
                    ...
                    val = f'{val:.3f}'
                    if output:
                        fncs.publish(prefix + '_power_' + idx, val)
                    ...
                    if tb % day == 0 or ts == 0:
                        ...
                        for j in range(history_cnt):
                            ...
                            time_series_history_pub.append(val)
                        if output_hist:
                            fncs.publish(prefix + '_power_history_' + idx, json.dumps(time_series_history_pub))
...
if __name__ == "__main__":
    # load_player_loop_f('./generate_case_config', 'genMn')
    # load_player_loop_f('./generate_case_config', 'genForecastHr')
    load_player_loop_f('./generate_case_config', 'refLoadMn')
    # load_player_loop_f('./generate_case_config', 'refLoadHr')
    # load_player_loop_f('./generate_case_config', 'gldLoad')
