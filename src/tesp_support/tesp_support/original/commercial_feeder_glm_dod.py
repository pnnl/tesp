...
def create_comm_zones(bldg, comm_loads, key, op, batt_metadata, storage_percentage, ev_metadata, ev_percentage,
                      solar_percentage, pv_rating_MW, solar_Q_player, case_type, metrics, metrics_interval,
                      player_name, mode=None):
    ...
    if bldg['type'] not in ['large_office', 'ZIPLOAD']:
        ...
        if player_name is None:
            turnoffziploads = 0
            # current_bldg_count_for_each_player, bldgs_per_feeder
            ...
            for phs in ['A', 'B', 'C']:
                if phs in phases:
                    ...
                    print('  base_power_{:s} street_lighting*{:.2f};'.format(phs, turnoffziploads * phsva), file=op)
            print('};', file=op)
        else:
            # current_bldg_count_for_each_player, bldgs_per_feeder
            phsva = 1000.0 * kva / nphs
            ...
            for phs in ['A', 'B', 'C']:
                if phs in phases:
                    ...
                    print('  current_pf_{:s} {:f};'.format(phs, c_i_pf), file=op)
                    print('  power_pf_{:s} {:f};'.format(phs, c_p_pf), file=op)
                    print(f'  base_power_{phs} {player_name}.value * {round(1/nphs, 2)};', file=op)
            print('};', file=op)

            # # temp test for helics
            # print('object load { // testing loads for helics evs', file=op)
