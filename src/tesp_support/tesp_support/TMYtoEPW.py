...
[weather_path]/TMY2EPW/source_code directory.

Public Functions:
    :convert_tmy2_to_epw: Converts TMY2 file to EPW.
    :convert_tmy3_to_epw: Converts TMY3 file to EPW.
"""
...
def convert_tmy2_to_epw(in_file_root, out_file_root=None):
    ...
    #  print ('tz',tz,'!')
    _dir = lines[0][37]
    if _dir.lower().find('n') != -1:
        sim = ''
    else:
        sim = '-'
    deg = float(lines[0][39:41])
    _min = round(float(lines[0][39:41]) / 60, 1)
    lat = sim + str(deg + _min)
    #  print ('lat',lat,'!')
    _dir = lines[0][45]
    if _dir.lower().find('e') != -1:
        sim = ''
    else:
        sim = '-'
    deg = float(lines[0][47:50])
    _min = round(float(lines[0][51:53]) / 60, 1)
    lot = sim + str(deg + _min)
    #  print ('lot',lot,'!')
    ele = lines[0][55:59].replace(' ', '')
    #  print ('ele',ele,'!')
    temp = 'LOCATION,' + str(city) + ',' + str(state) + ',' + str(country) + ',TMY2,' + \
           str(WBAN) + ',' + str(lat) + ',' + str(lot) + ',' + str(tz) + ',' + str(ele)
    newline.append(temp)
...
def convert_tmy3_to_epw(in_file_root, out_file_root=None):
    ...
    subprocess.Popen('TMY3toTMY2_ansi ' + in_file_root + '.tmy3' + ' >> ' + in_file_root + '.tmy2', shell=True).wait()
    if out_file_root is None:
        out_file_root = in_file_root
    convert_tmy2_to_epw(in_file_root, out_file_root)
