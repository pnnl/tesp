...
class DSOMarketDSOT:
    ...
    def curve_aggregator_DSO_RT(self, demand_curve_RT, Q_max):
        ...
        if max(demand_curve_RT.quantities) > Q_max:
            print(" Demand Curve range exceeds beyond Q_max, changing the LMP forecaster's Q_max to reflect that and extending the supply curve")
    ...
    def set_Pwclear_RT (self, hour_of_day, day_of_week, lmp=False):
        ...
        """
        if lmp is False:
            self.Pwclear_RT, self.trial_cleared_quantity_RT, self.trial_clear_type_RT = self.trial_wholesale_clearing(self.curve_ws_node[day_of_week][hour_of_day], self.curve_DSO_RT, day_of_week, hour_of_day)
            ...
            try:
                ...
                self.Pwclear_RT = self.curve_a[day_of_week][hour_of_day] * self.trial_cleared_quantity_RT * self.trial_cleared_quantity_RT + self.curve_b[day_of_week][hour_of_day] * self.trial_cleared_quantity_RT + self.curve_c[day_of_week][hour_of_day]
    ...
    def set_Pwclear_DA (self, hour_of_day, day_of_week):
        ...
        for idx in range(self.windowLength):
            ...
            self.Pwclear_DA[idx], self.trial_cleared_quantity_DA[idx], self.trial_clear_type_DA[idx] = self.trial_wholesale_clearing(self.curve_ws_node[day][hour], self.curve_DSO_DA[idx], day, hour)
    ...
    def trial_wholesale_clearing(self, curve_ws_node, curve_DSO, day, hour):
        ...
        if curve_DSO.uncontrollable_only == True:
            ...
            if temp < 0.0:
                log.info("Warning quantities submitted to DSO are negative. The returns are price set to 0, first quantity of the curve, and helpers.ClearingType.UNCONGESTED. BAU case.")
            ...
            if min(curve_ws_node.quantities) <= temp <= max(curve_ws_node.quantities):
                ...
                for idx in range(1, self.num_samples):
                    if curve_ws_node.quantities[idx-1] < cleared_quantity < curve_ws_node.quantities[idx]:
                        cleared_price = curve_ws_node.prices[idx-1]+(cleared_quantity-curve_ws_node.quantities[idx-1])*(curve_ws_node.prices[idx]-curve_ws_node.prices[idx-1])/(curve_ws_node.quantities[idx]-curve_ws_node.quantities[idx-1])
                ...
                log.info("ERROR dso min: " + str(min(curve_ws_node.quantities)) + ", max: " + str(max(curve_ws_node.quantities)) + " curve_DSO.quantities[0] " +str(curve_DSO.quantities[0]))
            ...
            for idx in range(len(buyer_quantities)-1):
                if buyer_prices[idx] > seller_prices[idx] and buyer_prices[idx+1] < seller_prices[idx+1]:
                    ...
                    if Pwclear == float('inf'):
                        ...
                        log.info(" Warning dso clearing problem points -- buyer_quantities[idx]: " + str(buyer_quantities[idx]) +
                        ...
                        for i in range(idx_old):
                            if buyer_quantities[idx] == buyer_quantities[idx + index_delta]:
                                ...
                                break  # break here
                            if i == idx_old-1:
                                log.info(" Error dso clearing problem points -- buyer_quantities: " + str(buyer_quantities) +
                                         " buyer_prices : " + str(buyer_prices) + " seller_prices: " + str(seller_prices) +
                                         " day: " + str(day) + " hour: " + str(hour))
                        p1 = (buyer_quantities[idx], buyer_prices[idx])
                        p2 = (buyer_quantities[idx + index_delta], buyer_prices[idx + index_delta])
            ...
            log.info("ERROR dso intersection not found (not supposed to happen). quantities: " + str(buyer_quantities) + ", buyer_prices: " + str(buyer_prices) + ", seller_prices: " + str(seller_prices))
    ...
    def substation_supply_curve_RT(self, retail_obj):
        ...
        supply_curve_RT = helpers.curve(self.retail_rate(self.pricecap), self.num_samples) # pricecap of the supply_curve has to be the retail pricecap 
    ...
    def substation_supply_curve_DA(self, retail_obj):
        ...
        for idx in range(self.windowLength):
            ...
            supply_curve_DA[idx] = helpers.curve(self.retail_rate(self.pricecap), self.num_samples) # pricecap of the supply_curve has to be the retail pricecap 
        
        if self.transformer_degradation == True: 
            for idx in range(self.windowLength):     
                supply_curve_DA[idx].quantities, supply_curve_DA[idx].prices = self.supply_curve(Prclear_DA[idx], FeederCongCapacity, FeederPkDemandCapacity, self.num_samples, Q_max_retail, maxPuLoading, TOC_dict)
        else:
            for idx in range(self.windowLength):
                if self.trial_cleared_quantity_DA[idx] > Q_max_retail:  # the DSO's Q_max has been increased after observing buyer bids to be higher than the substation Q max
                    ...
                    supply_curve_DA[idx].quantities = np.linspace(0,  self.trial_cleared_quantity_DA[idx], self.num_samples)
                else:
    ...
    def supply_curve(self, Prclear, FeederCongCapacity, FeederPkDemandCapacity, num_samples, Q_max, maxPuLoading, TOC_dict):
        ...
        dollars, pu_loading = self.generate_TOC(60, maxPuLoading, num_samples, TOC_dict)           #cost per 60 minutes (so kW is same as kWh)
...
if __name__ == "__main__":
    ...
    def test_dso_clearing_RT():
        ...
        retail_dict = {
                "num_samples": 1000,
                "pricecap": 1 * 1.25, # pricecap of the retail agent has to be pricecap of the DSO agent multiplies retail rate
                "Q_max": 14875000,
                "maxPuLoading": 1.5,         
                "windowLength": 48,
        ...
        buyer_bid_1 = [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]]
        market.curve_aggregator_RT('Buyer', buyer_bid_1, 'abc')
 
        buyer_bid_2 = [[-5.0, 0.18590669596595727], [0, 0.14961564637563854], [0, 0.11412489053123773], [50.0, 0.077833840940919]]
        market.curve_aggregator_RT('Buyer', buyer_bid_2, 'def')
        
        buyer_bid_3 = [[-5.0, 0.17548732414146312], [0, 0.13500968174215666], [0, 0.1287308551647196], [50.0, 0.08825321276541313]]
        market.curve_aggregator_RT('Buyer', buyer_bid_3, 'ghi')
        
        buyer_bid_4 = [[-5.0, 0.18667559364692246], [0, 0.1507218182969266], [0, 0.11301871860994966], [50.0, 0.0770649432599538]]
        market.curve_aggregator_RT('Buyer', buyer_bid_4, 'jkl')
        
        buyer_bid_5 = [[-5.0, 0.17887117592152127], [0, 0.1373476742467282], [0, 0.12639286266014807], [50.0, 0.08486936098535501]]
        ...
        market.curve_buyer_RT.quantities = market.curve_buyer_RT.quantities*((DSO.num_of_customers + 2150000.0)*DSO.customer_count_mix_residential/DSO.number_of_gld_homes)
        ...
        plt.plot(market.curve_buyer_RT.quantities, market.curve_buyer_RT.prices, label='Aggregated demand curve before preprocessing')
        ...
        plt.plot(DSO.curve_ws_node[day_of_week][hour_of_day].quantities, DSO.curve_ws_node[day_of_week][hour_of_day].prices, label='wholesale node supply curve')
        plt.axvline(dso_dict['DSO_Q_max'], label='Original Q max', linewidth=2.0,color='r')
        plt.axvline(max(DSO.curve_ws_node[day_of_week][hour_of_day].quantities), label='Adjusted Q max', linewidth=2.0,color='g')
        ...
        plt.plot(market.curve_seller_RT.quantities, market.curve_seller_RT.prices, label='Substation supply curve', linewidth=2.0)
        plt.title('Retail Clearing')
        plt.plot(market.curve_buyer_RT.quantities, market.curve_buyer_RT.prices, label='Aggregated curve', linewidth=2.0)
    ...
    def test_dso_clearing_DA():
        ...
        buyer_bid_2 = [[[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
        ...
        buyer_bid_3 = [[[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
        ...
        buyer_bid_4 = [[[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
        ...
        buyer_bid_5 = [[[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
                       [[-5.0, 0.19751947247066967], [0, 0.1505365976142517], [0, 0.11320393929262458], [50.0, 0.06622106443620661]],
        ...
        plt.plot(market.curve_buyer_DA[0].quantities, market.curve_buyer_DA[0].prices, label='Aggregated demand curve before preprocessing')
        plt.plot(DSO.curve_DSO_DA[0].quantities, DSO.curve_DSO_DA[0].prices, label='Aggregated demand curve at DSO level')
        plt.plot(DSO.curve_ws_node[day_of_week][hour_of_day].quantities, DSO.curve_ws_node[day_of_week][hour_of_day].prices, label='wholesale node supply curve')   
        plt.xlabel('Quantity')
        plt.ylabel('Price')
        plt.legend(loc='best')
