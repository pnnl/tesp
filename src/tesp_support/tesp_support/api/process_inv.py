...
def read_inv_metrics(path, name_root, diction_name=''):
    ...
    pre_dict_path = os.path.join(path, f'precool_{name_root}_metrics.json')
    ...
    # print ("\n\nFile", name_root, "has substation", sub_keys[0], "at bulk system bus", bulkBus, "with", xfMVA, "MVA transformer")
    # print("\nFeeder Dictionary:")
    # for key in sub_keys:
    ...
    data_s = np.empty(shape=(len(sub_keys), len(times), len(lst_s[time_key][sub_keys[0]])), dtype=float)
    ...
    if os.path.exists(pre_dict_path):
        ...
        hrs_p = np.array(times_p, dtype=float)
        denom = 3600.0
        hrs_p /= denom
        ...
        data_p = np.empty(shape=(1, len(times_p), len(lst_p[time_p_key])), dtype=float)
    ...
    vavg = np.zeros(shape=(len(times)), dtype=float)
    vmin = np.full(shape=(len(times)), fill_value=1000.0, dtype=float)
    vmax = np.zeros(shape=(len(times)), dtype=float)
    for i, mtr in enumerate(mtr_keys):
        vbase = diction['billingmeters'][mtr]['vln']
        vscale = 100.0 / vbase
    ...
    scalem = 1.0 / 3600.0
    ax[1, 2].plot(hrs, scalem * (data_m[:, :, MTR_AHI_DURATION_IDX]).squeeze().sum(axis=0), color="blue",
                  label="Range A Hi")
    ax[1, 2].plot(hrs, scalem * (data_m[:, :, MTR_BHI_DURATION_IDX]).squeeze().sum(axis=0), color="cyan",
                  label="Range B Hi")
    ax[1, 2].plot(hrs, scalem * (data_m[:, :, MTR_ALO_DURATION_IDX]).squeeze().sum(axis=0), color="green",
                  label="Range A Lo")
    ax[1, 2].plot(hrs, scalem * (data_m[:, :, MTR_BLO_DURATION_IDX]).squeeze().sum(axis=0), color="magenta",
                  label="Range B Lo")
    ax[1, 2].plot(hrs, scalem * (data_m[:, :, MTR_OUT_DURATION_IDX]).squeeze().sum(axis=0), color="red",
                  label="No Voltage")
...
def plot_inv(diction, save_file=None, save_only=False):
    # can only plot the last call to read_houses_metrics
    # could NOT separate data nicely
...
def process_inv(name_root, diction_name='', title=None, save_file=None, save_only=False):
    ...
    *billing_meter_[name_root]_metrics.json*,
    *capacitor_[name_root]_metrics.json*,
    *regulator_[name_root]_metrics.json*,
    *house_[name_root]_metrics.json* and
    *inverter_[name_root]_metrics.json* for the data;
    it reads *[name_root]_glm_dict.json* for the metadata.
    If possible, it reads *precool_[name_root]_metrics.json* for temperature deviation.
    ...
    path = os.getcwd()
    diction = read_inv_metrics(path, name_root, diction_name)
    plot_inv(diction, title, save_file, save_only)
