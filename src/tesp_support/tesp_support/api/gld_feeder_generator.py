...
class Config:
    ...
    def generate_recs(self) -> None:
        ...
        Returns:
            None
        """
        if hasattr(self, 'out_residential_meta_file'):
            if not self.out_residential_meta_file:
                # self.out_residential_meta_file = "RECS_residential_metadata.json"
                get_RECS_jsons(
                    os.path.join(self.data_path, self.residential_meta_file),
                    os.path.join(self.data_path, self.out_residential_meta_file),
    ...
    def load_recs(self) -> None:
        ...
        assign_defaults(self.res_bld, os.path.join(self.data_path, self.out_residential_meta_file))
    ...
    """
        if not hasattr(self, "in_file_glm") or not self.in_file_glm:
            ...
            self.gis_path = os.path.join(os.path.expandvars('$TESPDIR/data/feeders'), self.gis_file)
        elif self.gis_file:
            self.gis_path = os.path.join(self.data_path, self.gis_file)
        else:
            self.gis_path = False

        if self.gis_path:
            try:
                with open(self.gis_path) as gis:
    ...
    def add_houses(self, basenode: str, v_nom: float) -> None:
        ...
        for i in range(self.nhouse):
            ...
            if self.config.case_type['bt']:
                self.config.batt.add_batt(prob_batt, prob_solar, mtrname1, bat_m_name, bat_name, bat_i_name, phs, v_nom)
...
class Commercial_Build:
    ...
    def add_one_commercial_zone(self, bldg: dict, key: str) -> None:
        ...
        if self.config.case_type['pv']:
            self.config.sol.add_solar(self.config.solar_deployment, bldg['parent'], f'{name}_solmtr', f'{name}_sol', f'{name}_solinv', "ABC", 120.0, bldg['floor_area'])

        if self.config.case_type['bt']:
            self.config.batt.add_batt(self.config.storage_deployment, self.config.solar_deployment, bldg['parent'], f'{name}_batmtr', f'{name}_bat', f'{name}_batinv', "ABC", 120.0)
        ...
        if self.config.gis_file:
            ...
            self.config.pos[f'{name}_solmtr'] = self.config.pos_data[key]
            self.config.pos[f'{name}_batmtr'] = self.config.pos_data[key]
    ...
    def define_commercial_zones(self, rgn: int, key: str, kva: float, feed_type: str) -> None:
        ...
        if comm_type == 'ZIPload':
            ...
            bldg['floor_area'] = self.bldg_area
    ...
    def define_comm_bldg(self, dso_type: str, num_bldgs: float) -> list:
        ...
        while i < num_bldgs:
            ...
            if bldg_type not in ['large_office']:
                ...
                self.bldg_area = Commercial_Build.sub_bin_select(bldg_area_bin, 'total_area', rng.random())
...
class Battery:
    ...
    def add_batt(self, bat_prob: float, sol_prob: float, parent_mtr: str, bat_mtr: str, bat_name: str, inv_name: str, phs: float, v_nom: float) -> None:
        ...
        if rng.random() <= bat_prob and rng.random() <= sol_prob:
...
class Feeder:
    def __init__(self, config: Config, feed_type: str):
        ...
        package to perform graph-based capacity analysis, upgrading fuses, transformers and lines to serve the expected load. Transformers have
        a margin of 20% to avoid overloads, while fuses have a margin of 150% to
        ...
        if self.config.make_plot:
            ...
            if self.config.gis_file:
                ...
                # with rest of the feeder. TODO: validate this.
