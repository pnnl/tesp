...
def get_node_ids(time_series, id_column_name):
    """ Function queries a list of unique values from a time series dataframe 
        based upon a column id entered by the user

    Args:
        time_series (dataframe): time series dataframe that contains the values 
            to be queried
        id_column_name (str): name of the dataframe column where the values are 
            located
    Returns:
        list: list object containing the unique values found in the time series 
            dataframe
    """
...
def get_node_data(time_series, node_id, id_column_name):
    """ Function queries the time series dataframe for the data set identified 
        by the entered node id

    Args:
        time_series (dataframe): time series dataframe contains the node data to
            be queried
        node_id (str): the id of the node that is to be used in the query
        id_column_name (str): name of the time series dataframe column that 
            contains the node ids to be queried against
    Returns:
        dataframe: time series dataframe containing data specific to a single 
            node id
    """
...
def get_time_series_average(time_series, start_date, duration):
    ...
    Args:
        time_series (dataframe): time series dataframe containing the data to be 
            averaged
        start_date (str): the starting date and time that should be used in the 
            calculation of the averages
        duration (int): the duration in hours that the averages should be 
            calculated
    Returns:
        dataframe: dataframe containing the average value for each column in the 
            input dataframe
    """
...
def get_avg_column_value(time_series, val_index):
    ...
    Returns:
        float: calculated average value for the column identified in the function 
            arguments
    """
...
def get_max_column_value(time_series, val_index):
    """ Function searches a designated column in the time series dataframe and 
        returns the maximum value found in the column

    Args:
        time_series (dataframe): time series dataframe containing the data to be 
            searched for a maximum value
        val_index (str): name of the column where the data is located to 
...
def get_min_column_value(time_series, val_index):
    """ Function searches a designated column in the time series dataframe and 
        returns the minimum value found in the column

    Args:
        time_series (dataframe): time series dataframe containing the data to be 
            searched for a minimum value
        val_index (str): name of the column where the data is located to 
...
def get_avg_data_value(time_series, column_id):
    """ Function calculates the average of a column in the time series dataframe 
        and returns the average value of the column

    Args:
        time_series (dataframe): time series dataframe containing the data to be 
            averaged
        column_id (str): name of the data column for which the average is to be 
            calculated
    Returns:
...
def get_accuracy_ratio(input_df, actual_index, simulated_index):
    ...
    Args:
        input_df (dataframe): time series dataframe containing data columns for 
...
def adjust_date_time(start_date, offset_type, offset_val):
    """ Function returns a date time object that is calculated by adding the 
        offset_val to the entered start date
...
def check_for_full_year_data(time_series):
    """ Function checks if the input time series dataframe contains a full 
        year's worth of data

    Args:
        time_series (dataframe): time series dataframe containing a year's worth 
            of data
    Returns:
...
def check_for_hourly_data(time_series):
    ...
    Returns:
        bool: True if the data in the dataframe is hourly and False if it is not 
            hourly
    """
    date_diff = time_series.index[1] - time_series.index[0]
...
def check_for_5_minute_data(time_series):
    ...
    Args:
        time_series (dataframe): time series dataframe containing 5-minute data 
            records
    Returns:
...
def get_time_series_max_value_under(time_series, column_id, compare_value):
    """ Function calculates the maximum value out of the number of values in a 
        dataframe column that are less than a comparison value

    Args:
        time_series (dataframe): time series dataframe containing the data to be 
            compared
        column_id (str): the name of the column in the dataframe where the data 
            is located
...
def get_time_series_max_value_over(time_series, column_id, compare_value):
    """ Function calculates the maximum value out of the number of values in a 
        dataframe column that are greater than a comparison value
...
def get_column_total_value(time_series, column_id):
    ...
    Args:
        time_series (dataframe): the time series dataframe which contains the 
            data to be summed
...
def get_time_series_difference_values(time_series, column_id, time_series2, column_id2):
    """ Function calculates the difference between data in a column of a 
        dataframe with the data in a column of a second dataframe

    Args:
        time_series (dataframe): time series dataframe containing a data set to 
            be used in the calculation
        column_id (str): name of the column where the data to be used is located
        time_series2 (dataframe): time series dataframe containing a data set to
            be used in the calculation
...
def check_dataframe_synchronization(data_frame_1, data_frame_2):
    """ Function checks that two time series dataframes are synchronized by 
        comparing size, starting time, and ending time of the data sets. If they
        are synchronized, the returns "Synchronized". If they are not, then the 
        function will return an error message dependent upon what test failed.
...
def create_testing_dataframe(start_date, end_date, col_names, time_interval):
    ...
    Args:
        ...
        col_names (list<string>): the names to be used as the column headers in 
            the resultant dataframe
        time_interval (int): frequency of time intervals. These designations are
            the same as the designations used to define a pandas date_range 
            e.g. "T", "5T", "H", "12H",...
    Returns:
        dataframe: time series dataframe containing random data values over the 
        course of the defined time range
    """
    np.random.seed(0)
