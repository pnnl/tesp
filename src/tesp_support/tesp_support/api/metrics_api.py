...
def synch_time_series(series_list, synch_interval, interval_unit):
    ...
    :param series_list (List<dataframe>): List containing a set of pandas dataframes each
    representing a time series
    :param synch_interval (integer): the size of the time step which should be used to resample
    the dataframe
    :param interval_unit (string): the measurement unit of the interval to be sampled. The options for
    this function include the following options
    "nanoseconds""seconds","minutes","hours","days","months","years"
    :return(List<dataframe>): pandas dataframe time series containing the resampled columns of data
    """
    synchronized_series = []
...
def get_synch_date_range(time_series):
    ...
    :param time_series (List<dataframe>): List containing a set of pandas dataframes each
    representing a time series
    :return (datetime,datetime): the latest start and the earliest end times found in the list of data frames
    """
...
def synch_series_lengths(time_series):
    ...
    :param time_series (List<dataframe>): List containing a set of pandas dataframes each
    representing a time series
    :return (List<dataframe>): a list containing the clipped time series data frames
    """
    synchronized_series = []
...
def synch_series(time_series, synch_interval, interval_unit):
    """
    Function sychronizes all the time series data frames in the time_series list, so they
    all have the same start and ending times and the same number of times based upon a shared
    sampling interval
    :param time_series (List<dataframe>):
    :param synch_interval (integer):
...
def get_avg_customer_demand(time_series, start_date, val_col_id):
    ...
    :param time_series (dataframe): dataframe representing a time series containing customer demand records
    :param start_date (datetime): the start date and time that should be used in the calculation
    :param val_col_id (string): id of the dataframe column which contains the customer demand data values
    :return (double): the calculated yearly average customer demand
    """
    ...
    if len(averaging_time_series.index) < 8760:
        ...
        return
    avg_results = bc.get_time_series_average(averaging_time_series, begin_date, 8760, "H")
...
def get_under_voltage_count(time_series, val_col_id, minimum_value):
    ...
    :param time_series (dataframe): dataframe containing a time series of transmission values
    :param val_id (string): The id of the dataframe column where the transmission values are located
    :param minimum_value (double): The value that is to be used to compare transmission values against
    :return (dataframe):
    """
...
def get_valuation(time_series, start_date, column_index):
    ...
    :param column_index (string): the dataframe column id that is used to identify the location of the values in the dataframe
    :return (dataframe, double, double): function returns a tuple containing the dataframe containing the valuation
...
def get_pv_aep_valuation(solar_irradiation, pv_system_area, pv_system_efficiency):
    ...
    :return (double): the product of the three input values
    """
...
def actual_der_vs_projected_ratio(actual_der, actual_col_name, projected_col_name, projected_der=None):
    ...
    :param actual_der (dataframe): time series dataframe that contains the total benefits from DER, as observed ex post.
    :param actual_col_name (string): id of the dataframe column that contains the actual DER benefit values
    :param projected_col_name (string): id of the dataframe column that contains the projected DER benefit values
    :param projected_der (dataframe):time series dataframe that contains the projected benefits from DER,
    as observed ex post.
    :return (dataframe): time series dataframe that contains the calculated ratios
    """
...
def get_average_air_temp_deviation(actual_df, actual_col_name, set_point_col_name, set_points_df = None) :
    ...
    :param actual_df (dataframe): per-device average deviation from desired air temperature set point
    :param actual_col_name (string): dataframe column id for the location of actual temperatures
    :param set_point_col_name (string):dataframe column id for the location of set point data
    :param set_points_df (dataframe): time series data frame containing the set points data. This parameter is optional.
    If it is not set to a dataframe object it is expected that both the actual and set point data is in the actual_df
    dataframe
    :return (double): returns the average of the calculated differences between the average of actual indoor temperature
...
def get_transmission_voltage_magnitude(time_series, column_id, start_date, duration):
    ...
    :param time_series:(dataframe) time series dataframe containing the five-minute data
    :param column_id:(string) the name of the dataframe column with contains the transmission voltage data
    :param start_date:(datetime) the starting date and time when the calculations should take place
    :param duration:(integer) the duration in hours to calculate the ending date and time when the
    calculations should take place
    :return:(dataframe) the calculated hourly min, max, and average values in a time series dataframe
    """
...
def get_feeder_energy_losses(feeder_gen_df, gen_column_id, feeder_load_df, load_column_id, start_date_time, duration):
    ...
    Function calculates the impact of transactive energy systems on feeder energy losses. Data records in the time series
    entered as input must be recorded at five minute intervals
    :param feeder_gen_df:(dataframe) data frame containing the 5-min average total generation from bulk power system and DERs
    :param gen_column_id:(string) name of the column in the feeder generation dataframe where the generation data is located
    :param feeder_load_df:(dataframe) data frame containing the 5-min average total load
    :param load_column_id:(string) name of the column in the feeder load dataframe where the load data is located
    :param start_date_time:(string) calculation start date and time
    :param duration:(integer) the duration of time in hours that the calculations are to be performed
    :return:(dataframe) a dataframe object containing the generation, load, and losses data
    """
    calc_times = []
    calc_losses = []
    calc_gens = []
...
def get_peak_demand(time_series, column_id):
    ...
    :param time_series:(dataframe) time series dataframe that contains the demand values over the course of a year
    :param column_id:(string) name of the dataframe column where the demand data is located
    :return:(float) maximum value identified in the dataframe column identified by column_id
    """
    if len(time_series.index) < 8760:
        log_msg.log(log_msg.ERROR, "dataframe does not have 8761 rows")
        return
    rows_df = time_series.iloc[0:8759]
...
def get_peak_supply(time_series, column_id):
    ...
    :param time_series:(dataframe) time series dataframe that contains the supply values over the course of a year
    :param column_id:(string) name of the dataframe column where the supply data is located
    :return:(float) maximum value identified in the dataframe column identified by column_id
    """
    if len(time_series.index) < 8760:
        log_msg.log(log_msg.ERROR, "dataframe does not have 8761 rows")
...
def get_max_under_voltage(time_series, column_id, threshold_val, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe containing feeder voltage data in 5 minute intervals
    :param column_id:(string) the name of the dataframe column where the voltage data is located
    :param threshold_val:(float) the maximum threshold data that is used to compare against the voltage data
    :param start_date_time:(string) calculation start date and time
    :param duration:(integer) the duration of time in hours that the calculations are to be performed
    :return:(dataframe) time series dataframe containing the calculated hourly over voltage maximum values
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_max_over_voltage(time_series, column_id, threshold_val, start_date_time, duration):
    ...
    while end_time <= ts_end_time:
        ...
        #calc_dataframe2["voltage_violation_max"] = abs(calc_dataframe[column_id] - threshold_val)
...
def get_indoor_air_temp_deviation(time_series, column_id, set_point, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe that contains 5 minute max deviation data for a year
    :param column_id:(string) the name of the dataframe column where the deviation data is located
    :param set_point:(float) The set point value that is to be used in the calculation
    :param start_date_time:(string) the date and time that the calculations are to start
    :param duration:(float) the time duration in hours for which the calculations should be performed
    :param start_date_time:(string) calculation start date and time
    :param duration:(integer) the duration of time in hours that the calculations are to be performed
    :return: time series dataframe containing the maximum deviations calculated hourly from the input data
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_average_unit_price(time_series, column_id):
    ...
    :param time_series:(dataframe) time series dataframe that contains the hourly market prices for electricity for
    a year
    :param column_id:(string) name of the dataframe column that the price data is located
    :return:(float) the average unit price for the year
    """
    if len(time_series.index) < 8760:
        log_msg.log(log_msg.ERROR, "dataframe does not have 8761 rows")
        return
...
def get_hot_water_deficit(water_temperatures, water_column_id, desired_temperatures, desired_column_id, flow_rates, flow_column_id, delta_t, start_date_time, duration):
    """
    ...
    :param water_temperatures:(dataframe) per device 5-min average hot water actual temperature
    :param water_column_id:(string) name of the dataframe column where the temperature data is located
    :param desired_temperatures:(dataframe)per device 5-min average hot water temperature set point
    :param desired_column_id:(string) name of the dataframe column where the set point temperature data is located
    :param flow_rates:(dataframe) per device 5-min average hot water flow rate
    :param flow_column_id:(string) name of the dataframe column where the flow rate data is located
    :param delta_t:(float) time difference
    :return:(dataframe) time series dataframe containing the calculated deficit data
    """
...
def get_max_market_price(time_series, column_id, start_date_time):
    ...
    :param time_series:(dataframe) time series dataframe containing the hourly market price for electricity within a territory served by an
    ISO or balancing authority, for each of the 8,760 hours per year.
    :param column_id:(string) name of the dataframe column where the market price data is located
    :param start_date_time: (string) the date and time that the calculations are to start at
    :return:(float) the maximum market price value found in the market price dataset
    """
    st_time = pd.to_datetime(start_date_time)
...
def get_emergency_scarcity_sell(scarcity_power_df, scarcity_col_id, scarcity_price_df, price_col_id, generation_capacity_df, gen_col_id, available_power_df, available_col_id):
    ...
    :param scarcity_power_df:
    :param scarcity_col_id:
    :param scarcity_price_df:
    :param price_col_id:
    :param generation_capacity_df:
    :param gen_col_id:
    :param available_power_df:
    :param available_col_id:
    :return:
    """
    scarcity_sell = []
    calc_times = []
...
def get_max_comm_packet_size(time_series, size_column_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe that contains the communication network packet size Mbytes
    :param size_column_id:(string) name of the dataframe column that contains the packet size data
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time which the calculations should be executed
    :return:(float) the maximum communication packet size for the time period entered
    """
    st_time = pd.to_datetime(start_date_time)
...
def get_mean_absolute_percentage(actual_load_df, actual_col_id, forecasted_load_df, forecasted_col_id, start_date_time, duration):
    """
    Metric is defined in document VM_Mean Absolute Percentage (Load) Error.docx
    :param actual_load_df: time series dataframe containing the actual load observed over a period of time
    :param actual_col_id: name of the column where actual load data is located
    :param forecasted_load_df: time series dataframe containing the forecasted load observed over a period of time
    :param forecasted_col_id: name of the column where forecasted load data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(tuple dataframe,float) time series dataframe containing the calculated ratios, the calculated average value
    """
    calc_ratios= []
    calc_times = []
...
def get_minimum_market_price(time_series, price_col_id):
    ...
    :param time_series: (dataframe) Hourly market prices for electricity
    :param price_col_id: (string) name of the dataframe column where the price data is located
    :return: (float) the mininum market price found in the data over the course of a year
    """
    if len(time_series.index) < 8760:
...
def get_substation_peak_power(time_series, power_col_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series containing substation real power flow (Mvar) at 5 minute intervals
    :param power_col_id:(string) name of the dataframe column containing the power flow data
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the calculated hourly peak power flow
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_reactive_power_demand(time_series, max_col_id, avg_col_id):
    ...
    :param max_col_id:(string) name of the dataframe column containing the 5-minute maximum data
    :param avg_col_id:(string) name of the dataframe column containing the 5-minute average data
    :return:(dataframe) time series dataframe containing the hourly maximum and average values calculated by the function
    """
    st_time = time_series.index[0]
    ts_end_time = time_series.index[-1]
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_system_energy_losses(feeder_generation_df, gen_col_id, feeder_load_df, feeder_col_id, start_date_time, duration):
    """
    Metric is defined in document VM_System Energy Losses.docx
    Function calculates the total energy loss at a feeder
    :param feeder_generation_df:(dataframe)
    :param gen_col_id:(string)
    :param feeder_load_df:(dataframe)
    :param feeder_col_id:(string)
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:
    """
...
def get_total_pv_reactive_power(time_series, pv_col_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe that contains the 5-minute power data used in the calculations
    :param pv_col_id:(string) name of the dataframe column where the power data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the calculated hourly total reactive power values
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_system_energy_loss(energy_sold_df, sold_col_id, energy_purchased_df, purchased_col_id, start_date_time, duration):
    """
    ...
    :param energy_sold_df:(dataframe) time series dataframe containing the 5-minute sold energy data
    :param sold_col_id:(string) name of the dataframe column where the sold data is located
    :param energy_purchased_df:(datframe) time series dataframe containing the 5-minute purchased energy data
    :param purchased_col_id:(string) name of the dataframe column where the purchased data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the calculated hourly energy loss
    """
...
def get_total_wind_reactive_power(time_series, power_col_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe containing the 5-minute wind reactive power data
    :param power_col_id:(string) name of the dataframe column where the wind data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the hourly wind power results
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_total_wind_real_power(time_series, power_col_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe containing the 5-minute wind power data
    :param power_col_id:(string) name of the dataframe column where the wind data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the hourly total wind data results
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_transmission_over_voltage(time_series, voltage_col_id, compare_val, start_date_time, duration):
    ...
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return: time series dataframe containing the hourly transmission over voltage results
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_transmission_under_voltage(time_series, voltage_col_id, compare_val, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe that contains the 5-minute voltage data
    :param voltage_col_id:(string) name of the column where the voltage data is located
    :param compare_val:(float) threshold value to compare the voltage data against
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the calculated hourly under voltage results
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_wind_energy_production(time_series, prod_col_id, start_date_time, duration):
    ...
    :param time_series:(dataframe) time series dataframe that contains the 5-minute wind energy production data
    :param prod_col_id:(string) name of the column where the wind energy production data is located
    :param start_date_time: (string) the starting date and time when the calculation should start
    :param duration:(float) the length of time in hours which the calculations should be executed
    :return:(dataframe) time series dataframe containing the calculated hourly total wind energy results
    """
    st_time = pd.to_datetime(start_date_time)
    ts_end_time = bc.adjust_date_time(st_time, "hours", duration)
    end_time = bc.adjust_date_time(st_time, "minutes", 60)
...
def get_unserved_electric_load(supply_df, supply_col_id, demand_df, demand_col_id):
    ...
    :param supply_df:(dataframe) hourly supply data per year
    :param supply_col_id:(string) name of the dataframe column where the supply data is located
    :param demand_df:(dataframe) hourly demand data per year
    :param demand_col_id:(string) name of the dataframe column where the demand data is located
    :return:(dataframe) time series dataframe containing the calculated unserved load data
    """
