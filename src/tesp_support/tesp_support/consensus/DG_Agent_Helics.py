...
import tesp_support.consensus.consensus_generator as consensus
    ...
    *controller_metrics_root_metrics.json* upon completion.
    ...
    while time_granted < simulation_duration:
        ...
        if time_granted >= tnext_dso_bid_rt:
            ...
            dso_market_obj, time_granted = consensus.Consenus_dist_RT(dso_market_obj, fed, hour_of_day, time_granted,
        ...
        if time_granted >= tnext_retail_adjust_rt and with_market:
            ...
            if pub_count > 0:
                per_phase_generation = -1 * dso_market_obj.trial_cleared_quantity_RT / 3
                for wTopic in ['constant_power_A', 'constant_power_B', 'constant_power_C']:
                    key_gen = dso_market_obj.generators['name'] + '/' + wTopic
