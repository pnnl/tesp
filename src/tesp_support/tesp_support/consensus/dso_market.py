...
class DSOMarket:
    ...
    def set_Pwclear_DA(self, hour_of_day, day_of_week):
        ...
        for idx in range(self.windowLength):
            ...
            self.Pwclear_DA[idx], self.trial_cleared_quantity_DA[idx], \
                self.trial_clear_type_DA[idx] = self.trial_wholesale_clearing(self.curve_ws_node[day][hour],
                                                                              self.curve_DSO_DA[idx])
                
    def trial_wholesale_clearing(self, curve_ws_node, curve_DSO):
        """ A internal shared function called by set_Pwclear_RT and
...
def test():
    ...
    def test_dso_clearing_RT():
        ...
        DSO = DSOMarket(dso_dict, 'www')

        # Instantiate Retail agent
        market = RetailMarket(retail_dict, "retail_1")
