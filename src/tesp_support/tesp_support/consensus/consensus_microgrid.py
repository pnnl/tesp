...
def Consenus_dist_DA(dso_market_obj, DA_horizon, fed, time_granted, time_market_DA_complete):
    ...
    dso_market_obj.trial_cleared_quantity_DA = np.concatenate(
        [-1 * PG[:, agent_idx, kk], -1 * PG[:, agent_idx, kk]]).tolist()
    ...
    np.savetxt("convergence/" + fed_name + "_PG_DA_" + str(int(time_granted)) + ".csv", PG[:, agent_idx, 0:kk],
               delimiter=",")
    np.savetxt("convergence/" + fed_name + "_Delay_P_" + str(int(time_granted)) + ".csv", delay_P[:, 0:kk],
               delimiter=",")
    np.savetxt("convergence/" + fed_name + "_Delay_Q_" + str(int(time_granted)) + ".csv", delay_Q[:, 0:kk],
               delimiter=",")
