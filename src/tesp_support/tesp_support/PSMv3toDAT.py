...
def weatherdat(psmv3csvfile, bus_str, location_str):
    ...
    weather_df.rename(columns={'Relative Humidity':'humidity', 'Temperature':'temperature', 'Pressure':'pressure', 'DNI':'solar_direct', 'DHI':'solar_diffuse', 'Wind Speed':'wind_speed'}, inplace=True)
    ...
    # Use a rolling window to smooth out temperature values which suffer from discretization as they only have one
    # significant figure
    ...
    weather_df_5min.to_csv('weather_' + bus_str + '_' + location_str +
                           '.dat',
                           float_format='%.4f')
