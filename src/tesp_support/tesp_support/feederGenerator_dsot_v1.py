...
def process_nhts_data(data_file):
    ...
    # limit daily miles to maximum possibe range of EV from the ev model data as EVs cant travel more than the range in a day if we dont consider the highway charging
    ...
    # combine all 4 parameters: starttime, endtime, total_miles, travel_day. Ignore vehicle ids that dont have both leaving and arrival time at home
...
def match_driving_schedule(ev_range, ev_mileage, ev_max_charge):
    ...
    # come home and charging must end at the full hour just before vehicle leaves home, the actual chargable hours
    # duration may be smaller than the car home duration by maximum 2 hours.
    ...
    # 30 minutes for home-work. If remaining time is less than an hour, make that commute time but it should not occur as maximum home duration is always less than 23 hours
...
def is_hhmm_valid(time):
    ...
    min = time%100
    if hr > 23 or hr < 0 or min < 0 or min > 59 or type(min) != int:
        return False
    return True
...
# write the optional volt_dump and curr_dump for validation
        ...
        if forERCOT:
            ...
            for key in comm_loads:
                ...
                tesp_support.commbldgenerator.create_comm_zones(bldg_definition, comm_loads, key, op, batt_metadata, storage_percentage, ev_metadata, ev_driving_metadata, ev_percentage, solar_percentage, pv_rating_MW, solar_Q_player, caseType, metrics, metrics_interval, None)
        else:
            replace_commercial_loads(model, h, 'load', 0.001 * avgcommercial)
            ...
            for key in comm_loads:
                # write_commercial_loads (rgn, key, op)
                bldg_definition = tesp_support.commbldgenerator.define_comm_loads(comm_loads[key][1], comm_loads[key][2], dso_type, ashrae_zone, comm_bldg_metadata)
                tesp_support.commbldgenerator.create_comm_zones(bldg_definition, comm_loads, key, op, batt_metadata,
                                                                storage_percentage, ev_metadata, ev_percentage,
                                                                solar_percentage,
                                                                pv_rating_MW, solar_Q_player,
        ...
        print(solar_count, 'pv totaling', '{:.1f}'.format(solar_kw), 'kw with', battery_count, 'batteries and', ev_count, 'electric vehicles' )
        op.close()
...
def populate_feeder(configfile=None, config=None, taxconfig=None):
    ...
    global ev_reserved_soc  # minimum allowed soc level in EV battery. This will affect the assignment of daily miles to an EV
