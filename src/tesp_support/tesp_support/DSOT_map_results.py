...
def bulk_system_map_plot(dataPath, configPath, shapePath, case_config_path, case_config_name, day_range, hour,
	...
	contoursubjects = ['Wholesale LMP', 'Generation Utilization', 'Generation Load', 'Generation Capacity',
					   'Generation Capacity Fraction', 'Load', 'Net Load', 'Load Fraction', 'Renewable Capacity', 'Renewable Generation']
	...
	# Load Results Data
	case_config_data = tesp_support.DSOT_plots.load_json(case_config_path, case_config_name)
	if plotdata:
		if realtime:
			line_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'rt_line', day_range)
			lmp_data_df = tesp_support.DSOT_plots.load_ames_data(dataPath, day_range)
			q_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'rt_q', day_range)
		else:
			line_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'da_line', day_range)
			q_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'da_q', day_range)

			lmp_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'da_lmp', day_range)
		...
		# gen_data_df = tesp_support.DSOT_plots.load_gen_data(dataPath, 'gen', day_range)
	...
	if not ercot200:
		...
		dlines = np.genfromtxt(configPath+'Lines8.csv', dtype=str, skip_header=1, delimiter=',') # ['U',int,int,float,float,int,float,float,float,float,float], skip_header=1, delimiter=',')
		# bus,lon,lat,load,gen,diff,caps
		dbuses = np.genfromtxt(configPath+'Buses8.csv', dtype=[int, float, float, float, float, float, float], skip_header=1, delimiter=',')
		# idx,bus,mvabase,pmin,qmin,qmax,c2,c1,c0
		dunits = np.genfromtxt(configPath+'Units8.csv', dtype=[int, int, float, float, float, float, float, float, float], skip_header=1, delimiter=',')
		...
		# name,bus1,bus2,kV,length[miles],#parallel,r1[Ohms/mile],x1[Ohms/mile],b1[MVAR/mile],ampacity,capacity[MW]
		dlines = np.genfromtxt(configPath+'RetainedLines.csv', dtype=str, skip_header=1, delimiter=',') # ['U',int,int,float,float,int,float,float,float,float,float], skip_header=1, delimiter=',')
		# bus,lon,lat,load,gen,diff,caps
		dbuses = np.genfromtxt(configPath+'RetainedBuses.csv', dtype=[int, float, float, float, float, float, float], skip_header=1, delimiter=',')
		# idx,bus,mvabase,pmin,qmin,qmax,c2,c1,c0
		dunits = np.genfromtxt(configPath+'Units.csv', dtype=[int, int, float, float, float, float, float, float, float], skip_header=1, delimiter=',')
	...
	if ercot200:
		glines138 = nx.draw_networkx_edges(graph, xy, edgelist=lst138, edge_color=c138, width=w138) # , arrowsize=100, arrowstyle='fancy'
	if not plotdata:
		gnodes345 = nx.draw_networkx_nodes(graph, xy, nodelist=list(n345), node_color='saddlebrown', node_size=60)
	if not ercot200:
		gnodes345 = nx.draw_networkx_nodes(graph, xy, nodelist=list(n345), node_color='saddlebrown', node_size=60)
	if ercot200:
		gnodes138 = nx.draw_networkx_nodes(graph, xy, nodelist=list(n138), node_color='orange', node_size=20, linewidths=1)  # , alpha=0.3)
	...
	if plotdata:
		...
		plt.colorbar(matplotlib.cm.ScalarMappable(norm=norm, cmap=newcmap), ax=ax, shrink=0.5, label='Line Capacity (-)')
