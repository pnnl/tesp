...
def startWeatherAgent(file):
    ...
    # doing average of the forecast quantities to give more accurate representation of the hour
    ...
    if Helics_Flag:
        ...
        # config['coreInit'] = str('--federates=1')
        config['broker_port'] = broker_port
        config['coreName'] = str('Weather Federate')
        config['coreType'] = str('zmq')
        config['timeDelta'] = 1
        config['loglevel'] = "warning"
        config['logfile'] = str('output.log')
        config['publications'] = []
...
# if the time need to be published is real time
        if timeNeedToBePublished[i] in timeNeedToPublishRealtime:
            ...
            for key, value in row.items():
                ...
                pub = h.helicsFederateGetPublication(fed, str(fed_name + '/' + key))
                status = h.helicsPublicationPublishDouble(pub,value)
        ...
        if forecast == 1 and timeNeedToBePublished[i] in timeNeedToPublishForecast:
            ...
            for col in rows.columns:
                ...
                pub = h.helicsFederateGetPublication(fed, str(fed_name+ '/' + col + '/forecast'))
                status = h.helicsPublicationPublishString(pub,json.dumps(wd))
    ...
    if timeStopInSeconds not in timeNeedToPublish:
        ...
        time_granted = h.helicsFederateRequestTime(fed, timeStopInSeconds)
