...
Public Functions:
    :readtmy3: Read a TMY3 file in to a pandas dataframe, using python tkinter.
    :weathercsv: Converts TMY3 weather data to CSV in the requested time range.
    :weathercsv_cloudy_day: Converts weather data to cloudy days CSV in the requested time range.
"""
...
def weathercsv(tmyfile, output_file, start_time, end_time, year):
    ...
    result.index = result.index + pd.DateOffset(year=year)  # change all the year index to a same one
    result2 = result.resample(rule='5Min', closed='left').first()
    result2 = result2.interpolate(method='quadratic')
    # get the weather data for give time period and remove the improper value generated by interpolation
    dts = result2.loc[start_time:end_time, :].copy()
    dts.loc[dts.solar_direct < 1e-4, 'solar_direct'] = 0
    dts.loc[dts.solar_diffuse < 1e-4, 'solar_diffuse'] = 0
    dts.loc[dts.wind_speed < 1e-4, 'wind_speed'] = 0
    dts.to_csv(output_file, float_format='%.4f')
...
def weathercsv_cloudy_day(start_time, end_time, output_file):
    ...
    day_weather.to_csv(output_file)
