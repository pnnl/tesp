...
def read_inv_metrics(path, nameroot, dictname='', save_file=None, save_only=False):
    """ Plots inverter and volt-var data for the NIST TE Challenge 2 / IEEE 8500 examples
    ...
    *inverter_nameroot_metrics.json* for the data;
    ...
    Args:
      ...
      save_only (Boolean): set True with *save_file* to skip the display of the plot. Otherwise, script waits for user keypress.
    """
    inv_dict_path = os.path.join(path, f'{nameroot}_glm_dict.json')
    house_dict_path = os.path.join(path, f'house_{nameroot}_metrics.json')
    billing_meter_dict_path = os.path.join(path, f'billing_meter_{nameroot}_metrics.json')
    inverter_dict_path = os.path.join(path, f'inverter_{nameroot}_metrics.json')
    capacitor_dict_path = os.path.join(path, f'capacitor_{nameroot}_metrics.json')
    regulator_dict_path = os.path.join(path, f'regulator_{nameroot}_metrics.json')
    ...
    lst_h = json.loads(lp_h)
    lp_m = open(billing_meter_dict_path).read()
    lst_m = json.loads(lp_m)
    lp_i = open(inverter_dict_path).read()
    lst_i = json.loads(lp_i)
    lp_c = open(capacitor_dict_path).read()
    lst_c = json.loads(lp_c)
    lp_r = open(regulator_dict_path).read()
    lst_r = json.loads(lp_r)
...
def plot_inv(plt, save_file=None, save_only=False):
    plt.tight_layout(pad=1.0, w_pad=1.0, h_pad=2.0)
    if save_file is not None:
        plt.savefig(save_file)
    if not save_only:
...
def process_ninv(nameroot, title=None, save_file=None, save_only=False):
    """ Plots
    ...
    """
    path = os.getcwd()
    dict = read_inv_metrics(path, nameroot)
    plot_inv(dict, title, save_file, save_only)
