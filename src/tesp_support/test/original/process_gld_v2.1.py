...
print("\n\nFile", casename, "has substation <", sub_keys[0], ">at Matpower bus<", matBus, ">with", xfMVA,
      "MVA transformer")
print("\nFeeder Dictionary:")
for key in sub_keys:
...
print('\n\n solar inverter metrics: ',
      mp_inverter_metrics.sel_points(inverterID=mp_inverter_metrics.attrs['solar_inverter_ids']).data_vars)

print('\n\n battery inverter metrics: ',
      mp_inverter_metrics.sel_points(inverterID=mp_inverter_metrics.attrs['battery_inverter_ids']).data_vars)

# total solar generated energy
print('\n\n total solar energy [wh]: ', (mp_inverter_metrics.sel_points(
    inverterID=mp_inverter_metrics.attrs['solar_inverter_ids']).real_power_avg * time_interval_hours).sum(
    dim='time').values)

# total battery output (power > 0) energy
print('\n\n total battery generated energy [wh]: ', (
            mp_inverter_metrics.where(mp_inverter_metrics.real_power_avg > 0).sel_points(
                inverterID=mp_inverter_metrics.attrs['battery_inverter_ids']).real_power_avg * time_interval_hours).sum(
    dim='time').values)

# total batter charging (avg power < 0) energy
print('\n\n total battery charging energy [wh]: ', (
            mp_inverter_metrics.where(mp_inverter_metrics.real_power_avg < 0).sel_points(
                inverterID=mp_inverter_metrics.attrs['battery_inverter_ids']).real_power_avg * time_interval_hours).sum(
    dim='time').values)

# total battery net energy
print('\n\n battery net energy [wh]: ', (mp_inverter_metrics.sel_points(
    inverterID=mp_inverter_metrics.attrs['battery_inverter_ids']).real_power_avg * time_interval_hours).sum(
    dim='time').values)
