...
{
//std::cout << "reset_metric, reset: " << std::endl;
...
//  if(canPrint){
//std::cout << "continue_metric, reset: " << std::endl;
//std::cout << "continue_metric pVals[METRICS_LST]: " << pVals[METRICS_LST] << std::endl;
//std::cout << "continue_metric pVals[METRICS_MIN]: " << pVals[METRICS_MIN] << std::endl;
//std::cout << "continue_metric pVals[METRICS_MAX]: " << pVals[METRICS_MAX] << std::endl;
//std::cout << "continue_metric pVals[METRICS_SUM]: " << pVals[METRICS_SUM] << std::endl;
//std::cout << "continue_metric pVals[METRICS_CNT]: " << pVals[METRICS_CNT] << std::endl;
//canPrint = false;
//  }
}
...
//  if(canPrint){
//std::cout << "update_metric pVals[METRICS_LST]: " << pVals[METRICS_LST] << std::endl;
//std::cout << "update_metric pVals[METRICS_MIN]: " << pVals[METRICS_MIN] << std::endl;
//std::cout << "update_metric pVals[METRICS_MAX]: " << pVals[METRICS_MAX] << std::endl;
//std::cout << "update_metric pVals[METRICS_SUM]: " << pVals[METRICS_SUM] << std::endl;
//std::cout << "update_metric pVals[METRICS_CNT]: " << pVals[METRICS_CNT] << std::endl;
//canPrint = false;
//  }
}
...
{
//std::cout << "output_metrics: " << std::endl;
  double *pVals;
...
//    }
  ...
  bldg[key2] = ary;
  root[key1] = bldg;
//std::cout << "output_metrics done " << std::endl;
}
  ...
  if (argc > 9) { // create the optional HELICS federate, publications and subscriptions
    ...
    for (int i = 0; i < sub_count; i++) {
      ...
      if (sub.isValid() ) {
        ...
        msubs[thisInfo] = sub;
        //cout << " sub " << i << ":" << sub.getTarget() << ":" << sub.getKey() << ":" << sub.getType() << ":" << sub.getUnits() << ":" << sub.getInfo() << endl;
...
//  cout << "HELICS time_stop " << time_stop << endl;
...
//  }
  ...
  do {
//    helics_federate vfp = pHelicsFederate;
    auto currentTime = pHelicsFederate->getCurrentTime();
//    cout << "HELICS currentTime " << currentTime << endl;
//    time_granted = pHelicsFederate->requestTime(time_stop);
//    cout << "HELICS time requested: " << time_stop << ", HELICS time_granted " << time_granted << endl;
...
//        cout << "new price " << price << " at HELICS time " << time_granted << " and FNCS time " << time_granted << endl;
//      }
...
//    std::cout << "to_string(delta): " << to_string(delta) << std::endl;
...
//    cout << "HELICS time requested: " << nextTime << ", HELICS time_granted " << time_granted << endl;
