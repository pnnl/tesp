# FNCS_ARG_PARSE(ARG, VAR_LIBS, VAR_LDFLAGS, VAR_CPPFLAGS)
# --------------------------------------------------------
...
# $arg still unknown, look for "lib" and "include" anywhere...
    AS_IF([test "x$fncs_arg_parse_ok" = xno],
        [AS_CASE([$arg],
            [*lib*],    [AS_IF([test -d $arg], [$3="$$3 -L$arg"; fncs_arg_parse_ok=yes])],
            [*include*],[AS_IF([test -d $arg], [$4="$$4 -I$arg"; fncs_arg_parse_ok=yes])])])
