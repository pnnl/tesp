...
%% Using This Script
% This scripts is used to generate the populated feeder models for
% GridLAB-D. This models include the appropriate objects to enable
% co-simulation vis FNCS. Inputs that may need adjusting:
%
%   - taxonomy_files - Unpopulated models that will be populated. These
%   modles must exist in the "taxonomy_directory".
%   - taxonomy_directory - Location of the unpopulated model files that
%   will (or could be) populated.
%   - output_directory - Location where populated feeder models will be
%   placed.
%   - rng_seed - Random nubmer seed that is used when defining various
%   populated model parameters such as thermostat set-points, cost-comfort
%   preferences, etc.
%
% Each region has a default weather file assigned to it; these files can
% be changed by editing "regionalization_TSP.m". If new weather files are
% defined please add them to the "Supporting feeder model files" directory.
% Changes in weather files may result in overloading of some system
% components. This may be ameliorated by adjusting the "data.avg_house"
% value in "TaxFeederData.m".

%%

% Set technology to test
TechnologyToTest=0;
    % 0 - Base with controller messages for FNCS

% Load the user configuration for different files, flags, etc.
LUSolverVal = 1;
taxonomy_files =   {'R1-12.47-1.glm'}; 
...
%output_directory = './Output feeders/';
...
for tax_ind=1:no_of_tax 
    ...
    regional_data = regionalization_TSP(region);
    ...
    %  They get overridden inside of TechnologyParameters_TSP.m, if a tech_flag
    %  is set'
    use_flags.fncs_powerflow = 1;
    use_flags.fncs_transactive = 1;
    
    %% Residential Homes? - FINISHED
    use_flags.use_homes = 1;

    % Batteries? - FINISHED
    use_flags.use_batt = 1; %1 = add a battery at every center tap, 2 = add at substation, 0 = none
    ...
    use_flags.use_solar_res = 1;  % for residential solar
    ...
    [tech_data,use_flags] = TechnologyParameters_TSP(use_flags,TechnologyToTest);
    ...
    [s1,s2,s3,s4,s5,s6] = RandStream.create('mrg32k3a','NumStreams',6, 'Seed',rng_seed);
    ...
    if use_flags.fncs_powerflow == 1 || use_flags.fncs_transactive == 1
        fprintf(write_file,'module connection;\n');
    end
    ...
    if (metrics_interval > 0)
        fprintf(write_file,'object metrics_collector_writer {\n');
        fprintf(write_file,'     interval %d;\n', metrics_interval);
        fprintf(write_file,'     filename %s_metrics.json;\n', file4);
        fprintf(write_file,'};\n\n');
    end
...
%         fprintf(write_file,'};\n\n');
    ...
    fprintf(write_file,'     base_power 10MVA;\n');
    fprintf(write_file,'     power_convergence_value 100VA;\n');
    fprintf(write_file,'     phases ABCN;\n');
    if (metrics_interval > 0)
        fprintf(write_file,'     object metrics_collector {\n');
        fprintf(write_file,'         interval %d;\n', metrics_interval);
        fprintf(write_file,'     };\n');
    end
    if (use_flags.fncs_powerflow == 0)
        fprintf(write_file,'     object player {\n');
    ...
    if (use_flags.fncs_powerflow == 1) || (use_flags.fncs_transactive == 1)
        fprintf(write_file,'object fncs_msg {\n');
        fprintf(write_file,'     name gridlabdSimulator%d;\n',tax_ind);
        fprintf(write_file,'     parent network_node;\n');
        fprintf(write_file,'     configure %s;\n', fncs_config_filename);  
        fprintf(write_file,'     option "transport:hostname localhost, port 5570";\n');
        fprintf(write_file,'}\n\n');
    end
    
    if (use_flags.fncs_powerflow == 1)
        %Publish the complex power, subscribe to three-phase voltage
        fprintf(fncs_fid,'publish "commit:network_node.distribution_load -> distribution_load; 1000";\n');
        fprintf(fncs_fid,'subscribe "precommit:network_node.positive_sequence_voltage <- matpower/three_phase_voltage_B7";\n');
    end
...
%                        fprintf(write_file,'      power_12_reac street_lighting*%.4f;\n',bb_imag*tech_data.light_scalar_res);
                        fprintf(write_file,'      power_12_real 10.0;\n');
                        fprintf(write_file,'      power_12_reac 8.0;\n');
                    end
                end
            else
                fprintf(write_file,'%s %s %s %s %s %s %s %s\n',char(glm_final{1}{j}),char(glm_final{2}{j}),char(glm_final{3}{j}),char(glm_final{4}{j}),char(glm_final{5}{j}),char(glm_final{6}{j}),char(glm_final{7}{j}),char(glm_final{8}{j}));
    ...
    if (use_flags.use_homes == 1 && total_houses ~= 0)
        ...
        if (tech_data.get_IEEE_stats == 0)
            for jj=1:aS
                ...
                fncs_parent = strrep(parent2, ';','');
                fncs_parent = strrep(fncs_parent, '-','_');
                ...
                for kk=1:no_houses
                    ...
                    if ((use_flags.use_billing > 0) && (metrics_interval > 0))
                        fprintf(write_file,'      object metrics_collector {\n');
                        fprintf(write_file,'          interval %d;\n', metrics_interval);
                        fprintf(write_file,'      };\n');
                    end
                    ...
                    %Adding house-only meter
                    fprintf(write_file,'object triplex_meter {\n');
                    fprintf(write_file,'      name house_meter%d_%s\n',kk,parent2);
                    fprintf(write_file,'      parent tpm%d_%s\n',kk,parent2);
                    fprintf(write_file,'      phases %s\n',phase);
                    fprintf(write_file,'      nominal_voltage 120;\n');
                    fprintf(write_file,'      groupid Residential_tm_house;\n');
                    fprintf(write_file,'}\n');
                    
                    
                    fprintf(write_file,'object house {\n');
                    fprintf(write_file,'     name house%d_%s\n',kk,parent2);
                    fprintf(write_file,'     parent house_meter%d_%s\n',kk,parent2);
                    ...
                    if (use_flags.use_market == 0 || use_flags.use_market == 3) % normal and dlc
                        if (use_flags.fncs_transactive == 1)
                                fprintf(fncs_fid,'publish "commit:house%d_%s.air_temperature -> house%d_%s/air_temperature";\n',kk,fncs_parent,kk,fncs_parent);
                                fprintf(fncs_fid,'publish "commit:house%d_%s.power_state -> house%d_%s/power_state";\n',kk,fncs_parent,kk,fncs_parent);
                                fprintf(fncs_fid,'publish "commit:house%d_%s.hvac_load -> house%d_%s/hvac_load";\n',kk,fncs_parent,kk,fncs_parent);
                                fprintf(fncs_fid,'subscribe "precommit:house%d_%s.cooling_setpoint <- house%d_%s_thermostat_controller/cooling_setpoint";\n',kk,fncs_parent,kk,fncs_parent);
...
%                         fprintf(write_file,'     heating_setpoint heating%d*%.2f+%.2f;\n',heating_set,heat_night_diff/new_rand,heat_night);
                    ...
                    fprintf(write_file,'           //Used to be responsive load\n');           
                    fprintf(write_file,'           schedule_skew %.0f;\n',skew_value);
...
%                             fprintf(write_file,'         };\n');
                    ...
                    if (heat_type > (1 - regional_data.wh_electric) && tech_data.use_wh == 1)
                        ...
                        fprintf(write_file,'          tank_diameter 1.5;\n');                    
...
%                             fprintf(write_file,'          };\n');
    ...
    num_three_ph_loads = 0;
    for idx=1:no_loads
        if strcmp(load_houses{1,5}{idx}, 'ABCN;')
            num_three_ph_loads = num_three_ph_loads + 1;
            three_phase_list(num_three_ph_loads) = idx;
        end
    end
    
    if num_commercial > num_three_ph_loads
        msgID = 'feeder_generator:TooManyCommLoads';
        msg = sprintf('More three-phase commerical loads were requested (%d) than were available in unpopulated model (%d)',num_commercial, num_three_ph_loads);
        baseException = MException(msgID,msg);
        throw(baseException)
    else
        no_loads = num_commercial;
    end
    ...
    if (no_loads ~= 0 && use_flags.use_commercial == 1)
        ...
        fprintf(write_file,'      name line_configuration_commABCN;\n');     
        ...
        for idx = 1:no_loads %iii = 1:r2
            iii = three_phase_list(idx);
            my_name = strrep(load_houses{1,4}{iii},';','');
            
            fprintf(write_file,'object overhead_line {\n');
            fprintf(write_file,'     from %s;\n',my_name);
            fprintf(write_file,'     to %s_Eplus_meter%.0f;\n',my_name,iii);
            fprintf(write_file,'     phases ABCN;\n');
            fprintf(write_file,'     length 50ft;\n');
            fprintf(write_file,'     configuration line_configuration_commABCN;\n');
            fprintf(write_file,'}\n\n');
            
            
            fprintf(write_file,'object meter {\n');            
            fprintf(write_file,'     phases ABCN;\n');
            fprintf(write_file,'     name %s_Eplus_meter%.0f;\n',my_name,iii);
            ...
            fprintf(write_file,'     name %s_Eplus_load%.0f;\n',my_name,iii);
            fprintf(write_file,'     parent %s_Eplus_meter%.0f;\n',my_name,iii);
            fprintf(write_file,'     phases ABCN;\n');
            fprintf(write_file,'     nominal_voltage %f;\n',taxonomy_data.nom_volt2);
            fprintf(write_file,'     constant_power_A 25 kVA;\n');
            fprintf(write_file,'     constant_power_B 25 kVA;\n');
            fprintf(write_file,'     constant_power_C 25 kVA;\n');
            fprintf(write_file,'}\n\n');
            
            if (use_flags.fncs_powerflow == 1)
                %Publish the complex power, subscribe to three-phase voltage
                fprintf(fncs_fid,'subscribe "precommit:%s_Eplus_load%.0f.constant_power_A <- eplus_json/power_A";\n',my_name,iii);
                fprintf(fncs_fid,'subscribe "precommit:%s_Eplus_load%.0f.constant_power_B <- eplus_json/power_B";\n',my_name,iii);
                fprintf(fncs_fid,'subscribe "precommit:%s_Eplus_load%.0f.constant_power_C <- eplus_json/power_C";\n',my_name,iii);
            end
...
%             % 15 zones
%             if  (use_flags.fncs_powerflow == 0) %Dummy test %(total_comm_houses > 15 && no_of_phases == 3)
...
%            elseif (use_flags.fncs_powerflow == 0) %Dummy test %(total_comm_houses > 6 && no_of_phases >= 2)
...
%         fprintf(write_file,'     generator_status OFFLINE;\n');
...
%             fprintf(write_file,'	 parent %s\n',parent);
%             fprintf(write_file,'	 name inverter_%.0f;\n',jj);
%             fprintf(write_file,'     generator_status OFFLINE;\n');
%             fprintf(write_file,'     generator_mode CONSTANT_PQ;\n');
%             fprintf(write_file,'     inverter_type FOUR_QUADRANT;\n');
%             fprintf(write_file,'     four_quadrant_control_mode LOAD_FOLLOWING;\n');
%             fprintf(write_file,'	 sense_object %s\n','xxxxxxx'); %meter for house + solar + battery
...
%             fprintf(write_file,'     charge_on_threshold %.0f;\n',5000);
...
%             
%             fprintf(write_file,'     base_efficiency %.2f;\n',tech_data.efficiency);
%             fprintf(write_file,'     state_of_charge %.0f;\n',0.5);
%             fprintf(write_file,'     generator_mode SUPPLY_DRIVEN;\n');
%             fprintf(write_file,'}\n\n');
%             
%             fprintf(write_file,'object battery {\n');
%             fprintf(write_file,'	 parent inverter_%.0f;\n',jj);
...
%             fprintf(write_file,'     battery_capacity %.0f;\n',13500);%Tesla Powerwall 2 
%             fprintf(write_file,'     base_efficiency %.2f;\n',tech_data.efficiency);
%             fprintf(write_file,'     state_of_charge %.0f;\n',0.5);
%             fprintf(write_file,'     generator_mode SUPPLY_DRIVEN;\n');
%             fprintf(write_file,'}\n\n');
...
% 
    ...
    if (use_flags.use_solar ~=0 || use_flags.use_solar_res ~=0)
         if (exist('solar_residential_array','var'))
            ...
            residential_penetration_PV = rand(total_no_residential,1);
            residential_penetration_ES = rand(total_no_residential,1);
                ...
                if (~isempty(solar_residential_array{jj}))            
                    ...
                    for jjj=1:length(solar_residential_array{jj})
                        ...
                        if (~isempty(solar_residential_array{jj}{jjj}))
                            ...
                            if residential_penetration_PV(residential_count) <= PV_penetration_factor
                                countSOLAR = countSOLAR + 1;   
                                fprintf(write_file,'object triplex_meter {\n');
                                fprintf(write_file,'      name solar_meter_%s\n',parent1);
                                fprintf(write_file,'      phases %s\n',phase1);
                                fprintf(write_file,'      parent tpm%d_%s\n',jjj,parent);
                                fprintf(write_file,'      nominal_voltage 120;\n');
                                fprintf(write_file,'      groupid Residential_tm_solar;\n\n');
                                
                                %Calculating size of solar array
                                floor_area = solar_residential_array_fa{jj}{jjj}; %In square feet
                                ...
                                array_power  = floor_area * sq_feet_to_sq_m * rated_insolation * array_efficiency;
                                inverter_power = array_power * inverter_undersizing;
                                        
                                fprintf(write_file,'      object inverter {\n');
                                fprintf(write_file,'           name solar_inverter_%s\n',parent1);
                                fprintf(write_file,'           phases %s\n',phase1);
                                fprintf(write_file,'           generator_status ONLINE;\n');
                                fprintf(write_file,'           generator_mode CONSTANT_PF;\n');
                                fprintf(write_file,'           inverter_type FOUR_QUADRANT;\n');
                                ...
                                fprintf(write_file,'           object solar {\n');                    
                                fprintf(write_file,'                 name solar_%s\n',parent1);
                                fprintf(write_file,'                 generator_mode SUPPLY_DRIVEN;\n');
                                fprintf(write_file,'                 generator_status ONLINE;\n');
                                fprintf(write_file,'                 panel_type SINGLE_CRYSTAL_SILICON;\n');
                                fprintf(write_file,'                 efficiency 0.2;\n');
                            ...
                            if residential_penetration_ES(residential_count) <= ES_penetration_factor
                            %Adding battery in at the same time


                            fprintf(write_file,'object triplex_meter {\n');
                            fprintf(write_file,'      name battery_meter_%s\n',parent1);
                            fprintf(write_file,'      phases %s\n',phase1);
                            fprintf(write_file,'      parent tpm%d_%s\n',jjj,parent);
                            fprintf(write_file,'      nominal_voltage 120;\n');
                            fprintf(write_file,'      groupid Residential_tm_battery;\n\n');

                            fprintf(write_file,'      object inverter {\n');
                            fprintf(write_file,'        name battery_inverter_%s\n',parent1);
                            fprintf(write_file,'        generator_status ONLINE;\n');
                            fprintf(write_file,'        generator_mode CONSTANT_PQ;\n');
                            fprintf(write_file,'        inverter_type FOUR_QUADRANT;\n');
                            fprintf(write_file,'        four_quadrant_control_mode LOAD_FOLLOWING;\n');
                            ...
                            fprintf(write_file,'            generator_mode CONSTANT_PQ;\n');
                            fprintf(write_file,'            battery_type LI_ION;\n');
                            fprintf(write_file,'            nominal_voltage %.0f;\n',480);%Tesla Powerwall 2 
                            fprintf(write_file,'            battery_capacity %.0f;\n',13500);%Tesla Powerwall 2 
                            fprintf(write_file,'            round_trip_efficiency %.2f;\n',tech_data.efficiency);
                            fprintf(write_file,'            state_of_charge %.2f;\n',0.5);
                            fprintf(write_file,'            generator_mode SUPPLY_DRIVEN;\n');
                            fprintf(write_file,'         };\n');
                            if (metrics_interval > 0)
                                fprintf(write_file,'        object metrics_collector {\n');
