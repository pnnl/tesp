...
distThresh -- maximum allowable edge length in the communication graph, as
              given by the Euclidean distance between the edges endpoint nodes (Default=1000)
plotGraphs -- Set to True if plots of the feeder and corresponding communication
              network are desired.
...
def synFeederComGraph(feedername, perRew=0.1, distThresh=1000, plotGraphs=False):
    ...
    if plotGraphs:
        ...
        plt.figure(2)
        list(rewxy.values())
        rewG.subgraph(list(rewplotnodes))
