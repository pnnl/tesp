...
print("Sum of Real power during 240s to 2040s ",mp_bus_metrics.where(np.logical_and(mp_bus_metrics.time < 2040,mp_bus_metrics.time > 240)).PD.sum())
...
gen_emission_rate= {'coal':[205.57*10.09, 0.1*10.09, 0.06*10.09],'gas_combinedcycle': [117.08*7.67,0.001*7.67,0.0075*7.67], 'gas_singlecycle':[117.08*11.37, 0.001*11.37,0.0075*11.37] }
...
for key in gen_keys:
    ...
    if(dict['generators'][key]['genfuel']) == 'gas':
        if ((dict['generators'][key]['gentype']== 'combinedcycle') |  (dict['generators'][key]['gentype'] == 'combinedcycle')):
            co2_emission_rate[j] = gen_emission_rate[dict['generators'][key]['genfuel']+"_"+dict['generators'][key]['gentype']][0]
            sox_emission_rate[j] = \
...
print('\ntotal generation energy of each generator: ',(mp_gen_metrics.PGEN*time_interval_hours).sum(dim = 'time').values)
