...
def parse_json_file(json_file, out_path, out_file_name):
    ...
    Args:
        json_file (JSON file) - JSON file of the GLD power distribution
        system model

        out_path (path) - Path to send the dataframe. Saves the
        dataframe as a csv.
    ...
    # each other. So, we go through the (key, value) paris of
