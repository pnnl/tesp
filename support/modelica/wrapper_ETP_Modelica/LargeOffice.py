...
class LargeOffice (object):
	def __init__(self, envelop_coefficients_file, init_file,fmu_location, start_day, duration, step_size, input_settings=None):
		self.modelType = "LO"
		...
		self.Q  = np.genfromtxt("./core/Q.csv", delimiter=',',max_rows=(start_day - 1 + duration)*1440+1)[(start_day-1)*1440+1:,1:]
	...
	def ETP_init(self):
		midNightResetTemp = np.genfromtxt(self.etp_init_file, delimiter=',', max_rows=self.startDay -1 + self.duration)[self.startDay-1:,1:] #mid-night reset
	...
	def step(self,TO,current_t,control_inputs,Q_current): #TODO: Q_current needs to be delete, testing with 1 zone here
