...
class LargeOffice (object):
	def __init__(self, envelop_coefficients_file, init_file,fmu_location, start_day, duration, step_size, input_settings):
		self.modelType = "LO"
		self.modelSubType = "Prototype2007"
		#self.model = load_fmu(fmu_location) #load fmu to call modelica system
		self.zoneNames = ["BASEMENT",	"CORE_BOTTOM",	"CORE_MID",	"CORE_TOP",	"PERIMETER_BOT_ZN_3",	"PERIMETER_BOT_ZN_2",	"PERIMETER_BOT_ZN_1",	"PERIMETER_BOT_ZN_4",	"PERIMETER_MID_ZN_3",	"PERIMETER_MID_ZN_2",	"PERIMETER_MID_ZN_1",	"PERIMETER_MID_ZN_4",	"PERIMETER_TOP_ZN_3",	"PERIMETER_TOP_ZN_2",	"PERIMETER_TOP_ZN_1",	"PERIMETER_TOP_ZN_4",	"GROUNDFLOOR_PLENUM",	"MIDFLOOR_PLENUM",	"TOPFLOOR_PLENUM"]
	...
	def step(self, current_t, curInputs,T_prev,TO_current,Q_current,TG):
		...
		N_zone = 19
		R = np.multiply(T_prev, self.co[N_zone])+ np.multiply(TO_current,self.co[N_zone+1])+np.multiply(Q_current,self.co[N_zone+2])+np.multiply(TG,self.co[N_zone+3])
